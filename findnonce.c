...
#define R(a, b, c, d, e, f, g, h, w, k)                                        \
  h = h + (rotate(e, 26) ^ rotate(e, 21) ^ rotate(e, 7)) +                     \
      (g ^ (e & (f ^ g))) + k + w;                                             \
  d = d + h;                                                                   \
  h = h + (rotate(a, 30) ^ rotate(a, 19) ^ rotate(a, 10)) +                    \
      ((a & b) | (c & (a | b)))
...
void precalc_hash(dev_blk_ctx *blk, uint32_t *state, uint32_t *data) {
  ...
  blk->W16 = blk->fW0 =
      data[0] + (rotr(data[1], 7) ^ rotr(data[1], 18) ^ (data[1] >> 3));
  blk->W17 = blk->fW1 =
      data[1] + (rotr(data[2], 7) ^ rotr(data[2], 18) ^ (data[2] >> 3)) +
      0x01100000;
  blk->PreVal4 = blk->fcty_e = blk->ctx_e +
                               (rotr(B, 6) ^ rotr(B, 11) ^ rotr(B, 25)) +
                               (D ^ (B & (C ^ D))) + 0xe9b5dba5;
  blk->T1 = blk->fcty_e2 =
      (rotr(F, 2) ^ rotr(F, 13) ^ rotr(F, 22)) + ((F & G) | (H & (F | G)));
  blk->PreVal4_2 = blk->PreVal4 + blk->T1;
  blk->PreVal0 = blk->PreVal4 + blk->ctx_a;
  blk->PreW31 =
      0x00000280 + (rotr(blk->W16, 7) ^ rotr(blk->W16, 18) ^ (blk->W16 >> 3));
  blk->PreW32 =
      blk->W16 + (rotr(blk->W17, 7) ^ rotr(blk->W17, 18) ^ (blk->W17 >> 3));
  blk->PreW18 =
      data[2] + (rotr(blk->W16, 17) ^ rotr(blk->W16, 19) ^ (blk->W16 >> 10));
  blk->PreW19 =
      0x11002000 + (rotr(blk->W17, 17) ^ rotr(blk->W17, 19) ^ (blk->W17 >> 10));

  blk->W2 = data[2];

  blk->W2A =
      blk->W2 + (rotr(blk->W16, 19) ^ rotr(blk->W16, 17) ^ (blk->W16 >> 10));
  blk->W17_2 =
      0x11002000 + (rotr(blk->W17, 19) ^ rotr(blk->W17, 17) ^ (blk->W17 >> 10));

  blk->fW2 =
      data[2] + (rotr(blk->fW0, 17) ^ rotr(blk->fW0, 19) ^ (blk->fW0 >> 10));
  blk->fW3 =
      0x11002000 + (rotr(blk->fW1, 17) ^ rotr(blk->fW1, 19) ^ (blk->fW1 >> 10));
  blk->fW15 =
      0x00000280 + (rotr(blk->fW0, 7) ^ rotr(blk->fW0, 18) ^ (blk->fW0 >> 3));
  blk->fW01r =
      blk->fW0 + (rotr(blk->fW1, 7) ^ rotr(blk->fW1, 18) ^ (blk->fW1 >> 3));
...
#define P(t)                                                                   \
  (W[(t)&0xF] = W[(t - 16) & 0xF] +                                            \
                (rotate(W[(t - 15) & 0xF], 25) ^                               \
                 rotate(W[(t - 15) & 0xF], 14) ^ (W[(t - 15) & 0xF] >> 3)) +   \
                W[(t - 7) & 0xF] +                                             \
                (rotate(W[(t - 2) & 0xF], 15) ^ rotate(W[(t - 2) & 0xF], 13) ^ \
                 (W[(t - 2) & 0xF] >> 10)))
...
static void *postcalc_hash(void *userdata) {
  ...
  for (entry = 0; entry < pcd->res[found]; entry++) {
    ...
    applog(LOG_DEBUG,
           "[THR%d] OCL NONCE %08x (%lu) found in slot %d (found = %d)",
           thr->id, nonce, nonce, entry, found);
...
void postcalc_hash_async(struct thr_info *thr, struct work *work,
                         uint32_t *res) {
