...
static struct profile *add_profile() {
  ...
  profiles = (struct profile **)realloc(profiles, sizeof(struct profile *) *
                                                      (total_profiles + 2));
...
extern struct profile *get_profile(char *name) {
  int i;
...
char *set_default_algorithm(const char *arg) {
  ...
  applog(LOG_INFO, "Set default algorithm to %s",
...
char *set_default_nfactor(const char *arg) {
  ...
  applog(LOG_INFO, "Set algorithm N-factor to %d (N to %d)",
         default_profile.algorithm.nfactor);
...
char *set_profile_kernelfile(const char *arg) {
  ...
  applog(LOG_DEBUG, "Setting profile %s algorithm kernel file to %s",
         profile->name, arg);
...
* Helper Functions
****************************************/
extern json_t *json_sprintf(const char *fmt, ...) {
  va_list args;
  char *buf;
  size_t bufsize;
...
void parse_config_object(json_t *obj, const char *parentkey, bool fileconf,
...
static char *parse_config_array(json_t *obj, char *parentkey, bool fileconf) {
  ...
  // fix parent key - remove extra "s" to match opt names (e.g.
  // --pool-gpu-memclock not --pools-gpu-memclock)
  if (!strcasecmp(parentkey, "pools") || !strcasecmp(parentkey, "profiles") ||
      !strcasecmp(parentkey, "events"))
...
char *parse_config(json_t *val, const char *key, const char *parentkey,
  ...
  /*
  parse the json config items into opts instead of looking for opts in the json
  config...
  ...
  */
  ...
  default:
    ...
    sprintf(optname, "--%s%s%s", ((!empty_string(parentkey)) ? parentkey : ""),
            ((!empty_string(parentkey)) ? "-" : ""), key);
...
#ifdef HAVE_LIBCURL
  extern char *load_config_withlibcurl(const char *arg, const char *parentkey,
                  void __maybe_unused *unused);
...
void apply_pool_profile(struct pool *pool) {
  ...
  if (!empty_string(pool->profile)) {
    applog(LOG_DEBUG, "Loading settings from profile \"%s\" for pool %i",
           pool->profile, pool->pool_no);

    // find profile and apply settings to the pool
    if (!(profile = get_profile(pool->profile))) {
      // if not found, remove profile name and use default profile.
      applog(LOG_DEBUG, "Profile load failed for pool %i: profile %s not "
                        "found. Using default profile.",
             pool->pool_no, pool->profile);
      // remove profile name
      pool->profile[0] = '\0';
  ...
  // no profile specified in pool, use default profile
    ...
    applog(LOG_DEBUG, "Loading settings from default_profile for pool %i",
           pool->pool_no);
    profile = &default_profile;
  }
  ...
  applog(LOG_DEBUG, "Pool %i Algorithm set to \"%s\"", pool->pool_no,
         pool->algorithm.name);
  ...
  if (empty_string(pool->algorithm.kernelfile)) {
    ...
    if (!empty_string(profile->algorithm.kernelfile)) {
      ...
      applog(LOG_DEBUG, "Pool %i Kernel File set to \"%s\"", pool->pool_no,
             pool->algorithm.kernelfile);
  ...
  applog(LOG_DEBUG, "Pool %i devices set to \"%s\"", pool->pool_no,
         pool->devices);
  ...
  applog(LOG_DEBUG, "Pool %i lookup gap set to \"%s\"", pool->pool_no,
         pool->lookup_gap);
  ...
  switch (int_type) {
  case 2:
    applog(LOG_DEBUG, "Pool %d Raw Intensity set to \"%s\"", pool->pool_no,
           pool->rawintensity);
    break;

  case 1:
    applog(LOG_DEBUG, "Pool %d XIntensity set to \"%s\"", pool->pool_no,
           pool->xintensity);
    break;

  default:
    applog(LOG_DEBUG, "Pool %d Intensity set to \"%s\"", pool->pool_no,
           pool->intensity);
    break;
  }
  ...
  applog(LOG_DEBUG, "Pool %i Thread Concurrency set to \"%s\"", pool->pool_no,
  ...
  applog(LOG_DEBUG, "Pool %i GPU Clock set to \"%s\"", pool->pool_no,
         pool->gpu_engine);
  ...
  applog(LOG_DEBUG, "Pool %i GPU Memory clock set to \"%s\"", pool->pool_no,
         pool->gpu_memclock);
  ...
  applog(LOG_DEBUG, "Pool %i GPU Threads set to \"%s\"", pool->pool_no,
         pool->gpu_threads);
  ...
  applog(LOG_DEBUG, "Pool %i GPU Fan set to \"%s\"", pool->pool_no,
         pool->gpu_fan);
  ...
  applog(LOG_DEBUG, "Pool %i GPU Powertune set to \"%s\"", pool->pool_no,
         pool->gpu_powertune);
  ...
  applog(LOG_DEBUG, "Pool %i GPU Vddc set to \"%s\"", pool->pool_no,
         pool->gpu_vddc);
#endif
  ...
  applog(LOG_DEBUG, "Pool %i Shaders set to \"%s\"", pool->pool_no,
         pool->shaders);
  ...
  applog(LOG_DEBUG, "Pool %i Worksize set to \"%s\"", pool->pool_no,
         pool->worksize);
}
...
void api_profile_list(struct io_data *io_data, __maybe_unused SOCKETTYPE c,
  ...
  for (i = 0; i <= total_profiles; i++) {
    ...
    // if default profile name is profile name or loop index is beyond the last
    // profile, then it is the default profile
    if ((b = (((i == total_profiles) ||
               (!strcasecmp(default_profile.name, profile->name)))
                  ? true
                  : false)))
      default_done = true;
    ...
    root = api_add_escape(root, "Algorithm",
                          isnull((char *)profile->algorithm.name, ""), true);
    root = api_add_escape(root, "Algorithm Type",
                          (char *)algorithm_type_str[profile->algorithm.type],
                          false);

    // show nfactor for nscrypt
    if (profile->algorithm.type == ALGO_NSCRYPT)
      root = api_add_int(root, "Algorithm NFactor",
                         (int *)&(profile->algorithm.nfactor), false);

    root = api_add_escape(root, "LookupGap",
                          isnull((char *)profile->lookup_gap, ""), true);
    root = api_add_escape(root, "Devices", isnull((char *)profile->devices, ""),
                          true);
    root = api_add_escape(root, "Intensity",
                          isnull((char *)profile->intensity, ""), true);
    root = api_add_escape(root, "XIntensity",
                          isnull((char *)profile->xintensity, ""), true);
    root = api_add_escape(root, "RawIntensity",
                          isnull((char *)profile->rawintensity, ""), true);
    root = api_add_escape(root, "Gpu Engine",
                          isnull((char *)profile->gpu_engine, ""), true);
    root = api_add_escape(root, "Gpu MemClock",
                          isnull((char *)profile->gpu_memclock, ""), true);
    root = api_add_escape(root, "Gpu Threads",
                          isnull((char *)profile->gpu_threads, ""), true);
    root = api_add_escape(root, "Gpu Fan%",
                          isnull((char *)profile->gpu_fan, ""), true);
    root = api_add_escape(root, "Gpu Powertune%",
                          isnull((char *)profile->gpu_powertune, ""), true);
    root = api_add_escape(root, "Gpu Vddc",
                          isnull((char *)profile->gpu_vddc, ""), true);
    root = api_add_escape(root, "Shaders", isnull((char *)profile->shaders, ""),
                          true);
    root =
        api_add_escape(root, "Thread Concurrency",
                       isnull((char *)profile->thread_concurrency, ""), true);
    root = api_add_escape(root, "Worksize",
                          isnull((char *)profile->worksize, ""), true);
...
void api_profile_add(struct io_data *io_data, __maybe_unused SOCKETTYPE c,
  ...
  // split all params by colon (:) because the comma (,) can be used in some of
  // those parameters
...
void api_profile_remove(struct io_data *io_data, __maybe_unused SOCKETTYPE c,
                        char *param, bool isjson, __maybe_unused char group) {
  struct profile *profile;
  struct pool *pool;
  int i;
...
void api_pool_profile(struct io_data *io_data, __maybe_unused SOCKETTYPE c,
                      char *param, bool isjson, __maybe_unused char group) {
  struct profile *profile;
  struct pool *pool;
  char *p;
...
void update_config_intensity(struct profile *profile) {
  ...
  // if this profile is also default profile, make sure to set the
  // default_profile structure value
