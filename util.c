...
extern void suffix_string_double(double val, char *buf, size_t bufsiz,
                                 int sigdigits);
...
static void keep_sockalive(SOCKETTYPE fd) {
  ...
  setsockopt(fd, SOL_SOCKET, SO_KEEPALIVE, (const char *)&tcp_one,
             sizeof(tcp_one));
  if (!opt_delaynet)
#ifndef __linux
    setsockopt(fd, IPPROTO_TCP, TCP_NODELAY, (const char *)&tcp_one,
               sizeof(tcp_one));
#else  /* __linux */
    setsockopt(fd, SOL_TCP, TCP_NODELAY, (const void *)&tcp_one,
               sizeof(tcp_one));
...
/* Subtract b from a */
static void __maybe_unused timersubspec(struct timespec *a,
...
int cgtimer_to_ms(cgtimer_t *cgt) { return (int)(cgt->QuadPart / 10000LL); }
...
#else  /* WIN32 */
static void cgsleep_spec(struct timespec *ts_diff,
...
bool extract_sockaddr(char *url, char **sockaddr_url, char **sockaddr_port) {
  ...
  *sockaddr_url = url;
  ...
  if (url_len >= sizeof(url_address)) {
    applog(LOG_WARNING, "%s: Truncating overflowed address '%.*s'", __func__,
           url_len, url_begin);
...
enum send_ret { SEND_OK, SEND_SELECTFAIL, SEND_SENDFAIL, SEND_INACTIVE };
...
char *recv_line(struct pool *pool) {
  ...
  if ((tok = strtok(pool->sockbuf, "\n")) == NULL) {
    applog(LOG_DEBUG,
           "Failed to parse a \\n terminated string in recv_line: buffer = %s",
           pool->sockbuf);
    goto out;
  }
  sret = strdup(tok);
...
static bool parse_notify(struct pool *pool, json_t *val) {
  ...
  if (!job_id || !prev_hash || !coinbase1 || !coinbase2 || !bbversion ||
      !nbit || !ntime || (has_trie && !trie)) {
  ...
  alloc_len = pool->swork.cb_len =
      cb1_len + pool->n1_len + pool->n2size + cb2_len;
  pool->nonce2_offset = cb1_len + pool->n1_len;

  for (i = 0; i < pool->swork.merkles; i++)
    free(pool->swork.merkle_bin[i]);
  if (merkles) {
    pool->swork.merkle_bin = (unsigned char **)realloc(
        pool->swork.merkle_bin, sizeof(char *) * merkles + 1);
    for (i = 0; i < merkles; i++) {
  ...
  if (clean)
    pool->nonce2 = 0;
  pool->merkle_offset =
      strlen(pool->swork.bbversion) + strlen(pool->swork.prev_hash);
  pool->merkle_offset /= 2;
  header = (char *)alloca(257);
  snprintf(header, 257, "%s%s%s%s%s%s%s", pool->swork.bbversion,
           pool->swork.prev_hash, blank_merkel, has_trie ? trie : "",
           pool->swork.ntime, pool->swork.nbit, "00000000" /* nonce */
           );
  header_len = strlen(header);
  memset(header + header_len, '0', 256 - header_len);
  header[256] = '\0';
  if (unlikely(!hex2bin(pool->header_bin, header, 128))) {
    applog(LOG_WARNING, "%s: Failed to convert header to header_bin, got %s",
           __func__, header);
    pool_failed(pool);
...
static bool parse_notify_ethash(struct pool *pool, json_t *val) {
  ...
  // EthereunStratum
  // Unknown stratum msg:
  // {"params":["00000037309a282c","dc5ebd0b5d7d952125aae1c58ca4abd1d7b4b5acd0c4ca307af5dddd52dff161","7e5d93aaa464736820489f345c8f2f2946fe0a13ad694467bf3fd0962aaeb249",true],"id":null,"method":"mining.notify"}
  //                                        string job =
  //                                        params.get((Json::Value::ArrayIndex)0,
  //                                        "").asString();
  //                                        string sSeedHash =
  //                                        params.get((Json::Value::ArrayIndex)1,
  //                                        "").asString();
  //                                       string sHeaderHash =
  //                                       params.get((Json::Value::ArrayIndex)2,
  //                                       "").asString();
  //
  //                                        if (sHeaderHash != "" && sSeedHash
  //                                        != "")
  //                                        {
  //                                               cnote << "Received new job #"
  //                                               + job;
  //
  //                                                h256 seedHash =
  //                                                h256(sSeedHash);
  ...
  //                                                h256(sHeaderHash);
  //
  //                                                m_previous.headerHash =
  //                                                m_current.headerHash;
  //                                                m_previous.seedHash =
  //                                                m_current.seedHash;
  //                                                m_previous.boundary =
  //                                                m_current.boundary;
  //                                                m_previous.startNonce =
  //                                                m_current.startNonce;
  //                                                m_previous.exSizeBits =
  //                                                m_previous.exSizeBits;
  //                                                m_previousJob = m_job;
  //
  //                                                m_current.headerHash =
  ...
  //                                               diffToTarget((uint32_t*)m_current.boundary.data(),
  //                                               m_nextWorkDifficulty);
  //                                               m_current.startNonce =
  //                                               ethash_swap_u64(*((uint64_t*)m_extraNonce.data()));
  //                                               m_current.exSizeBits =
  //                                               m_extraNonceHexSize * 4;
  //                                               m_job = job;
  //
  ...
  //	if (SeedHashStr == NULL || EthWorkStr == NULL || TgtStr == NULL) {
  ...
  ret &=
      hex2bin(EthWork, EthWorkStr + 2, 32) || hex2bin(EthWork, EthWorkStr, 32);

  ret &= hex2bin(SeedHash, SeedHashStr + 2, 32) ||
         hex2bin(SeedHash, SeedHashStr, 32);
...
static bool parse_diff(struct pool *pool, json_t *val) {
  ...
  if (opt_diff_mult == 0.0)
    diff = json_number_value(json_array_get(val, 0)) *
           pool->algorithm.diff_multiplier1;
  else
  ...
  if (old_diff != diff) {
    ...
    if ((double)idiff == diff) {
      applog(pool == current_pool() ? LOG_NOTICE : LOG_DEBUG,
             "%s difficulty changed to %d", get_pool_name(pool), idiff);
    } else {
      applog(pool == current_pool() ? LOG_NOTICE : LOG_DEBUG,
             "%s difficulty changed to %.3f", get_pool_name(pool), diff);
    }
  } else {
...
static bool parse_reconnect(struct pool *pool, json_t *val) {
  ...
  applog(LOG_NOTICE, "Reconnect requested from %s to %s", get_pool_name(pool),
         address);
...
static bool send_version(struct pool *pool, json_t *val) {
  ...
  sprintf(s, "{\"id\": %d, \"result\": \"" PACKAGE "/" CGMINER_VERSION
             "\", \"error\": null}",
          id);
...
bool parse_method(struct pool *pool, char *s) {
  ...
  if (!(val = JSON_LOADS(s, &err))) {
    applog(LOG_INFO, "parse_method (util.c) JSON decode failed(%d): %s",
           err.line, err.text);
    return ret;
  }
  ...
  if (!strncasecmp(buf, "mining.notify", 13)) {
    if (pool->algorithm.type == ALGO_ETHASH) {
      if (parse_notify_ethash(pool, params))
...
bool subscribe_extranonce(struct pool *pool) {
  ...
  sprintf(s, "{\"id\": 2, \"method\": \"mining.extranonce.subscribe\", "
             "\"params\": []}");
  ...
  if (!res_val || json_is_false(res_val) ||
    ...
    if (err_val) {
      ...
      if (ss && (strcmp(ss, "Method 'subscribe' not found for service "
                            "'mining.extranonce'") == 0)) {
        applog(LOG_INFO, "Cannot subscribe to mining.extranonce on %s",
               get_pool_name(pool));
        ret = true;
        goto out;
      }
      ...
      ss = strdup("(unknown reason)");
    applog(LOG_INFO, "%s JSON stratum auth failed in subscribe_extranonce: %s",
           get_pool_name(pool), ss);
    free(ss);
    goto out;
  }

  ret = true;
  applog(LOG_INFO, "Stratum extranonce subscribe successful for %s",
         get_pool_name(pool));
out:
  json_decref(val);
  return ret;
...
bool auth_stratum(struct pool *pool) {
  ...
  bool ret = false;
  if (pool->algorithm.type == ALGO_CRYPTONIGHT) {
    sprintf(s, "{\"method\": \"login\", \"params\": {\"login\": \"%s\", "
               "\"pass\": \"%s\", \"agent\": \"%s/%s\"}, \"id\": 1}",
            pool->rpc_user, pool->rpc_pass, PACKAGE, VERSION);
    swork_id++;
  } else {
    sprintf(s, "{\"id\": %d, \"method\": \"mining.authorize\", \"params\": "
               "[\"%s\", \"%s\"]}",
  ...
  if (!res_val || json_is_false(res_val) ||
    ...
    if (err_val)
      ...
      ss = strdup("(unknown reason)");
    applog(LOG_INFO, "%s JSON stratum auth failed im auth_stratum: %s",
           get_pool_name(pool), ss);
    free(ss);
    suspend_stratum(pool);
    goto out;
...
static bool http_negotiate(struct pool *pool, int sockd, bool http0) {
  ...
  applog(LOG_DEBUG, "Sending proxy %s:%s - %s", pool->sockaddr_proxy_url,
  ...
  buf[len] = '\0';
  applog(LOG_DEBUG, "Received from proxy %s:%s - %s", pool->sockaddr_proxy_url,
         pool->sockaddr_proxy_port, buf);
...
static bool socks4_negotiate(struct pool *pool, int sockd, bool socks4a) {
  ...
  if ((int)inp != -1)
    ...
    if (!ret) {
      applog(LOG_ERR, "getaddrinfo() in socks4_negotiate() returned %i: %s",
...
static bool setup_stratum_socket(struct pool *pool) {
  ...
  if (ret) {
    applog(LOG_INFO, "getaddrinfo() in setup_stratum_socket() returned %i: %s",
           ret, gai_strerror(ret));
    if (!pool->probed) {
      applog(LOG_WARNING, "Failed to resolve (wrong URL?) %s:%s", sockaddr_url,
             sockaddr_port);
      pool->probed = true;
    } else {
      applog(LOG_INFO, "Failed to getaddrinfo for %s:%s", sockaddr_url,
             sockaddr_port);
    }
    return false;
  }
  ...
  if (p == NULL) {
    applog(LOG_INFO, "Failed to connect to stratum on %s:%s", sockaddr_url,
           sockaddr_port);
    freeaddrinfo(servinfo);
    return false;
  }
...
resend:
  if (!setup_stratum_socket(pool)) {
    ...
    applog(LOG_INFO, "setup_stratum_socket() on %s failed",
           get_pool_name(pool));
    sockd = false;
    goto out;
  }
  ...
  if (recvd) {
    ...
    if (pool->extranonce_subscribe) {
      // If we want extranonce support for a non-standard algo, adapt stratum
      // protocol accordingly
      if (pool->algorithm.type == ALGO_ETHASH) {
        // In the case of Ethereum, if we want extranonce then with it we want
        // EthereumStratum
        sprintf(s, "{\"id\": 1, \"method\": \"mining.subscribe\", \"params\": "
                   "[\"" PACKAGE "/" CGMINER_VERSION
  ...
  cg_wunlock(&pool->data_lock);
  if (sessionid)
    applog(LOG_DEBUG, "%s stratum session id: %s", get_pool_name(pool),
           pool->sessionid);
  ret = true;
out:
  if (ret) {
...
#ifdef __APPLE__
void _cgsem_init(cgsem_t *cgsem, const char *file, const char *func,
                 const int line) {
  int flags, fd, i;
...
void _cgsem_post(cgsem_t *cgsem, const char *file, const char *func,
                 const int line) {
  const char buf = 1;
  int ret;
retry:
...
void _cgsem_wait(cgsem_t *cgsem, const char *file, const char *func,
                 const int line) {
  char buf;
  int ret;
retry:
  ...
  if (unlikely(ret == 0))
    applog(LOG_WARNING, "Failed to read errno=%d" IN_FMT_FFL, errno, file, func,
           line);
  else if (unlikely(ret < 0 && interrupted))
    goto retry;
}
...
int _cgsem_mswait(cgsem_t *cgsem, int ms, const char *file, const char *func,
                  const int line) {
  struct timeval timeout;
  int ret, fd;
  fd_set rd;
...
void _cgsem_init(cgsem_t *cgsem, const char *file, const char *func,
  ...
  if ((ret = sem_init(cgsem, 0, 0)))
    quitfrom(1, file, func, line, "Failed to sem_init ret=%d errno=%d", ret,
             errno);
}
