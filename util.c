...
json_t *json_rpc_call(CURL * curl, char *curl_err_str, const char *url,
		      ...
		      bool probe, bool longpoll, int *rolltime, struct pool * pool, bool share)
{
	...
	for (i = 0; proxynames[i].name; i++) {
		...
		if (strncmp(url, proxynames[i].name, plen) == 0) {
			...
			*split = '\0';
			...
			extract_sockaddr(pool->rpc_proxy,
...
bool extract_sockaddr(char *url, char **sockaddr_url, char **sockaddr_port)
{
	...
	*sockaddr_url = url;
	...
	if (url_len >= sizeof(url_address)) {
		applog(LOG_WARNING, "%s: Truncating overflowed address '%.*s'", __func__, url_len,
		       url_begin);
...
char *recv_line(struct pool *pool)
{
	...
	if (!strstr(pool->sockbuf, "\n")) {
		...
		do {
			...
			if (n < 0) {
				if (!sock_blocks()
				    || !socket_full(pool, DEFAULT_SOCKWAIT - waited)) {
...
{
	char *job_id, *prev_hash, *coinbase1, *coinbase2, *bbversion, *nbit, *ntime, *header,
	    *trie = NULL;
	...
	if (merkles) {
		pool->swork.merkle_bin =
		    (unsigned char **)realloc(pool->swork.merkle_bin, sizeof(char *) * merkles + 1);
		for (i = 0; i < merkles; i++) {
	...
	snprintf(header, 257, "%s%s%s%s%s%s%s", pool->swork.bbversion, pool->swork.prev_hash, blank_merkel, has_trie ? trie : "", pool->swork.ntime, pool->swork.nbit, "00000000"	/* nonce */
	    );
	header_len = strlen(header);
	memset(header + header_len, '0', 256 - header_len);
	header[256] = '\0';
	if (unlikely(!hex2bin(pool->header_bin, header, 128))) {
		applog(LOG_WARNING, "%s: Failed to convert header to header_bin, got %s", __func__,
		       header);
		pool_failed(pool);
...
bool parse_diff_ethash(unsigned char *Target, char *TgtStr)
{
	...
	if (len != 64 && len != 66) {
		...
		ret = hex2bin(Target, TgtStr + 2, 32)
		    || hex2bin(Target, TgtStr, 32);
	return ret;
}
...
// EthereunStratum
// Unknown stratum msg: {"params":["00000037309a282c","dc5ebd0b5d7d952125aae1c58ca4abd1d7b4b5acd0c4ca307af5dddd52dff161","7e5d93aaa464736820489f345c8f2f2946fe0a13ad694467bf3fd0962aaeb249",true],"id":null,"method":"mining.notify"} 
...
//	if (SeedHashStr == NULL || EthWorkStr == NULL || TgtStr == NULL) {
	...
	ret &= hex2bin(EthWork, EthWorkStr + 2, 32)
	    || hex2bin(EthWork, EthWorkStr, 32);
...
static bool send_version(struct pool *pool, json_t * val)
{
	...
	sprintf(s, "{\"id\": %d, \"result\": \"" PACKAGE "/" CGMINER_VERSION "\", \"error\": null}",
		id);
...
bool parse_method(struct pool * pool, char *s)
{
	...
	if (!(val = JSON_LOADS(s, &err))) {
		applog(LOG_INFO, "parse_method (util.c) JSON decode failed(%d): %s", err.line,
		       err.text);
		return ret;
	}
...
bool subscribe_extranonce(struct pool * pool)
{
	...
	sprintf(s, "{\"id\": 2, \"method\": \"mining.extranonce.subscribe\", \"params\": []}");
	...
	if (!res_val || json_is_false(res_val)
		...
		if (err_val) {
			...
			if (ss
			    &&
			    (strcmp
			     (ss, "Method 'subscribe' not found for service 'mining.extranonce'")
			     == 0)) {
				applog(LOG_INFO,
			...
			ss = strdup("(unknown reason)");
		applog(LOG_INFO,
		       "%s JSON stratum auth failed in subscribe_extranonce: %s",
	...
	ret = true;
	applog(LOG_INFO, "Stratum extranonce subscribe successful for %s", get_pool_name(pool));
 out:
	json_decref(val);
	return ret;
}
...
static bool http_negotiate(struct pool *pool, int sockd, bool http0)
{
	...
	applog(LOG_DEBUG, "Received from proxy %s:%s - %s",
	       pool->sockaddr_proxy_url, pool->sockaddr_proxy_port, buf);
	if (strcmp(buf, "HTTP/1.1 200")
	    && strcmp(buf, "HTTP/1.0 200")) {
...
static bool socks4_negotiate(struct pool *pool, int sockd, bool socks4a)
{
	...
	if ((int)inp != -1)
		...
		if (!ret) {
			applog(LOG_ERR,
			       "getaddrinfo() in socks4_negotiate() returned %i: %s", ret,
			       gai_strerror(ret));
...
static bool setup_stratum_socket(struct pool *pool)
{
	...
	if (ret) {
		applog(LOG_INFO,
		       "getaddrinfo() in setup_stratum_socket() returned %i: %s", ret,
		       gai_strerror(ret));
		if (!pool->probed) {
			applog(LOG_WARNING,
			       "Failed to resolve (wrong URL?) %s:%s", sockaddr_url, sockaddr_port);
			pool->probed = true;
		} else {
 ...
 resend:
	...
	if (recvd) {
		...
		if (pool->extranonce_subscribe) {
			// If we want extranonce support for a non-standard algo, adapt stratum protocol accordingly
			if (pool->algorithm.type == ALGO_ETHASH) {
				// In the case of Ethereum, if we want extranonce then with it we want EthereumStratum
				sprintf(s,
		...
		if (pool->sessionid)
			sprintf(s,
				"{\"id\": %d, \"method\": \"mining.subscribe\", \"params\": [\""
				PACKAGE "/" CGMINER_VERSION "\", \"%s\"]}", swork_id++,
				pool->sessionid);
		else
			sprintf(s,
				"{\"id\": %d, \"method\": \"mining.subscribe\", \"params\": [\""
				PACKAGE "/" CGMINER_VERSION "\"]}", swork_id++);
	}
...
void _cgsem_post(cgsem_t * cgsem, const char *file, const char *func, const int line)
{
	if (unlikely(sem_post(cgsem)))
		quitfrom(1, file, func, line, "Failed to sem_post errno=%d cgsem=0x%p", errno,
			 cgsem);
}
...
void _cgsem_wait(cgsem_t * cgsem, const char *file, const char *func, const int line)
{
 retry:
	if (unlikely(sem_wait(cgsem))) {
		if (interrupted())
			goto retry;
		quitfrom(1, file, func, line, "Failed to sem_wait errno=%d cgsem=0x%p", errno,
			 cgsem);
	}
}
...
int _cgsem_mswait(cgsem_t * cgsem, int ms, const char *file, const char *func, const int line)
{
 ...
 retry:
	...
	if (ret) {
		...
		if (interrupted())
			goto retry;
		quitfrom(1, file, func, line, "Failed to sem_timedwait errno=%d cgsem=0x%p", errno,
			 cgsem);
	}
	return 0;
}
...
bool cg_completion_timeout(void *fn, void *fnarg, int timeout)
{
	...
	pthread_t pthread;
	bool ret = false;
	cgc = (struct cg_completion *)
	    malloc(sizeof(struct cg_completion));
	if (unlikely(!cgc))
		return ret;
