...
uint32_t EthCalcEpochNumber(uint8_t * SeedHash)
{
	...
	for (int Epoch = 0; Epoch < 2048; ++Epoch) {
		SHA3_256(((struct ethash_h256 *)TestSeedHash), TestSeedHash,
			 32);
...
Node CalcDAGItem(const Node * CacheInputNodes, uint32_t NodeCount,
		 uint32_t NodeIdx)
{
	...
	for (uint32_t i = 0; i < 256; ++i) {
		uint32_t parent_index =
		    fnv(NodeIdx ^ i, DAGNode.words[i % 16]) % NodeCount;
...
void LightEthash(uint8_t * restrict OutHash, uint8_t * restrict MixHash,
		 const uint8_t * restrict HeaderPoWHash, const Node * Cache,
		 const uint64_t EpochNumber, const uint64_t Nonce)
{
	uint32_t MixState[32], TmpBuf[24], NodeCount =
	    EthGetCacheSize(EpochNumber) / sizeof(Node);
	uint64_t DagSize;
	const Node *EthCache = Cache;
	...
	// Main mix of Ethash
	for (uint32_t i = 0, Init0 = MixState[0], MixValue = MixState[0];
	     i < 64; ++i) {
		uint32_t row = fnv(Init0 ^ i, MixValue) % DagSize;
		Node DAGSliceNodes[2];
		DAGSliceNodes[0] = CalcDAGItem(EthCache, NodeCount, row << 1);
		DAGSliceNodes[1] =
		    CalcDAGItem(EthCache, NodeCount, (row << 1) + 1);
		DAG128 *DAGSlice = (DAG128 *) DAGSliceNodes;

		for (uint32_t col = 0; col < 32; ++col) {
			MixState[col] =
			    fnv(MixState[col], DAGSlice->Columns[col]);
			MixValue =
			    col == ((i + 1) & 0x1F) ? MixState[col] : MixValue;
		}
	}
...
//	work->applog(LOG_DEBUG, "Regenhash: First qword of input: 0x%016llX.",work->Nonce);
	...
	applog(LOG_DEBUG, "Last ulong: 0x%016llX.",
	       bswap_64(*((uint64_t *) (work->hash + 0))));
	free(DbgHash);
}
