...
typedef struct _DAG128 { uint32_t Columns[32]; } DAG128;
...
Node CalcDAGItem(const Node *CacheInputNodes, uint32_t NodeCount,
  ...
  for (uint32_t i = 0; i < 256; ++i) {
    ...
    Node const *parent =
        CacheInputNodes + parent_index; //&cache_nodes[parent_index];
...
void LightEthash(uint8_t *restrict OutHash, uint8_t *restrict MixHash,
  ...
  uint32_t MixState[32], TmpBuf[24],
      NodeCount = EthGetCacheSize(EpochNumber) / sizeof(Node);
  uint64_t DagSize;
  ...
  // Main mix of Ethash
  for (uint32_t i = 0, Init0 = MixState[0], MixValue = MixState[0]; i < 64;
       ++i) {
...
void ethash_regenhash(struct work *work) {
  ...
  //	work->applog(LOG_DEBUG, "Regenhash: First qword of input:
  //0x%016llX.",work->Nonce);
  int idx = work->EpochNumber % 2;
  cg_rlock(&EthCacheLock[idx]);
  LightEthash(work->hash, work->mixhash, work->data, ((EthCache[idx] + 64)),
