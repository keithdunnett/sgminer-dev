...
static void CNKeccakF1600(uint64_t *st) {
  ...
  for (round = 0; round < 24; ++round) {
    bc[0] = st[0] ^ st[5] ^ st[10] ^ st[15] ^ st[20] ^
            ROTL64(st[2] ^ st[7] ^ st[12] ^ st[17] ^ st[22], 1UL);
    bc[1] = st[1] ^ st[6] ^ st[11] ^ st[16] ^ st[21] ^
            ROTL64(st[3] ^ st[8] ^ st[13] ^ st[18] ^ st[23], 1UL);
    bc[2] = st[2] ^ st[7] ^ st[12] ^ st[17] ^ st[22] ^
            ROTL64(st[4] ^ st[9] ^ st[14] ^ st[19] ^ st[24], 1UL);
    bc[3] = st[3] ^ st[8] ^ st[13] ^ st[18] ^ st[23] ^
            ROTL64(st[0] ^ st[5] ^ st[10] ^ st[15] ^ st[20], 1UL);
    bc[4] = st[4] ^ st[9] ^ st[14] ^ st[19] ^ st[24] ^
            ROTL64(st[1] ^ st[6] ^ st[11] ^ st[16] ^ st[21], 1UL);
...
void CNAESRnd(long unsigned int *X, long unsigned int *key) {
  ...
  Y[0] = CNAESTbl[BYTE(X[0], 0)] ^ ROTL32(CNAESTbl[BYTE(X[1], 1)], 8) ^
         ROTL32(CNAESTbl[BYTE(X[2], 2)], 16) ^
         ROTL32(CNAESTbl[BYTE(X[3], 3)], 24);
  Y[1] = CNAESTbl[BYTE(X[1], 0)] ^ ROTL32(CNAESTbl[BYTE(X[2], 1)], 8) ^
         ROTL32(CNAESTbl[BYTE(X[3], 2)], 16) ^
         ROTL32(CNAESTbl[BYTE(X[0], 3)], 24);
  Y[2] = CNAESTbl[BYTE(X[2], 0)] ^ ROTL32(CNAESTbl[BYTE(X[3], 1)], 8) ^
         ROTL32(CNAESTbl[BYTE(X[0], 2)], 16) ^
         ROTL32(CNAESTbl[BYTE(X[1], 3)], 24);
  Y[3] = CNAESTbl[BYTE(X[3], 0)] ^ ROTL32(CNAESTbl[BYTE(X[0], 1)], 8) ^
         ROTL32(CNAESTbl[BYTE(X[1], 2)], 16) ^
         ROTL32(CNAESTbl[BYTE(X[2], 3)], 24);
...
#define SubWord(inw)                                                           \
  ((CNAESSbox[BYTE(inw, 3)] << 24) | (CNAESSbox[BYTE(inw, 2)] << 16) |         \
   (CNAESSbox[BYTE(inw, 1)] << 8) | CNAESSbox[BYTE(inw, 0)])
...
void AESExpandKey256(uint64_t *keybuf) {
  for (uint32_t c = 8, i = 1; c < 60; ++c) {
    ...
    // then XOR this with previously generated uint. If it's 4 after a multiple
    // of 8, only the sbox permutation
    // is done, followed by the XOR. If neither are true, only the XOR with the
    // previously generated uint is done.
    keybuf[c] =
        keybuf[c - 8] ^
        ((!(c & 7)) ? ROTL32(t, 24U) ^ ((uint32_t)(CNAESRcon[i++])) : t);
  }
}
...
void cryptonight(unsigned int *Output, unsigned int *Input) {
  ...
  for (int i = 0; i < 4; ++i)
    ((uint64_t *)ExpandedKey1)[i] = CNCtx.State[i];
  for (int i = 0; i < 4; ++i)
    ((uint64_t *)ExpandedKey2)[i] = CNCtx.State[i + 4];
