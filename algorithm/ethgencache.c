...
void EthGenerateCache(void *cache_nodes_in, const uint8_t *seedhash,
                      uint64_t cache_size) {
  ...
  for (uint32_t j = 0; j < 3; j++) // this one can be unrolled entirely,
                                   // ETHASH_CACHE_ROUNDS is constant
  {
    for (uint32_t i = 0; i != num_nodes; i++) {
      uint32_t const idx = cache_nodes[i].words[0] % num_nodes;
      node data;
      data = cache_nodes[(num_nodes - 1 + i) % num_nodes];
      for (uint32_t w = 0; w != 16;
           ++w) // this one can be unrolled entirely as well
      {
