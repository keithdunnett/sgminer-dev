...
int clDevicesNum(void) {
  ...
  status = clGetPlatformInfo(platform, CL_PLATFORM_VENDOR, sizeof(pbuff), pbuff,
                             NULL);
  if (status != CL_SUCCESS) {
    applog(LOG_ERR, "Error %d: Getting Platform Info. (clGetPlatformInfo)",
           status);
    goto out;
  }
...
static cl_int create_opencl_context(cl_context *context,
...
static float get_opencl_version(cl_device_id device) {
  ...
  status =
      clGetDeviceInfo(device, CL_DEVICE_VERSION, 1024, (void *)devoclver, NULL);
  if (status != CL_SUCCESS) {
...
static cl_int
create_opencl_command_queue(cl_command_queue *command_queue,
                            cl_context *context, cl_device_id *device,
                            cl_command_queue_properties cq_properties) {
  cl_int status;
  *command_queue =
      clCreateCommandQueue(*context, *device, cq_properties, &status);
...
_clState *initCl(unsigned int gpu, char *name, size_t nameSize,
  ...
  build_kernel_data *build_data =
      (build_kernel_data *)alloca(sizeof(struct _build_kernel_data));
  ...
  for (int i = 0; i < numDevices; ++i) {
    size_t tmpsize;
    if (clGetDeviceInfo(devices[i], CL_DEVICE_NAME, 0, NULL, &tmpsize) !=
        CL_SUCCESS) {
      applog(LOG_ERR, "Error while getting the length of the name for GPU #%d.",
             i);
      return NULL;
    }

    // Does the size include the NULL terminator? Who knows, just add one, it's
    // faster than looking it up.
    pbuff[i] = (char *)alloca(sizeof(char) * (tmpsize + 1));
    if (clGetDeviceInfo(devices[i], CL_DEVICE_NAME, sizeof(char) * tmpsize,
                        pbuff[i], NULL) != CL_SUCCESS) {
  ...
  if (status != CL_SUCCESS) {
    applog(LOG_ERR, "Error %d: Creating Context. (clCreateContextFromType)",
           status);
    return NULL;
  }
  ...
  status =
      create_opencl_command_queue(&clState->commandQueue, &clState->context,
                                  &devices[gpu], cgpu->algorithm.cq_properties);
  if (status != CL_SUCCESS) {
    applog(LOG_ERR, "Error %d: Creating Command Queue. (clCreateCommandQueue)",
           status);
    return NULL;
  }

  status = clGetDeviceInfo(devices[gpu], CL_DEVICE_PREFERRED_VECTOR_WIDTH_INT,
                           sizeof(cl_uint), (void *)&preferred_vwidth, NULL);
  if (status != CL_SUCCESS) {
    applog(LOG_ERR, "Error %d: Failed to clGetDeviceInfo when trying to get "
  ...
  status =
      clGetDeviceInfo(devices[gpu], CL_DEVICE_MAX_WORK_GROUP_SIZE,
  ...
  applog(LOG_DEBUG, "Max work group size reported %d",
         (int)(clState->max_work_size));

  status = clGetDeviceInfo(devices[gpu], CL_DEVICE_MAX_COMPUTE_UNITS,
                           sizeof(size_t), (void *)&compute_units, NULL);
  if (status != CL_SUCCESS) {
    applog(LOG_ERR, "Error %d: Failed to clGetDeviceInfo when trying to get "
  ...
  applog(LOG_INFO, "Maximum work size for this GPU (%d) is %d.", gpu,
         clState->max_work_size);
  applog(
      LOG_INFO,
      "Your GPU (#%d) has %d compute units, and all AMD cards in the 7 series or newer (GCN cards) \
		have 64 shaders per compute unit - this means it has %d shaders.",
      gpu, compute_units, clState->compute_shaders);

  status = clGetDeviceInfo(devices[gpu], CL_DEVICE_MAX_MEM_ALLOC_SIZE,
                           sizeof(cl_ulong), (void *)&cgpu->max_alloc, NULL);
  if (status != CL_SUCCESS) {
    applog(LOG_ERR, "Error %d: Failed to clGetDeviceInfo when trying to get "
  ...
  applog(LOG_DEBUG, "Max mem alloc size is %lu",
   ...
   * name + g + lg + lookup_gap + tc + thread_concurrency + nf + nfactor + w +
   * work_size + l + sizeof(long) + .bin
   */

  sprintf(filename, "%s.cl", (!empty_string(cgpu->algorithm.kernelfile)
                                  ? cgpu->algorithm.kernelfile
                                  : cgpu->algorithm.name));
  ...
  clState->wsize =
      (cgpu->work_size && cgpu->work_size <= clState->max_work_size)
          ? cgpu->work_size
          : 256;
  ...
  if ((strcmp(cgpu->algorithm.name, "zuikkis") == 0) &&
      (cgpu->lookup_gap != 2)) {
    applog(
        LOG_WARNING,
  ...
  if ((strcmp(cgpu->algorithm.name, "bufius") == 0) &&
      ((cgpu->lookup_gap != 2) && (cgpu->lookup_gap != 4) &&
       (cgpu->lookup_gap != 8))) {
    applog(LOG_WARNING, "Kernel bufius only supports lookup-gap of 2, 4 or 8 "
                        "(currently %d), forcing to 2",
           cgpu->lookup_gap);
    cgpu->lookup_gap = 2;
  }
  ...
  if (cgpu->algorithm.type == ALGO_NEOSCRYPT && !cgpu->opt_tc) {
    ...
    if (cgpu->rawintensity > 0) {
      ...
      glob_thread_count =
          clState->compute_shaders *
          ((cgpu->algorithm.xintensity_shift)
               ? (1UL << (cgpu->algorithm.xintensity_shift + cgpu->xintensity))
               : cgpu->xintensity);
      max_int = cgpu->xintensity;
      type = 1;
    } else {
      glob_thread_count =
          1UL << (cgpu->algorithm.intensity_shift + cgpu->intensity);
      max_int = ((cgpu->dynamic) ? MAX_INTENSITY : cgpu->intensity);
    }

    glob_thread_count =
        ((glob_thread_count < cgpu->work_size) ? cgpu->work_size
                                               : glob_thread_count);

    // if TC * scratchbuf size is too big for memory... reduce to max
    if ((glob_thread_count * NEOSCRYPT_SCRATCHBUF_SIZE) >=
        (uint64_t)cgpu->max_alloc) {

      /* Selected intensity will not run on this GPU. Not enough memory.
       * Adapt the memory setting. */
      // depending on intensity type used, reduce the intensity until it fits
      // into the GPU max_alloc
      switch (type) {
      // raw intensity
      case 2:
        while ((glob_thread_count * NEOSCRYPT_SCRATCHBUF_SIZE) >
               (uint64_t)cgpu->max_alloc) {
          --glob_thread_count;
        }
      ...
      case 1:
        ...
        while (max_int && ((clState->compute_shaders * (1UL << max_int)) >
                           glob_thread_count)) {
          --max_int;
        }
    ...
    applog(LOG_DEBUG, "GPU %d: computing max. global thread count to %u", gpu,
           (unsigned)(cgpu->thread_concurrency));
  ...
  // pluck TC
    ...
    // if TC * scratchbuf size is too big for memory... reduce to max
    if ((glob_thread_count * PLUCK_SCRATCHBUF_SIZE) >=
        (uint64_t)cgpu->max_alloc) {
      ...
      * Adapt the memory setting. */
      ...
      // raw intensity
      case 2:
        while ((glob_thread_count * PLUCK_SCRATCHBUF_SIZE) >
               (uint64_t)cgpu->max_alloc) {
          --glob_thread_count;
        }
  ...
  //    }
  //    else if (cgpu->xintensity > 0) {
  //      glob_thread_count = clState->compute_shaders *
  //      ((cgpu->algorithm.xintensity_shift) ? (1UL <<
  //      (cgpu->algorithm.xintensity_shift + cgpu->xintensity)) :
  //      cgpu->xintensity);
  //      max_int = cgpu->xintensity;
  //      type = 1;
  //    }
  //    else {
  //      glob_thread_count = 1UL << (cgpu->algorithm.intensity_shift +
  //      cgpu->intensity);
  //      max_int = ((cgpu->dynamic) ? MAX_INTENSITY : cgpu->intensity);
  //    }
  //
  //    glob_thread_count = ((glob_thread_count < cgpu->work_size) ?
  //    cgpu->work_size : glob_thread_count);
  //
  //    // if TC * scratchbuf size is too big for memory... reduce to max
  //    if ((glob_thread_count * YESCRYPT_SCRATCHBUF_SIZE) >=
  //    (uint64_t)cgpu->max_alloc) {
  //
  //      /* Selected intensity will not run on this GPU. Not enough memory.
  //      * Adapt the memory setting. */
  //      // depending on intensity type used, reduce the intensity until it
  //      fits into the GPU max_alloc
  //      switch (type) {
  //        //raw intensity
  //      case 2:
  //        while ((glob_thread_count * YESCRYPT_SCRATCHBUF_SIZE) >
  //        (uint64_t)cgpu->max_alloc) {
  //          --glob_thread_count;
  //        }
  //
  ...
  //    cgpu->thread_concurrency = glob_thread_count;
  //
  //    applog(LOG_DEBUG, "GPU %d: computing max. global thread count to %u",
  //    gpu, (unsigned)(cgpu->thread_concurrency));
  //  }
  ...
  //         clState->compute_shaders *
  //          ((cgpu->algorithm.xintensity_shift)
  //               ? (1UL << (cgpu->algorithm.xintensity_shift +
  //               cgpu->xintensity))
  //               : cgpu->xintensity);
  //      max_int = cgpu->xintensity;
  //      type = 1;
  ...
  //
  //    // if TC * scratchbuf size is too big for memory... reduce to max
  //    if ((glob_thread_count * LYRA_SCRATCHBUF_SIZE) >=
  //        (uint64_t)cgpu->max_alloc) {
  //
  //      /* Selected intensity will not run on this GPU. Not enough memory.
  //      * Adapt the memory setting. */
  ...
  //      // raw intensity
  //      case 2:
  //        while ((glob_thread_count * LYRA_SCRATCHBUF_SIZE) >
  //               (uint64_t)cgpu->max_alloc) {
  //          --glob_thread_count;
  //        }
  //
  ...
  // }
    ...
    applog(LOG_DEBUG, "GPU %d: selecting thread concurrency of %d", gpu,
           (int)(cgpu->thread_concurrency));
  } else {
  ...
  build_data->binary_filename[strlen(filename) - 3] =
      0x00; // And one NULL terminator, cutting off the .cl suffix.
  ...
  // Load kernels
  applog(LOG_NOTICE, "Initialising kernel %s with nfactor %d, n %d", filename,
         algorithm->nfactor, algorithm->n);

  /* get a kernel object handle for a kernel with the given name */
  clState->kernel = clCreateKernel(clState->program, "search", &status);
  if (status != CL_SUCCESS) {
    applog(LOG_ERR, "Error %d: Creating Kernel from program. (clCreateKernel)",
           status);
    return NULL;
  }
  ...
  if (algorithm->type == ALGO_ETHASH) {
    clState->GenerateDAG =
        clCreateKernel(clState->program, "GenerateDAG", &status);
  ...
  if (clState->n_extra_kernels > 0) {
    ...
    clState->extra_kernels =
        (cl_kernel *)malloc(sizeof(cl_kernel) * clState->n_extra_kernels);

    for (i = 0; i < clState->n_extra_kernels; i++) {
      snprintf(kernel_name, 9, "%s%d", "search", i + 1);
      clState->extra_kernels[i] =
          clCreateKernel(clState->program, kernel_name, &status);
      if (status != CL_SUCCESS) {
        applog(
            LOG_ERR,
  ...
  if (algorithm->type == ALGO_CRE)
    readbufsize = 168;
  else if (algorithm->type == ALGO_DECRED)
    readbufsize = 192;
  else if (algorithm->type == ALGO_LBRY)
    readbufsize = 112;
  ...
  if (algorithm->rw_buffer_size < 0) {
    ...
    if (algorithm->type == ALGO_NEOSCRYPT) {
      ...
      applog(LOG_DEBUG, "Neoscrypt buffer sizes: %lu RW, %lu R",
             (unsigned long)bufsize, (unsigned long)readbufsize);
      // scrypt/n-scrypt
      ...
      * 80 bytes only. */
      ...
      applog(LOG_DEBUG, "pluck buffer sizes: %lu RW, %lu R",
             (unsigned long)bufsize, (unsigned long)readbufsize);
      // scrypt/n-scrypt
    }
    //    else if (algorithm->type == ALGO_YESCRYPT || algorithm->type ==
    //    ALGO_YESCRYPT_MULTI) {
    //      /* The scratch/pad-buffer needs 32kBytes memory per thread. */
    ...
    //      applog(LOG_DEBUG, "yescrypt buffer sizes: %lu RW, %lu R", (unsigned
    //      long)bufsize, (unsigned long)readbufsize);
    //     // scrypt/n-scrypt
    //    }
    ...
    //      readbufsize = 80;
    //
    //      applog(LOG_DEBUG, "lyra2REv2 buffer sizes: %lu RW, %lu RW",
    //             (unsigned long)bufsize, (unsigned long)buf1size);
    //      // scrypt/n-scrypt
    //  } else {
    //   size_t ipt = (algorithm->n / cgpu->lookup_gap +
    //                 (algorithm->n % cgpu->lookup_gap > 0));
    //   bufsize = 128 * ipt * cgpu->thread_concurrency;
    //   applog(LOG_DEBUG, "Scrypt buffer sizes: %lu RW, %lu R",
    //          (unsigned long)bufsize, (unsigned long)readbufsize);
  } else {
    bufsize = (size_t)algorithm->rw_buffer_size;
    applog(LOG_DEBUG, "Buffer sizes: %lu RW, %lu R", (unsigned long)bufsize,
  ...
  if (bufsize > 0) {
    applog(LOG_DEBUG, "Creating read/write buffer sized %lu",
    ...
    //    if (algorithm->type == ALGO_YESCRYPT || algorithm->type ==
    //    ALGO_YESCRYPT_MULTI) {
    //      // need additionnal buffers
    //      clState->buffer1 = clCreateBuffer(clState->context,
    //      CL_MEM_READ_WRITE, buf1size, NULL, &status);
    //      if (status != CL_SUCCESS && !clState->buffer1) {
    //        applog(LOG_DEBUG, "Error %d: clCreateBuffer (buffer1), decrease TC
    //        or increase LG", status);
    //        return NULL;
    //      }
    //
    //      clState->buffer2 = clCreateBuffer(clState->context,
    //      CL_MEM_READ_WRITE, buf2size, NULL, &status);
    //      if (status != CL_SUCCESS && !clState->buffer2) {
    //        applog(LOG_DEBUG, "Error %d: clCreateBuffer (buffer2), decrease TC
    //        or increase LG", status);
    //        return NULL;
    //      }
    //
    //      clState->buffer3 = clCreateBuffer(clState->context,
    //      CL_MEM_READ_WRITE, buf3size, NULL, &status);
    //      if (status != CL_SUCCESS && !clState->buffer3) {
    //        applog(LOG_DEBUG, "Error %d: clCreateBuffer (buffer3), decrease TC
    //        or increase LG", status);
    //        return NULL;
    //      }
    //    }
    //   else if (algorithm->type == ALGO_LYRA2REV2) {
    //    // need additionnal buffers
    //    clState->buffer1 = clCreateBuffer(clState->context, CL_MEM_READ_WRITE,
    //                                      buf1size, NULL, &status);
    //    if (status != CL_SUCCESS && !clState->buffer1) {
    //      applog(LOG_DEBUG,
    //             "Error %d: clCreateBuffer (buffer1), decrease TC or increase
    //             LG",
    //             status);
    //      return NULL;
    //    }
    ...
    //  else {
    clState->buffer1 =
        clCreateBuffer(clState->context, CL_MEM_READ_WRITE, bufsize, NULL,
                       &status); // we don't need that much just tired...
    if (status != CL_SUCCESS && !clState->buffer1) {
      applog(LOG_DEBUG,
             "Error %d: clCreateBuffer (buffer1), decrease TC or increase LG",
             status);
      return NULL;
    }
  }
  ...
  if (algorithm->type == ALGO_CRYPTONIGHT) {
    ...
    set_threads_hashes(1, clState->compute_shaders, &GlobalThreads, 1,
                       &cgpu->intensity, &cgpu->xintensity, &cgpu->rawintensity,
                       &cgpu->algorithm);

    for (int i = 0; i < 4; ++i) {
      clState->BranchBuffer[i] =
          clCreateBuffer(clState->context, CL_MEM_READ_WRITE,
                         sizeof(cl_uint) * (GlobalThreads + 2), NULL, &status);
    ...
    clState->States = clCreateBuffer(clState->context, CL_MEM_READ_WRITE,
                                     200 * GlobalThreads, NULL, &status);
    ...
    clState->Scratchpads =
        clCreateBuffer(clState->context, CL_MEM_READ_WRITE,
                       (1 << 21) * GlobalThreads, NULL, &status);

    if (status != CL_SUCCESS) {
      applog(LOG_ERR,
             "Error %d when creating Cryptonight scratchpads buffer.\n",
             status);
      return NULL;
    }
  }
