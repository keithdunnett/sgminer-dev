...
static void get_timestamp(char *f, size_t fsiz, struct timeval *tv)
{
	...
	const time_t tmp_time = tv->tv_sec;
	tm = localtime(&tmp_time);
	snprintf(f, fsiz, "[%02d:%02d:%02d]", tm->tm_hour, tm->tm_min,
		 tm->tm_sec);
}
...
static char *set_pool_algorithm(const char *arg)
{
	...
	applog(LOG_DEBUG, "Setting pool %i algorithm to %s", pool->pool_no,
	       arg);
...
static char *set_pool_state(char *arg)
{
	...
	applog(LOG_INFO, "Setting pool %s state to %s", get_pool_name(pool),
	       arg);
...
static char *set_pool_description(char *arg)
{
	...
	applog(LOG_DEBUG, "Setting pool %i description to %s", pool->pool_no,
	       arg);
...
static char *set_log_file(char *arg)
{
	...
	if ((!*r) && i >= 0 && i <= INT_MAX)
		...
		fd = open(arg, O_WRONLY | O_APPEND | O_CREAT,
			  S_IRUSR | S_IWUSR);
		if (unlikely(fd == -1))
...
static bool __build_gbt_txns(struct pool *pool, json_t * res_val)
{
	...
	pool->txn_hashes =
	    (unsigned char *)calloc(32 * (pool->gbt_txns + 1), 1);
...
static unsigned char *__gbt_merkleroot(struct pool *pool)
{
	...
	while (txns > 1) {
		if (txns % 2) {
			memcpy(&merkle_hash[txns * 32],
			       &merkle_hash[(txns - 1) * 32], 32);
			txns++;
		}
		for (i = 0; i < txns; i += 2) {
...
static double get_work_blockdiff(const struct work *work)
{
	...
	if (work->pool->algorithm.type == ALGO_NEOSCRYPT) {
		...
		if (!diff64)
			diff64 = 1;
		double d =
		    (double)work->pool->algorithm.diff_numerator /
		    (double)diff64;
...
static bool gbt_decode(struct pool *pool, json_t * res_val)
{
	...
	coinbasetxn =
	    json_string_value(json_object_get
			      (json_object_get(res_val, "coinbasetxn"),
			       "data"));
	...
	*extra_len += pool->n2size;
	hex2bin(pool->coinbase + 42 + *extra_len,
		pool->coinbasetxn + 84 + (orig_len * 2),
		cbt_len - orig_len - 42);
...
static bool getwork_decode(json_t * res_val, struct work *work)
{
	...
	if (work->pool->algorithm.type == ALGO_DECRED) {
		...
		((uint32_t *) work->data)[37] =
		    (rand() * 4) << 8 | work->thr_id;
	}
	return true;
}
...
static void suffix_string(uint64_t val, char *buf, size_t bufsiz, int sigdigits)
{
	...
	if (!sigdigits) {
		if (decimal)
			...
			snprintf(buf, bufsiz, "%d%s", (unsigned int)dval,
				 suffix);
	} else {
		...
		snprintf(buf, bufsiz, "%*.*f%s", sigdigits + 1, ndigits, dval,
			 suffix);
	}
}
...
static void get_statline(char *buf, size_t bufsiz, struct cgpu_info *cgpu)
{
	...
	tailsprintf(buf, bufsiz,
		    ...
		    cgpu->diff_accepted, cgpu->diff_rejected, cgpu->hw_errors,
		    wu);
...
{
	return (pool_strategy == POOL_LOADBALANCE
...
static void curses_print_status(void)
{
	...
	if (shared_strategy() && total_pools > 1) {
		...
		cg_mvwprintw(statuswin, ++line, 0,
			     ...
			     get_pool_name(pool),
			     pool->
			     has_stratum ? "stratum" : (pool->has_gbt ? "GBT" :
							"longpoll"), pool->diff,
			     get_pool_user(pool));
	}
...
static void curses_print_devstatus(struct cgpu_info *cgpu, int count)
{
	...
	cg_wprintw(statuswin, "/%6sh/s | R:%*.1f%% HW:%*d WU:%*.3f/m",
		   displayed_hashes,
		   drwidth, reject_pct, hwwidth, cgpu->hw_errors, wuwidth + 2,
		   wu);
	logline[0] = '\0';
...
static void switch_logsize(bool __maybe_unused newdevs)
{
	if (curses_active_locked()) {
		...
		if (opt_compact) {
			...
			logstart =
			    devcursor +
			    (opt_removedisabled ? most_devices : total_devices)
			    + 1;
		}
	     ...
	     char *worktime)
{
	...
	if (json_is_true(res) || (work->gbt && json_is_null(res))
		...
		applog(LOG_DEBUG,
		       "[THR%d] PROOF OF WORK RESULT: false (booooo)",
		       work->thr_id);
		if (!QUIET) {
			...
			if (res) {
				...
				if (reason_val && json_is_string(reason_val)) {
					reason_str = (char *)
					    json_string_value(reason_val);
					snprintf(reason, 31, " (%s)",
						 reason_str);
				}
			}
	...
	if (work->pool->algorithm.type == ALGO_ETHASH) {
		...
		if (work->gbt) {
			...
			if (work->gbt_txns < 0xfd) {
				...
				varint =
				    bin2hex((const unsigned char *)&val, 1);
			} else if (work->gbt_txns <= 0xffff) {
				uint16_t val = htole16(work->gbt_txns);

				gbt_block =
				    (char *)realloc_strcat(gbt_block, "fd");
				varint =
				    bin2hex((const unsigned char *)&val, 2);
			} else {
				uint32_t val = htole32(work->gbt_txns);

				gbt_block =
				    (char *)realloc_strcat(gbt_block, "fe");
				varint =
				    bin2hex((const unsigned char *)&val, 4);
			}
			gbt_block = (char *)realloc_strcat(gbt_block, varint);
			free(varint);
			gbt_block =
			    (char *)realloc_strcat(gbt_block, work->coinbase);
		...
		if (!QUIET) {
			...
			if (opt_worktime) {
				...
				double work_to_submit = tdiff(&tv_submit,
							      ...
							      (work->tv_work_found));
				double submit_time =
				    tdiff(&tv_submit_reply, &tv_submit);
				int diffplaces = 3;
...
static void calc_diff(struct work *work, double known)
{
	struct sgminer_pool_stats *pool_stats =
	    &(work->pool->sgminer_pool_stats);
	double difficulty;
...
static void kill_mining(void)
{
	...
	for (i = 0; i < mining_threads; i++) {
		thr = mining_thr[i];
		forcelog(LOG_DEBUG, "Waiting for thread %d to finish...",
			 thr->id);
...
static void *submit_work_thread(void *userdata)
{
	...
	while (!submit_upstream_work
	       (work, ce->curl, ce->curl_err_str, resubmit)) {
		if (opt_lowmem) {
			applog(LOG_NOTICE,
...
static void pool_died(struct pool *pool)
{
	if (!pool_tset(pool, &pool->idle)) {
		cgtime(&pool->tv_idle);
		if (pool == current_pool()) {
			applog(LOG_WARNING, "%s not responding!",
			       get_pool_name(pool));
			switch_pools(NULL);
		} else {
			applog(LOG_INFO, "%s failed to return work",
			       get_pool_name(pool));
		}
	}
}
...
static double share_diff(const struct work *work)
{
	...
	if (unlikely(ret > best_diff)) {
		new_best = true;
		best_diff = ret;
		suffix_string_double(best_diff, best_share, sizeof(best_share),
				     0);
	}
...
void __switch_pools(struct pool *selected, bool saveprio)
{
	...
	if (pool != last_pool && pool_strategy != POOL_LOADBALANCE
		...
		if (gpu_initialized || startup) {
			applog(LOG_WARNING, "Switching to %s",
			       get_pool_name(pool));
...
static void set_blockdiff(const struct work *work)
{
	...
	if (unlikely(current_diff != ddiff)) {
		...
		current_diff = ddiff;
		if (opt_morenotices)
			applog(LOG_NOTICE, "Network diff set to %s",
			       block_diff);
	}
}
...
static bool test_work_current(struct work *work)
{
	...
	if (!block_exists(hexstr)) {
		struct block *s =
		    (struct block *)calloc(sizeof(struct block), 1);
		int deleted_block = 0;
		...
		if (memcmp(pool->prev_block, bedata, 32)) {
			...
			if (memcmp(bedata, current_block, 32)) {
				...
				applog(LOG_DEBUG, "Stale data from %s",
				       get_pool_name(pool));
				ret = false;
			} else {
				/* Work is from new block and pool is up now
				 * current. */
				applog(LOG_INFO, "%s now up to date",
				       get_pool_name(pool));
				memcpy(pool->prev_block, bedata, 32);
			}
		}
		...
		if (memcmp(bedata, current_block, 32))
			applog(LOG_DEBUG, "%s still on old block",
			       get_pool_name(pool));
#endif
		if (work->longpoll) {
			work->work_block = ++work_block;
 ...
 retry:
	...
	if (!strncasecmp(&input, "a", 1)) {
		...
		if (selected == POOL_ROTATE) {
			opt_rotate_period =
			    curses_int("Select interval in minutes");
...
static bool parse_stratum_response(struct pool *pool, char *s)
{
	...
	if (!val) {
		applog(LOG_INFO, "JSON decode failed(%d): %s", err.line,
		       err.text);
		goto out;
	}
	...
	if (!sshare) {
		...
		if (pool->algorithm.type == ALGO_CRYPTONIGHT) {
			...
			if ((res_id = json_object_get(res_val, "id"))) {
				cg_wlock(&pool->data_lock);
				strcpy(pool->XMRAuthID,
				       json_string_value(res_id));
				cg_wunlock(&pool->data_lock);
				    ...
				    (json_object_get(res_val, "status")), "OK"))
			{
				...
				applog(LOG_INFO,
				       "JSON-RPC response decode failed: %s",
				       ss);
...
//              uint64_t *nonce2_64;
		...
		sshare =
		    (struct stratum_share *)calloc(sizeof(struct stratum_share),
						   1);
...
// Send the share to the pool connection. Vary stratum protocol by algorithm.
		if (pool->algorithm.type == ALGO_ETHASH) {
			...
			snprintf(s, sizeof(s),
				 ...
				 work->ntime, noncehex, (opt_vote << 1) | 1,
				 sshare->id);
		} else {
			snprintf(s, sizeof(s),
...
// Log the share submission
		...
		if (pool->algorithm.type == ALGO_CRYPTONIGHT) {
			applog(LOG_INFO, "Submitting share to %s",
			       get_pool_name(pool));
		} else {
		...
		while (time(NULL) < sshare->sshare_time + 120) {
			...
			if (likely(stratum_send(pool, s, strlen(s)))) {
				...
				ssdiff =
				    sshare->sshare_sent - sshare->sshare_time;
		...
		if (unlikely(!submitted)) {
			applog(LOG_DEBUG,
			       "Failed to submit stratum share, discarding");
			free_work(work);
			free(sshare);
			pool->stale_shares++;
			total_stale++;
...
static bool pool_active(struct pool *pool, bool pinging)
{
 ...
 retry_stratum:if (pool->has_stratum) {
		...
		if (val) {
			...
			for (i = 0; i < mutsize; i++) {
				...
				if (json_is_string(arrval)) {
					const char *mut =
					    json_string_value(arrval);
					if (!strncasecmp
					    (mut, "coinbase/append", 15))
						append = true;
					else if (!strncasecmp
						 (mut, "submit/coinbase", 15))
						submit = true;
				}
			}
			json_decref(val);
	...
	if (pool->algorithm.type == ALGO_ETHASH) {
		...
		if (pool->stratum_url && !opt_fix_protocol
		    && stratum_works(pool)) {
		...
		if (pool->hdr_path) {
			...
			if (strstr(hdr_path, "://")) {
				...
				if (pool->rpc_url[strlen(pool->rpc_url) - 1] !=
					...
					need_slash = true;
				siz =
				    strlen(pool->rpc_url) + strlen(copy_start) +
				    2;
				pool->lp_url = (char *)malloc(siz);
				if (!pool->lp_url) {
					applog(LOG_ERR,
					       "Malloc failure in pool_active");
					goto out;
				}

				snprintf(pool->lp_url, siz, "%s%s%s",
					 pool->rpc_url, need_slash ? "/" : "",
					 copy_start);
			}
		} else
			pool->lp_url = NULL;
...
{
	if (pool_strategy == POOL_FAILOVER && pool->prio < cp_prio())
		applog(LOG_WARNING, "%s alive, testing stability",
		       get_pool_name(pool));
	else
...
hash_pop(bool blocking)
{
	...
	if (!HASH_COUNT(staged_work)) {
		...
		do {
			...
			pthread_cond_signal(&gws_cond);
			rc = pthread_cond_timedwait(&getq->cond, stgd_lock,
						    &then);
	   ...
	   double diff, double diff_multiplier2, const int thr_id)
{
	...
	if (unlikely(diff == 0.0)) {
		...
		applog(LOG_ERR, "[THR%d] Diff zero passed to set_target",
		       thr_id);
		diff = 1.0;
	}
	...
	*data64 = htole64(h64);
	if (opt_debug) {
		...
		applog(LOG_DEBUG, "[THR%d] Generated target %s", thr_id,
		       htarget);
		free(htarget);
	}
...
void set_target_neoscrypt(unsigned char *target, double diff, const int thr_id)
{
	...
	if (opt_debug) {
		...
		applog(LOG_DEBUG,
		       "[THR%d] Generated neoscrypt target 0x%s", thr_id,
		       htarget);
		free(htarget);
	}
}
...
static void gen_stratum_work(struct pool *pool, struct work *work)
{
	...
	if (pool->algorithm.type != ALGO_DECRED
		...
		memcpy(pool->coinbase + pool->nonce2_offset, &nonce2le,
		       pool->n2size);
	}
	...
	if (pool->algorithm.type == ALGO_NEOSCRYPT) {
		...
		memcpy(work->data + 144 + nonce2_offset, &nonce2le,
		       pool->n2size);
	} else if (pool->algorithm.type == ALGO_SIA) {
		...
		memcpy(work->data + 32 + 4 + nonce2_offset, &nonce2le,
		       pool->n2size);
	...
	if (opt_debug) {
		...
		if (pool->algorithm.type != ALGO_DECRED) {
			merkle_hash =
			    bin2hex((const unsigned char *)merkle_root, 32);
			applog(LOG_DEBUG, "[THR%d] Generated stratum merkle %s",
			       work->thr_id, merkle_hash);
			free(merkle_hash);
		}
		applog(LOG_DEBUG,
		       "[THR%d] Generated stratum header %s", work->thr_id,
		       header);
		applog(LOG_DEBUG,
...
/*
	...
	if (!empty_string
	    ((opt =
	      get_pool_setting(pool->gpu_threads,
...
compare_pool_settings(struct pool *oldpool, struct pool *newpool)
{
	...
	opt1 =
	    ((oldpool) ?
	     get_pool_setting(oldpool->gpu_threads,
			      default_profile.gpu_threads) : "");
	opt2 =
	    get_pool_setting(newpool->gpu_threads, default_profile.gpu_threads);
	...
	opt1 =
	    ((oldpool) ?
	     get_pool_setting(oldpool->lookup_gap,
			      default_profile.lookup_gap) : "");
	opt2 =
	    get_pool_setting(newpool->lookup_gap, default_profile.lookup_gap);
	...
	if (!empty_string(newpool->rawintensity)) {
		...
		opt1 =
		    ((oldpool
		      && !empty_string(oldpool->
				       xintensity)) ? oldpool->xintensity : "");
		opt2 =
		    get_pool_setting(newpool->xintensity,
		...
		opt1 =
		    ((oldpool
		      && !empty_string(oldpool->
				       intensity)) ? oldpool->intensity : "");
		opt2 =
		    get_pool_setting(newpool->intensity,
	...
	opt1 =
	    ((oldpool) ?
	     get_pool_setting(oldpool->worksize,
	...
	opt1 =
	    ((oldpool) ?
	     get_pool_setting(oldpool->gpu_engine,
			      default_profile.gpu_engine) : "");
	opt2 =
	    get_pool_setting(newpool->gpu_engine, default_profile.gpu_engine);
	if (strcasecmp(opt1, opt2) != 0 && !empty_string(opt2)) {
	...
	opt2 =
	    get_pool_setting(newpool->gpu_memclock,
			     default_profile.gpu_memclock);
	if (strcasecmp(opt1, opt2) != 0 && !empty_string(opt2)) {
	...
	opt2 =
	    get_pool_setting(newpool->gpu_powertune,
			     default_profile.gpu_powertune);
	if (strcasecmp(opt1, opt2) != 0 && !empty_string(opt2)) {
	...
	opt1 =
	    ((oldpool) ?
	     get_pool_setting(oldpool->gpu_vddc,
...
static void apply_switcher_options(unsigned long options, struct pool *pool)
{
	...
	if (opt_isset(options, SWITCHER_APPLY_LG)) {
		if (!empty_string
		    ((opt =
		      get_pool_setting(pool->lookup_gap,
				       default_profile.lookup_gap))))
			set_lookup_gap((char *)opt);
	}
	...
	if (opt_isset(options, SWITCHER_APPLY_RAWINT)) {
		...
		opt =
		    get_pool_setting(pool->rawintensity,
				     default_profile.rawintensity);
	...
	//xintensity
		...
		opt =
		    get_pool_setting(pool->xintensity,
				     default_profile.xintensity);
	...
	//intensity
		...
		opt =
		    get_pool_setting(pool->intensity,
				     default_profile.intensity);
	...
	if (opt_isset(options, SWITCHER_APPLY_WORKSIZE)) {
		if (!empty_string
		    ((opt =
		      get_pool_setting(pool->worksize,
				       default_profile.worksize))))
			set_worksize(opt);
	}
#ifdef HAVE_ADL
	...
	if (opt_isset(options, SWITCHER_APPLY_GPU_ENGINE)) {
		if (!empty_string
		    ((opt =
		      get_pool_setting(pool->gpu_engine,
				       default_profile.gpu_engine))))
			set_gpu_engine((char *)opt);
	}
	//GPU memory clock
	...
	if (opt_isset(options, SWITCHER_APPLY_GPU_VDDC)) {
		if (!empty_string
		    ((opt =
		      get_pool_setting(pool->gpu_vddc,
				       default_profile.gpu_vddc))))
			set_gpu_vddc((char *)opt);
	}
	//apply gpu settings
 ...
 get_work_prepare_thread(struct thr_info *mythr, struct work *work)
{
	...
	if (algo_switch_n == 0) {
		...
		if (opt_switchmode == SWITCH_ALGO) {
			...
			if (!cmp_algorithm
				...
				pool_switch_options =
				    compare_pool_settings(pools[mythr->pool_no],
							  work->pool);
		}
	...
	if (algo_switch_n >= active_threads) {
		...
		if (opt_isset(pool_switch_options, SWITCHER_HARD_RESET)) {
			...
			if (opt_isset
			    (pool_switch_options, SWITCHER_APPLY_DEVICE))
				enable_devices();
			...
			if (unlikely
				...
				quit(1,
				     "restart_mining_threads create thread failed");
			applog(LOG_DEBUG,
	...
	while (algo_switch_n > 0)
		pthread_cond_wait(&algo_switch_wait_cond,
				  &algo_switch_wait_lock);
...
struct work *get_work(struct thr_info *thr, const int thr_id)
{
	...
	while (!work) {
		work = hash_pop(true);
		if (stale_work(work, false)) {
			applog(LOG_DEBUG, "[THR%d] Work is stale, discarding",
			       thr_id);
			discard_work(work);
			work = NULL;
			wake_gws();
	...
	work->mined = true;
	work->device_diff =
	    MIN(thr->cgpu->drv->max_diff, work->work_difficulty);
	return work;
}
...
static void update_work_stats(struct thr_info *thr, struct work *work)
{
	...
	if (unlikely(test_diff > 0 && work->share_diff >= test_diff)) {
		...
		found_blocks++;
		work->mandatory = true;
		applog(LOG_NOTICE, "Found block for %s!",
...
static void hash_sole_work(struct thr_info *mythr)
{
	...
	while (likely(!cgpu->shutdown)) {
		...
		work->device_diff =
		    MIN(drv->working_diff, work->work_difficulty);
		...
		if (work->pool->algorithm.type == ALGO_NEOSCRYPT)
			...
			*(uint64_t *) (work->device_target + 24) =
			...
			set_target(work->device_target,
				   work->device_diff,
				   work->pool->algorithm.diff_multiplier2,
				   work->thr_id);
		do {
			cgtime(&tv_start);
			...
			if (time_more
			    (&getwork_start, &dev_stats->getwork_wait_max))
				copy_time(&dev_stats->getwork_wait_max,
					  &getwork_start);
			if (time_less
			    (&getwork_start, &dev_stats->getwork_wait_min))
				copy_time(&dev_stats->getwork_wait_min,
					  &getwork_start);
			dev_stats->getwork_calls++;
			pool_stats = &(work->pool->sgminer_stats);
			addtime(&getwork_start, &pool_stats->getwork_wait);
			if (time_more
			    (&getwork_start, &pool_stats->getwork_wait_max))
				copy_time(&pool_stats->getwork_wait_max,
					  &getwork_start);
			if (time_less
			    (&getwork_start, &pool_stats->getwork_wait_min))
				copy_time(&pool_stats->getwork_wait_min,
					  &getwork_start);
			pool_stats->getwork_calls++;
			...
			if (unlikely
			    (mythr->pause || cgpu->deven != DEV_ENABLED))
...
static void *longpoll_thread(void *userdata)
{
 ...
 retry_pool:pool = select_longpoll_pool(cp);
	if (!pool) {
		applog(LOG_WARNING, "No suitable long-poll found for %s",
		       cp->rpc_url);
		while (!pool) {
			cgsleep_ms(60000);
	...
	if (pool->has_gbt) {
		...
		lp_url = pool->lp_url;
		if (cp == pool)
			applog(LOG_WARNING, "Long-polling activated for %s",
			       lp_url);
		else
			applog(LOG_WARNING,
	...
	while (42) {
		...
		if (likely(val)) {
			soval =
			    json_object_get(json_object_get(val, "result"),
					    "submitold");
			if (soval)
...
static void *watchpool_thread(void __maybe_unused * userdata)
{
	...
	while (42) {
		...
		for (i = 0; i < total_pools; ++i) {
			...
			if (pool->idle
			    && now.tv_sec - pool->tv_idle.tv_sec > 30) {
				cgtime(&pool->tv_idle);
...
static void noop_get_statline(char __maybe_unused
			      * buf,
			      size_t
			      __maybe_unused
			      bufsiz, struct cgpu_info __maybe_unused * cgpu);
...
void print_summary(void)
{
	...
	if (total_pools > 1) {
		for (i = 0; i < total_pools; i++) {
			...
			applog(LOG_WARNING, " Share submissions: %d",
			       pool->accepted + pool->rejected);
			applog(LOG_WARNING, " Accepted shares: %d",
			       pool->accepted);
			applog(LOG_WARNING, " Rejected shares: %d",
			       pool->rejected);
			...
			if (pool->accepted || pool->rejected)
				applog(LOG_WARNING,
				       ...
				       (pool->rejected *
					100) / (double)(pool->accepted +
							pool->rejected));
			applog(LOG_WARNING, " Items worked on: %d",
			       pool->works);
			applog(LOG_WARNING,
...
add_pool_details(struct pool * pool, bool live,
		 char *url, char *user,
		 char *pass, char *name, char *desc, char *profile, char *algo)
{
	size_t siz;
	...
	if (empty_string(pool->profile)) {
		if (!empty_string(algo))
			...
			set_algorithm(&pool->algorithm,
				      default_profile.algorithm.name);
	}
...
static bool input_pool(bool live)
{
	...
	ret =
	    add_pool_details(pool, live, url, user, pass, name, desc, profile,
			     algo);
 out:
	immedok(logwin, false);
	if (!ret) {
...
static void fork_monitor()
{
	...
	if (0 == forkpid) {
		...
		execl("/bin/bash", "/bin/bash", "-c", opt_stderr_cmd,
		      (char *)NULL);
		perror
	...
	if (r < 0) {
		perror
		    ("close - failed to close read end of pipe for --monitor");
		exit(1);
	}
}
#endif				// defined(unix)
...
blank_get_statline_before(char *buf,
			  size_t bufsiz, struct cgpu_info __maybe_unused * cgpu)
{
	tailsprintf(buf, bufsiz, "               | ");
} static void
 ...
 restart_mining_threads(unsigned int new_n_threads)
{
	...
	if (mining_thr) {
		applog(LOG_DEBUG,
		       "Free old mining and device thread memory...");
		rd_lock(&devices_lock);
		for (i = 0; i < total_devices; i++) {
			if (devices[i]->thr)
				free(devices[i]->thr);
	...
	for (i = 0; i < total_devices; ++i) {
		...
		for (j = 0; j < cgpu->threads; ++j) {
			thr = cgpu->thr[j];
			applog(LOG_DEBUG,
			       "Starting device %d mining thread %d...", i, j);
			if (unlikely
			    (thr_info_create(thr, NULL, miner_thread, thr)))
...
int main(int argc, char *argv[])
{
	...
	if (unlikely(pthread_mutex_init(&lockstat_lock, NULL)))
		quithere(1,
			 "Failed to pthread_mutex_init lockstat_lock errno=%d",
			 errno);
#endif
	...
	rd_lock(&devices_lock);
	for (i = 0; i < total_devices; ++i)
		devices[i]->sgminer_stats.getwork_wait_min.tv_sec =
		    MIN_SEC_UNSET;
	rd_unlock(&devices_lock);
	if (!opt_compact) {
	...
	for (i = 0; i < total_pools; i++) {
		...
		pool->sgminer_pool_stats.getwork_wait_min.tv_sec =
		    MIN_SEC_UNSET;
		if (!pool->rpc_userpass) {
			if (!pool->rpc_user || !pool->rpc_pass)
				quit(1, "No login credentials supplied for %s",
				     get_pool_name(pool));
			siz =
			    strlen(pool->rpc_user) + strlen(pool->rpc_pass) + 2;
			pool->rpc_userpass = (char *)malloc(siz);
			if (!pool->rpc_userpass)
				quit(1, "Failed to malloc userpass");
	...
	if (slept >= 60)
		applog(LOG_WARNING,
		       "GPUs did not become initialized in 60 seconds...");
	rd_lock(&devices_lock);
	total_mhashes_done = 0;
	for (i = 0; i < total_devices; i++) {
		struct cgpu_info *cgpu = devices[i];
	...
	while (42) {
		...
		if (lagging && !pool_tset(cp, &cp->lagging)) {
			applog(LOG_WARNING, "%s not providing work fast enough",
			       cp->name);
			cp->getfail_occasions++;
			total_go++;
			if (!pool_localgen(cp))
				applog(LOG_INFO, "Increasing queue to %d",
				       ++opt_queue);
		}
