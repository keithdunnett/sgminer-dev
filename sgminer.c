...
static bool
time_before (struct tm *tm1, struct tm *tm2)
{
...
void
get_datestamp (char *f, size_t fsiz, struct timeval *tv)
{
  struct tm *tm;
...
static void
get_timestamp (char *f, size_t fsiz, struct timeval *tv)
{
  struct tm *tm;
...
static void
applog_and_exit (const char *fmt, ...)
{
  va_list ap;
...
sharelog (const char *disposition, const struct work *work)
{
  ...
  rv = snprintf (s, sizeof (s), "%lu,%s,%s,%s,%s%u,%u,%s,%s\n", t,
		 disposition, target, pool->rpc_url, cgpu->drv->name,
		 cgpu->device_id, thr_id, hash, data);
  free (target);
  free (hash);
  free (data);
  if (rv >= (int) (sizeof (s)))
...
add_pool (void)
{
  ...
  pools =
    (struct pool **) realloc (pools,
			      sizeof (struct pool *) * (total_pools + 2));
...
static bool
pool_tset (struct pool *pool, bool * var)
{
  bool ret;
...
bool
pool_tclear (struct pool * pool, bool * var)
{
  bool ret;
...
char *
set_int_range (const char *arg, int *i, int min, int max)
{
...
char *
set_int_0_to_9999 (const char *arg, int *i)
{
  return set_int_range (arg, i, 0, 9999);
}

char *
set_int_1_to_65535 (const char *arg, int *i)
{
  return set_int_range (arg, i, 1, 65535);
}

char *
set_int_0_to_10 (const char *arg, int *i)
{
  return set_int_range (arg, i, 0, 10);
}

char *
set_int_1_to_10 (const char *arg, int *i)
{
  return set_int_range (arg, i, 1, 10);
}

void
get_intrange (char *arg, int *val1, int *val2)
{
...
set_devices (char *arg)
{
  ...
  do
      ...
      if (nextptr == NULL)
	{
      ...
      if (val1 < 0 || val1 > MAX_DEVICES || val2 < 0
	  || val2 > MAX_DEVICES || val1 > val2)
	{
	  free (p);
...
static char *
set_balance (enum pool_strategy *strategy)
{
...
static char *
set_loadbalance (enum pool_strategy *strategy)
{
...
static char *
set_rotate (const char *arg, int *i)
{
...
static char *
set_rr (enum pool_strategy *strategy)
{
...
bool
detect_stratum (struct pool * pool, char *url)
{
...
static void
setup_url (struct pool *pool, char *arg)
{
...
static char *
set_pool_algorithm (const char *arg)
{
...
static char *
set_pool_backup (const char *arg)
{
  struct pool *pool = get_current_pool ();
  pool->backup = true;
  return NULL;
...
static char *
set_pool_devices (const char *arg)
{
  struct pool *pool = get_current_pool ();
  pool->devices = arg;
  return NULL;
...
static char *
set_pool_kernelfile (const char *arg)
{
  struct pool *pool = get_current_pool ();
...
static char *
set_pool_lookup_gap (const char *arg)
{
  struct pool *pool = get_current_pool ();
  pool->lookup_gap = arg;
  return NULL;
...
static char *
set_pool_intensity (const char *arg)
{
  struct pool *pool = get_current_pool ();
...
static char *
set_pool_xintensity (const char *arg)
{
  struct pool *pool = get_current_pool ();
...
static char *
set_pool_rawintensity (const char *arg)
{
  struct pool *pool = get_current_pool ();
...
static char *
set_pool_thread_concurrency (const char *arg)
{
  struct pool *pool = get_current_pool ();
...
#ifdef HAVE_ADL
static char *
set_pool_gpu_engine (const char *arg)
{
  struct pool *pool = get_current_pool ();
  pool->gpu_engine = arg;
  return NULL;
...
static char *
set_pool_gpu_memclock (const char *arg)
{
  struct pool *pool = get_current_pool ();
...
static char *
set_pool_gpu_threads (const char *arg)
{
  struct pool *pool = get_current_pool ();
...
static char *
set_pool_gpu_fan (const char *arg)
{
  struct pool *pool = get_current_pool ();
  pool->gpu_fan = arg;
  return NULL;
...
static char *
set_pool_gpu_powertune (const char *arg)
{
  struct pool *pool = get_current_pool ();
...
static char *
set_pool_gpu_vddc (const char *arg)
{
  struct pool *pool = get_current_pool ();
  pool->gpu_vddc = arg;
  return NULL;
...
static char *
set_pool_nfactor (const char *arg)
{
  struct pool *pool = get_current_pool ();
...
static char *
set_poolname_deprecated (char *arg)
{
  applog (LOG_ERR,
...
static char *
set_pool_shaders (const char *arg)
{
  struct pool *pool = get_current_pool ();
  pool->shaders = arg;
  return NULL;
...
static char *
set_pool_worksize (const char *arg)
{
  struct pool *pool = get_current_pool ();
  pool->worksize = arg;
  return NULL;
...
set_quota (char *arg)
{
  ...
  *semicolon = '\0';
  ...
  setup_url (pool, url);
  pool->quota = quota;
  applog (LOG_INFO, "Setting %s to quota %d", get_pool_name (pool),
	  pool->quota);
  adjust_quota_gcd ();
...
static char *
set_no_extranonce_subscribe (char *arg)
{
  struct pool *pool = get_current_pool ();
...
static char *
set_schedtime (const char *arg, struct schedtime *st)
{
  ...
  if ((!*r) && i >= 0 && i <= INT_MAX)
    {
      ...
      if (!sharelog_file)
	applog (LOG_ERR, "Failed to open fd %u for share log",
		(unsigned int) i);
    }
  else if (!strcmp (arg, "-"))
    {
...
load_temp_cutoffs ()
{
  ...
  if (temp_cutoff_str)
    {
      for (device = 0, nextptr = strtok (temp_cutoff_str, ",");
	   nextptr; ++device, nextptr = strtok (NULL, ","))
	{
...
static char *
set_api_description (const char *arg)
{
...
static char *
set_api_mcast_addr (const char *arg)
{
...
static char *
set_api_mcast_code (const char *arg)
{
...
static char *
set_api_mcast_des (const char *arg)
{
...
static char *
set_null (const char __maybe_unused * arg)
{
  return NULL;
}

char *
set_difficulty_multiplier (char *arg)
{
  applog (LOG_WARNING,
	  "Option difficulty-multiplier is deprecated and will be removed in v5.1. Pools that need this option should fix their setup.");
  if (!(arg && arg[0]))
...
struct opt_table opt_config_table[] = {
  ...
  OPT_WITH_ARG ("--failover-switch-delay",
		set_int_1_to_65535, opt_show_intval,
  ...
  OPT_WITH_ARG ("--xintensity|-X",
		...
		MIN_XINTENSITY_STR " to " MAX_XINTENSITY_STR
...
static char *
opt_verusage_and_exit (const char *extra)
{
  printf ("%s\n", packagename);
...
#ifdef HAVE_LIBCURL
static bool
jobj_binary (const json_t * obj, const char *key,
...
static bool
__build_gbt_txns (struct pool *pool, json_t * res_val)
{
  json_t *txn_array;
  bool ret = false;
  size_t cal_len;
  ...
  for (i = 0; i < pool->gbt_txns; i++)
    {
      json_t *txn_val =
	json_object_get (json_array_get (txn_array, i), "data");
...
__gbt_merkleroot (struct pool *pool)
{
...
update_gbt (struct pool *pool)
{
  ...
  val =
    json_rpc_call (curl, curl_err_str, pool->rpc_url, pool->rpc_userpass,
  ...
  if (val)
    ...
    {
      applog (LOG_DEBUG, "FAILED to update GBT from %s",
	      get_pool_name (pool));
    }
...
static double
get_work_blockdiff (const struct work *work)
{
  ...
  if (work->pool->algorithm.type == ALGO_NEOSCRYPT)
    {
      diff64 = bswap_64 (((uint64_t)
			  (be32toh (*((uint32_t *) (work->data + 72))) &
			   0xFFFFFF00)) << 8);
	...
	diff64 = 1;
      double d =
	(double) work->pool->algorithm.diff_numerator / (double) diff64;
  ...
  else
    {
      ...
      powdiff = (8 * (0x1d - 3)) - (8 * (shift - 3));;
      diff64 =
	be32toh (*((uint32_t *) (work->data + 72))) & 0x0000000000FFFFFF;
...
static void
gen_gbt_work (struct pool *pool, struct work *work)
{
...
static bool
gbt_decode (struct pool *pool, json_t * res_val)
{
  ...
  coinbasetxn =
    json_string_value (json_object_get
		       (json_object_get (res_val, "coinbasetxn"), "data"));
...
double
le256todouble (const void *target)
{
  uint64_t *data64;
  double dcut64;
...
static bool
getwork_decode (json_t * res_val, struct work *work)
{
  size_t worklen = 128;
  ...
  if (work->pool->algorithm.type == ALGO_CRE
    ...
    {
      if (!jobj_binary
	  (res_val, "midstate", work->midstate,
  ...
  if (unlikely
      (!jobj_binary
       (res_val, "target", work->target, sizeof (work->target), true)))
    {
      if (opt_morenotices)
...
static bool
pool_localgen (struct pool *pool)
{
  return (pool->has_stratum || pool->has_gbt);
}

static bool
work_decode (struct pool *pool, struct work *work, json_t * val)
{
...
static bool
work_decode_eth (struct pool *pool, struct work *work,
		 json_t * val, json_t * ethval2)
{
//      int i;
  bool ret = false;
...
void
decay_time (double *f, double fadd, double fsecs)
{
  double ftotal, fprop;
...
static void
suffix_string (uint64_t val, char *buf, size_t bufsiz, int sigdigits)
{
  ...
  if (!sigdigits)
  ...
  else
    {
       ...
       * followed by suffix */
      int ndigits =
	sigdigits - 1 - (dval > 0.0 ? (int) floor (log10 (dval)) : 0);
 ...
 * associated suitable for Mega, Giga etc. Buf array needs to be long enough */
void
suffix_string_double (double val, char *buf, size_t bufsiz, int sigdigits)
{
  if (val < 10)
    {
...
double
cgpu_runtime (struct cgpu_info *cgpu)
{
  struct timeval now;
  double dev_runtime;
...
static void
get_statline (char *buf, size_t bufsiz, struct cgpu_info *cgpu)
{
...
static void
curses_print_uptime (struct timeval *start_time)
{
...
curses_print_status (void)
{
  ...
  cg_mvwprintw (statuswin, line, 0,
		PACKAGE " " CGMINER_VERSION " - Started: %s", datestamp);
  ...
  if (shared_strategy () && total_pools > 1)
  ...
  else
    {
      cg_mvwprintw (statuswin, ++line, 0,
		    ...
		    get_pool_name (pool),
		    pool->has_stratum ? "stratum" : (pool->has_gbt ? "GBT" :
						     "longpoll"), pool->diff,
		    get_pool_user (pool));
    }
  ...
  cg_mvwprintw (statuswin, devcursor - 1, 0,
		"[P]ool management [G]PU management [S]ettings [D]isplay options [Q]uit");
}

static void
adj_width (int var, int *length)
{
...
static void
curses_print_devstatus (struct cgpu_info *cgpu, int count)
{
  ...
  if ((cgpu->diff_accepted + cgpu->diff_rejected) > 0)
    reject_pct =
      (cgpu->diff_rejected /
       (cgpu->diff_accepted + cgpu->diff_rejected)) * 100;
  ...
  cg_wprintw (statuswin, "/%6sh/s | R:%*.1f%% HW:%*d WU:%*.3f/m",
	      displayed_hashes,
	      drwidth, reject_pct, hwwidth, cgpu->hw_errors, wuwidth + 2, wu);
  logline[0] = '\0';
...
static void
switch_logsize (bool __maybe_unused newdevs)
{
  if (curses_active_locked ())
    {
#ifdef WIN32
      ...
      else
	{
	  logstart =
	    devcursor +
	    (opt_removedisabled ? most_devices : total_devices) + 1;
	}
...
#ifdef HAVE_CURSES
bool
_log_curses_only (int prio, const char *datetime, const char *str)
{
  bool high_prio;
...
static void
share_result (json_t * val, json_t * res, json_t * err,
  ...
  if (json_is_true (res) || (work->gbt && json_is_null (res))
      ...
      if (!QUIET)
	{
	  if (total_pools > 1)
	    {
	      applog (LOG_NOTICE,
		      "Accepted %s %s %d at %s %s%s", hashshow,
	    ...
	    {
	      applog (LOG_DEBUG,
		      "[THR%d] Accepted %s %s %d %s%s",
		      work->thr_id, hashshow, cgpu->drv->name,
		      cgpu->device_id,
		      resubmit ? "(resubmit)" : "", worktime);
	      applog (LOG_NOTICE, "Accepted %s %s %d %s%s",
		      hashshow, cgpu->drv->name,
		      cgpu->device_id,
		      resubmit ? "(resubmit)" : "", worktime);
	    }
	}
  ...
  else
    {
      ...
      applog (LOG_DEBUG,
	      "[THR%d] PROOF OF WORK RESULT: false (booooo)", work->thr_id);
      if (!QUIET)
	{
	  ...
	  applog (LOG_NOTICE, "Rejected %s %s %d %s%s %s%s",
		  ...
		  reason, resubmit ? "(resubmit)" : "", worktime);
...
static void
show_hash (struct work *work, char *hashshow)
{
  ...
  suffix_string_double (work->work_difficulty, wdiffdisp,
			sizeof (wdiffdisp), 0);
  if (opt_show_coindiff)
    {
      snprintf (hashshow, 64, "Coin %.0f Diff %s/%s%s",
		get_work_blockdiff (work), diffdisp, wdiffdisp,
		work->block ? " BLOCK!" : "");
    }
  else
    {
      ...
      for (ofs = 0; ofs <= 28; ++ofs)
	{
      ...
      snprintf (hashshow, 64, "%08lx Diff %s/%s%s", h32, diffdisp,
		wdiffdisp, work->block ? " BLOCK!" : "");
    }
}

#ifdef HAVE_LIBCURL
static void
text_print_status (int thr_id)
{
  struct cgpu_info *cgpu;
  char logline[256];
...
static bool
submit_upstream_work (struct work *work, CURL * curl,
  ...
  if (work->pool->algorithm.type == ALGO_ETHASH)
    {
      ...
      applog (LOG_DEBUG,
	      "eth_submitWork: {\"jsonrpc\":\"2.0\", \"method\":\"eth_submitWork\", \"params\":[\"0x%s\", \"0x%s\", \"0x%s\"],\"id\":1}",
	      ASCIINonce, ASCIIPoWHash, ASCIIMixHash);
      ...
      else
	{
      ...
      applog (LOG_DEBUG, "DBG: sending %s submit RPC call: %s",
	      pool->rpc_url, s);
      s = (char *) realloc_strcat (s, "\n");
      ...
      val =
	json_rpc_call (curl, curl_err_str, pool->rpc_url,
		       pool->rpc_userpass, s, false, false,
		       &rolltime, pool, true);
	  ...
	  if (opt_worktime)
	    {
	      ...
	      double getwork_time = tdiff ((struct timeval *)
		...
		tdiff ((struct timeval *) &(work->tv_work_start),
	      ...
	      double work_time = tdiff ((struct timeval *)
					&(work->tv_work_found),
					(struct timeval *)
					&(work->tv_work_start));
	      ...
	      if (work->clone)
		{
		  snprintf (workclone, sizeof (workclone),
			    "C:%1.3f",
			    tdiff ((struct timeval *)
				   &(work->tv_cloned),
				   (struct timeval *)
				   &(work->tv_getwork_reply)));
		}
		...
		diffplaces = 6;
	      ...
	      snprintf (worktime, sizeof (worktime),
			...
			(unsigned long) be32toh (*(uint32_t *) &
						 (work->data
						  [32])),
			(unsigned long) be32toh (*(uint32_t *) &
						 (work->data
						  [28])),
			work->getwork_mode, diffplaces,
			work->work_difficulty,
			tm_getwork.tm_hour, tm_getwork.tm_min,
			tm_getwork.tm_sec, getwork_time,
			workclone, getwork_to_work, work_time,
			work_to_submit, submit_time,
			tm_submit_reply.tm_hour,
...
const char eth_gethighestblock_rpc[] =
  "{\"jsonrpc\":\"2.0\",\"method\":\"eth_getBlockByNumber\",\"params\":[\"latest\", false],\"id\":1}";

static bool
get_upstream_work (struct work *work, CURL * curl, char *curl_err_str)
{
  struct pool *pool = work->pool;
  ...
  if (pool->algorithm.type == ALGO_ETHASH)
    {
      ...
      val = json_rpc_call (curl, curl_err_str, url, pool->rpc_userpass,
			   pool->rpc_req, false, false,
			   &work->rolltime, pool, false);

      ethval2 =
	json_rpc_call (curl, curl_err_str, pool->rpc_url,
		       pool->rpc_userpass, eth_gethighestblock_rpc,
    ...
    {
      val =
	json_rpc_call (curl, curl_err_str, url, pool->rpc_userpass,
		       pool->rpc_req, false, false, &work->rolltime,
		       pool, false);
    }
  ...
  if (likely (val))
    {
      rc = (pool->algorithm.type ==
	    ALGO_ETHASH) ? work_decode_eth (pool, work, val,
					    ethval2) :
  ...
  pool_stats->getwork_wait_rolling +=
    ((double) tv_elapsed.tv_sec +
     ((double) tv_elapsed.tv_usec / 1000000)) * 0.63;
...
static bool
pool_unworkable (struct pool *pool)
{
  if (pool->idle)
    return true;
  if (pool->state != POOL_ENABLED)
...
static struct pool *
select_balanced (struct pool *cp)
{
 ...
 */
static void
calc_diff (struct work *work, double known)
{
  ...
  if (known)
  ...
  else
    {
      ...
      applog (LOG_DEBUG, "calc_diff() algorithm = %s",
	      work->pool->algorithm.name);
      // Neoscrypt
      if (work->pool->algorithm.type == ALGO_NEOSCRYPT)
	{
  ...
  suffix_string_double (difficulty, work->pool->diff,
			sizeof (work->pool->diff), 0);
...
static bool
curses_active_forcelocked (void)
{
  bool ret;
...
static void
kill_timeout (struct thr_info *thr)
{
...
static void
sighandler (int __maybe_unused sig)
{
#ifndef _MSC_VER
 ...
 * this pool. */
static void
recruit_curl (struct pool *pool)
{
  struct curl_ent *ce =
    (struct curl_ent *) calloc (sizeof (struct curl_ent), 1);
...
static struct curl_ent *
pop_curl_entry (struct pool *pool)
{
...
static void
push_curl_entry (struct curl_ent *ce, struct pool *pool)
{
  mutex_lock (&pool->pool_lock);
...
static uint32_t
_get_work_time (struct work *work)
{
  uint32_t *data = (uint32_t *) work->data;
...
static void
_set_work_time (struct work *work, uint32_t ntime)
{
  uint32_t *data = (uint32_t *) work->data;
  uint32_t *work_ntime = &data[17];
  if (work->pool && work->pool->algorithm.type == ALGO_DECRED)
...
static void *
submit_work_thread (void *userdata)
{
...
#else /* HAVE_LIBCURL */
static void *
submit_work_thread (void __maybe_unused * userdata)
{
  pthread_detach (pthread_self ());
  return NULL;
}
...
static char *
offset_ntime (const char *ntime, int noffset)
{
  unsigned char bin[4];
...
static void
_copy_work (struct work *work, const struct work *base_work, int noffset)
{
  int id = work->id;
...
struct work *
copy_work_noffset (struct work *base_work, int noffset)
{
  struct work *work = make_work ();
...
stale_work (struct work *work, bool share)
{
  ...
  if (opt_fail_only && !share && pool != current_pool ()
      && !work->mandatory && pool_strategy != POOL_LOADBALANCE
      && pool_strategy != POOL_BALANCE)
    {
...
static double
share_diff (const struct work *work)
{
  bool new_best = false;
  double d64, s64;
  double ret;
...
static bool
pool_unusable (struct pool *pool)
{
  if (pool->idle)
    return true;
  if (pool->state != POOL_ENABLED)
...
void
__switch_pools (struct pool *selected, bool saveprio)
{
...
discard_work (struct work *work)
{
  if (!work->clone && !work->rolls && !work->mined)
    ...
    applog (LOG_DEBUG, "[THR%d] Discarded cloned or rolled work",
	    work->thr_id);
  free_work (work);
}
...
discard_stale (void)
{
  ...
  if (stale)
    applog (LOG_DEBUG,
	    "Discarded %d stales that didn't match current hash", stale);
}

static void *
restart_thread (void __maybe_unused * arg)
{
...
static void
set_curblock (char *hexstr, unsigned char *bedata)
{
  int ofs;
...
static int
block_sort (struct block *blocka, struct block *blockb)
{
...
static void
set_blockdiff (const struct work *work)
{
...
static bool
test_work_current (struct work *work)
{
  struct pool *pool = work->pool;
...
static int
tv_sort (struct work *worka, struct work *workb)
{
  return worka->tv_staged.tv_sec - workb->tv_staged.tv_sec;
}

static bool
work_rollable (struct work *work)
{
  return (!work->clone && work->rolltime);
}

static bool
hash_push (struct work *work)
{
  bool rc = true;
...
static void
stage_work (struct work *work)
{
  applog (LOG_DEBUG, "[THR%d] Pushing work from %s to hash queue",
	  work->thr_id, get_pool_name (work->pool));
  work->work_block = work_block;
...
#ifdef HAVE_CURSES
static void
display_pool_summary (struct pool *pool)
{
  ...
  if (curses_active_locked ())
	...
	wlog (" Reject ratio: %.1f%%\n",
      ...
      efficiency =
	pool->getwork_requested ? pool->accepted * 100.0 /
	pool->getwork_requested : 0.0;
  ...
  for (i = 0; i < total_pools; i++)
	...
	default:
      ...
      wlogprint ("%s Quota %d  Prio %d '%s'  User:%s\n",
		 ...
		 pool->quota,
		 pool->prio, get_pool_name (pool), get_pool_user (pool));
...
void
default_save_file (char *filename)
{
...
static void *
input_thread (void __maybe_unused * userdata)
{
...
static void
thread_reportin (struct thr_info *thr)
{
  cgtime (&thr->last);
  thr->getwork = false;
 ...
 * be restarted */
static void
thread_reportout (struct thr_info *thr)
{
  cgtime (&thr->last);
  thr->getwork = true;
  thr->cgpu->status = LIFE_WELL;
...
static void
hashmeter (int thr_id, struct timeval *diff, uint64_t hashes_done)
{
  ...
  if (thr)
    {
      ...
      applog (LOG_DEBUG,
	      "[thread %d: %" PRIu64 " hashes, %.1f khash/sec]",
...
static void
stratum_share_result (json_t * val, json_t * res_val,
		      json_t * err_val, struct stratum_share *sshare)
{
...
static bool
parse_stratum_response (struct pool *pool, char *s)
{
      ...
      if (pool->algorithm.type == ALGO_CRYPTONIGHT)
	{
	  ...
	  if (json_is_null (err_val)
	      ...
	      !strcmp (json_string_value
		       (json_object_get (res_val, "status")), "OK"))
	    {
	      success = true;
	    }
	  else
...
void
clear_stratum_shares (struct pool *pool)
{
...
void
clear_pool_work (struct pool *pool)
{
  struct work *work, *tmp;
  int cleared = 0;
...
static void
stratum_resumed (struct pool *pool)
{
  if (!pool->stratum_notify)
    return;
  if (pool_tclear (pool, &pool->idle))
    {
      applog (LOG_INFO, "Stratum connection to %s resumed",
	      get_pool_name (pool));
      pool_resus (pool);
    }
}
...
static bool
supports_resume (struct pool *pool)
{
  bool ret;
...
stratum_rthread (void *userdata)
{
  ...
  while (42)
      ...
      if (!sock_full (pool) && !cnx_needed (pool))
	{
      ...
      if (!sock_full (pool)
	  && (sel_ret =
	      select (pool->sock + 1, &rd, NULL, NULL, &timeout)) < 1)
	{
	  applog (LOG_DEBUG,
...
//              uint64_t *nonce2_64;
      ...
      if (unlikely (work->nonce2_len > 8))
	{
      ...
      sshare =
	(struct stratum_share *) calloc (sizeof (struct stratum_share), 1);
      ...
      applog (LOG_DEBUG, "stratum_sthread() algorithm = %s",
	      pool->algorithm.name);
...
// Common to all algorithms
      ...
      *((uint64_t *) nonce2) = htole64 (work->nonce2);
      ...
      sshare->id = swork_id++;
      mutex_unlock (&sshare_lock);
// Send the share to the pool connection. Vary stratum protocol by algorithm.
      if (pool->algorithm.type == ALGO_ETHASH)
	{
	  uint64_t tmp = bswap_64 (work->Nonce);
	  char *ASCIIMixHash = bin2hex (work->mixhash, 32);
      ...
      else if (pool->algorithm.type == ALGO_CRYPTONIGHT)
	{
	  ...
	  snprintf (s, sizeof (s),
		    ...
		    pool->XMRAuthID, work->job_id, ASCIINonce,
		    ASCIIResult, sshare->id);
	  free (ASCIINonce);
	  free (ASCIIResult);
	}
      else if (pool->algorithm.type == ALGO_DECRED && opt_vote)
	{
	  snprintf (s, sizeof (s),
		    ...
		    pool->rpc_user, work->job_id, nonce2hex,
		    work->ntime, noncehex, (opt_vote << 1) | 1, sshare->id);
	}
      else
	{
	  snprintf (s, sizeof (s),
...
// Log the share submission
      ...
      else
	{
	  applog (LOG_INFO, "Submitting share %08lx to %s",
		  (long unsigned int) htole32 (hash32[6]),
		  get_pool_name (pool));
	}
      ...
      while (time (NULL) < sshare->sshare_time + 120)
	{
	  ...
	  cg_rlock (&pool->data_lock);
	  sessionid_match = (pool->nonce1
			     && !strcmp (work->nonce1, pool->nonce1));
	  cg_runlock (&pool->data_lock);
	  if (!sessionid_match)
	    {
	      applog (LOG_DEBUG,
...
static void
init_stratum_threads (struct pool *pool)
{
  have_longpoll = true;
  if (unlikely
      (pthread_create
       (&pool->stratum_sthread, NULL, stratum_sthread, (void *) pool)))
    quit (1, "Failed to create stratum sthread");
  if (unlikely
      (pthread_create
       (&pool->stratum_rthread, NULL, stratum_rthread, (void *) pool)))
    quit (1, "Failed to create stratum rthread");
  applog (LOG_DEBUG, "Stratum threads created.");
} static void *longpoll_thread (void *userdata);

static bool
stratum_works (struct pool *pool)
{
  applog (LOG_INFO, "Testing %s stratum %s",
	  get_pool_name (pool), pool->stratum_url);
  if (!extract_sockaddr
      (pool->stratum_url, &pool->sockaddr_url, &pool->stratum_port))
    return false;
...
static bool
pool_active (struct pool *pool, bool pinging)
{
  ...
  if (pool->has_gbt)
    applog (LOG_DEBUG,
	    "Retrieving block template from %s", get_pool_name (pool));
  else
    applog (LOG_INFO, "Testing %s in pool_active", get_pool_name (pool));

  /* This is the central point we activate stratum when we can */
retry_stratum:if (pool->has_stratum)
    {
      ...
      if (!init)
	{
	  bool ret = initiate_stratum (pool) && (!pool->extranonce_subscribe
						 ||
  ...
  if (!pool->probed)
    {
      ...
      val =
	json_rpc_call (curl, curl_err_str, pool->rpc_url,
		       pool->rpc_userpass, gbt_req, true,
  ...
  if (pool->algorithm.type == ALGO_ETHASH)
    {
      ...
      val =
	json_rpc_call (curl, curl_err_str, pool->rpc_url,
		       pool->rpc_userpass, pool->rpc_req,
	...
	{
	  applog (LOG_NOTICE, "Switching %s to %s",
		  get_pool_name (pool), pool->stratum_url);
	  if (!pool->rpc_url)
  ...
  if (val)
      ...
      if (rc)
	{
	  ...
	  work->pool = pool;
	  work->rolltime =
	    (pool->algorithm.type == ALGO_ETHASH) ? 0 : rolltime;
      ...
      if (pool->hdr_path)
	{
	  ...
	  if (strstr (hdr_path, "://"))
	      ...
	      if (!pool->lp_url)
		{
	      ...
	      snprintf (pool->lp_url, siz, "%s%s%s",
			pool->rpc_url, need_slash ? "/" : "", copy_start);
	    }
	}
      else
	pool->lp_url = NULL;
      if (!pool->lp_started)
	{
	  ...
	  if (unlikely
	      (pthread_create
	       (&pool->longpoll_thread, NULL,
		longpoll_thread, (void *) pool)))
      ...
      if (initiate_stratum (pool))
	{
      ...
      applog (LOG_DEBUG,
	      "FAILED to retrieve work from %s", get_pool_name (pool));
      if (!pinging)
	applog (LOG_WARNING,
...
void
set_target (unsigned char *dest_target,
	    double diff, double diff_multiplier2, const int thr_id)
{
 ...
 ****************************************************/
void
set_target_neoscrypt (unsigned char *target, double diff, const int thr_id)
{
  uint64_t m;
  int k;
  diff /= 65536.0;
  ...
  if (opt_debug)
    {
      ...
      applog (LOG_DEBUG,
	      "[THR%d] Generated neoscrypt target 0x%s", thr_id, htarget);
      free (htarget);
    }
}
...
static void
gen_stratum_work_eth (struct pool *pool, struct work *work)
{
  if (pool->algorithm.type != ALGO_ETHASH)
    return;
  applog (LOG_DEBUG,
	  "[THR%d] gen_stratum_work_eth() - algorithm = %s",
 ...
 * other means to detect when the pool has died in stratum_thread */
static void
gen_stratum_work_cn (struct pool *pool, struct work *work)
{
  if (pool->algorithm.type != ALGO_CRYPTONIGHT)
    return;
  applog (LOG_DEBUG,
	  "[THR%d] gen_stratum_work_cn() - algorithm = %s",
	  work->thr_id, pool->algorithm.name);
  cg_rlock (&pool->data_lock);
  work->job_id = strdup (pool->swork.job_id);
...
static void
gen_stratum_work (struct pool *pool, struct work *work)
{
  ...
  if (pool->algorithm.type != ALGO_DECRED && pool->algorithm.type != ALGO_SIA)
    {
      ...
      pool->algorithm.gen_hash (pool->coinbase,
				pool->swork.cb_len, merkle_root);
  ...
  applog (LOG_DEBUG,
	  "[THR%d] gen_stratum_work() - algorithm = %s",
	  work->thr_id, pool->algorithm.name);
  ...
  if (pool->algorithm.type == ALGO_NEOSCRYPT)
      ...
      for (i = 0; i < temp; ++i)
	{
	  ((uint32_t *) work->data)[i] =
	    be32toh (((uint32_t *) pool->header_bin)[i]);
	}
	...
	{
	  ((uint32_t *) work->data)[i] =
	    le32toh (((uint32_t *) merkle_root)[j]);
	}
  ...
  if (opt_debug)
      ...
      if (pool->algorithm.type != ALGO_DECRED)
	{
	  ...
	  applog (LOG_DEBUG, "[THR%d] Generated stratum merkle %s",
		  work->thr_id, merkle_hash);
	  free (merkle_hash);
	}
      applog (LOG_DEBUG,
	      "[THR%d] Generated stratum header %s", work->thr_id, header);
      applog (LOG_DEBUG,
	      "[THR%d] Work job_id %s nonce2 %" PRIu64 " ntime %s",
  ...
  if (pool->algorithm.type == ALGO_NEOSCRYPT)
	...
	pool->algorithm.calc_midstate (work);
      set_target (work->target, work->sdiff,
		  pool->algorithm.diff_multiplier2, work->thr_id);
    }
...
static void
apply_initial_gpu_settings (struct pool *pool)
{
  int i;
  const char *opt;
...
*/
  ...
  for (i = 0; i < nDevs; i++)
    {
      applog (LOG_DEBUG, "Set GPU %d to %s", i,
	      isnull (pool->algorithm.name, ""));
...
/*
  ...
  if (!empty_string
      ((opt =
	get_pool_setting (pool->gpu_threads, default_profile.gpu_threads))))
...
compare_pool_settings (struct pool *oldpool, struct pool *newpool)
{
  ...
  opt1 = ((oldpool) ?
	  get_pool_setting (oldpool->devices,
			    ((!empty_string
			      (default_profile.devices)) ?
			     default_profile.devices : "all")) : "");
  opt2 =
    get_pool_setting (newpool->devices,
		      ((!empty_string
			(default_profile.devices)) ?
		       default_profile.devices : "all"));
  ...
  opt1 =
    ((oldpool) ?
     get_pool_setting (oldpool->gpu_threads,
  ...
  //compare algorithm
  if ((oldpool
       && !cmp_algorithm (&oldpool->algorithm,
			  &newpool->algorithm)) || (!oldpool))
    {
  ...
  opt1 =
    ((oldpool) ?
     get_pool_setting (oldpool->lookup_gap, default_profile.lookup_gap) : "");
  ...
  if (!empty_string (newpool->rawintensity))
    {
      ...
      opt1 =
	((oldpool
	  && !empty_string (oldpool->rawintensity)) ?
	 oldpool->rawintensity : "");
      opt2 =
	get_pool_setting (newpool->rawintensity,
  ...
  else if (!empty_string (newpool->xintensity))
    {
      ...
      opt1 =
	((oldpool
	  && !empty_string (oldpool->xintensity)) ? oldpool->xintensity : "");
      opt2 =
	get_pool_setting (newpool->xintensity, default_profile.xintensity);
    }
  else
    {
      ...
      opt1 =
	((oldpool
	  && !empty_string (oldpool->intensity)) ? oldpool->intensity : "");
  ...
  opt1 =
    ((oldpool) ?
     get_pool_setting (oldpool->shaders, default_profile.shaders) : "");
  ...
  if (newpool->algorithm.type == ALGO_NEOSCRYPT)
    {
      opt2 =
	((empty_string (newpool->thread_concurrency)) ?
	 "0" :
  ...
  opt1 =
    ((oldpool) ?
     get_pool_setting (oldpool->thread_concurrency,
		       default_profile.thread_concurrency) : "");
  ...
  opt1 =
    ((oldpool) ?
     get_pool_setting (oldpool->worksize, default_profile.worksize) : "");
  ...
  opt1 =
    ((oldpool) ?
     get_pool_setting (oldpool->gpu_engine, default_profile.gpu_engine) : "");
  ...
  opt1 =
    ((oldpool) ?
     get_pool_setting (oldpool->gpu_memclock,
		       default_profile.gpu_memclock) : "");
  opt2 =
    get_pool_setting (newpool->gpu_memclock, default_profile.gpu_memclock);
  if (strcasecmp (opt1, opt2) != 0 && !empty_string (opt2))
    {
  ...
  opt1 =
    ((oldpool) ?
     get_pool_setting (oldpool->gpu_fan, default_profile.gpu_fan) : "");
  ...
  opt1 =
    ((oldpool) ?
     get_pool_setting (oldpool->gpu_powertune,
  ...
  opt1 =
    ((oldpool) ?
     get_pool_setting (oldpool->gpu_vddc, default_profile.gpu_vddc) : "");
...
static void
apply_switcher_options (unsigned long options, struct pool *pool)
{
  int i;
  const char *opt;
  ...
  if (opt_isset (options, SWITCHER_APPLY_LG))
    {
      if (!empty_string
	  ((opt =
	    get_pool_setting (pool->lookup_gap, default_profile.lookup_gap))))
	set_lookup_gap ((char *) opt);
    }
  ...
  if (opt_isset (options, SWITCHER_APPLY_RAWINT))
    {
      applog (LOG_DEBUG,
	      "Switching to rawintensity: pool = %s, default = %s",
	      pool->rawintensity, default_profile.rawintensity);
      opt =
	get_pool_setting (pool->rawintensity, default_profile.rawintensity);
  ...
  //xintensity
    ...
    {
      applog (LOG_DEBUG,
	      "Switching to xintensity: pool = %s, default = %s",
  ...
  //intensity
    ...
    {
      applog (LOG_DEBUG,
	      "Switching to intensity: pool = %s, default = %s",
  ...
  if (opt_isset (options, SWITCHER_APPLY_TC))
	...
	{
	  opt =
	    ((empty_string (pool->thread_concurrency)) ? "0" :
  ...
  if (opt_isset (options, SWITCHER_APPLY_WORKSIZE))
    {
      if (!empty_string
	  ((opt =
	    get_pool_setting (pool->worksize, default_profile.worksize))))
	set_worksize (opt);
    }
#ifdef HAVE_ADL
  //GPU clock
  if (opt_isset (options, SWITCHER_APPLY_GPU_ENGINE))
    {
      if (!empty_string
	  ((opt =
	    get_pool_setting (pool->gpu_engine, default_profile.gpu_engine))))
	set_gpu_engine ((char *) opt);
    }
  //GPU memory clock
  if (opt_isset (options, SWITCHER_APPLY_GPU_MEMCLOCK))
    {
      if (!empty_string
	  ((opt =
	    get_pool_setting (pool->gpu_memclock,
			      default_profile.gpu_memclock))))
	set_gpu_memclock ((char *) opt);
    }
  //GPU fans
  ...
  if (opt_isset (options, SWITCHER_APPLY_GPU_POWERTUNE))
    {
      if (!empty_string
	  ((opt =
	    get_pool_setting (pool->gpu_powertune,
			      default_profile.gpu_powertune))))
	set_gpu_powertune ((char *) opt);
    }
  //GPU vddc
  if (opt_isset (options, SWITCHER_APPLY_GPU_VDDC))
    {
      if (!empty_string
	  ((opt =
	    get_pool_setting (pool->gpu_vddc, default_profile.gpu_vddc))))
	set_gpu_vddc ((char *) opt);
    }
  //apply gpu settings
  for (i = 0; i < nDevs; ++i)
...
static void
mutex_unlock_cleanup_handler (void *mutex)
{
...
get_work_prepare_thread (struct thr_info *mythr, struct work *work)
{
  ...
  if (algo_switch_n == 0)
      ...
      if (opt_switchmode == SWITCH_ALGO)
	{
	  ...
	  if (!cmp_algorithm
	      (&work->pool->algorithm, &mythr->cgpu->algorithm))
	    pool_switch_options =
      ...
      else if (opt_switchmode == SWITCH_POOL)
	{
	  if (work->pool->pool_no != mythr->pool_no)
	    {
	      if ((pool_switch_options =
		   compare_pool_settings (pools
					  [mythr->pool_no], work->pool)) == 0)
		{
		  applog (LOG_DEBUG,
  ...
  if (algo_switch_n >= active_threads)
      ...
      if (opt_isset (pool_switch_options, SWITCHER_HARD_RESET))
	{
	  ...
	  if (opt_isset (pool_switch_options, SWITCHER_APPLY_GT))
	    {
	      if (!empty_string
		  ((opt =
		    get_pool_setting (work->pool->gpu_threads,
				      default_profile.gpu_threads))))
		set_gpu_threads (opt);
	    }
	  ...
	  applog (LOG_DEBUG,
		  "Hard reset: Exiting mining thread %d", mythr->id);
	  pthread_exit (NULL);
	}
      else
	{
    ...
    {
      bool hard_reset = opt_isset (pool_switch_options,
				   SWITCHER_HARD_RESET);
      mutex_unlock (&algo_switch_lock);
      if (hard_reset)
	{
  ...
  pthread_cleanup_push
    (mutex_unlock_cleanup_handler, (void *) &algo_switch_wait_lock);
...
get_work (struct thr_info *thr, const int thr_id)
{
  ...
  applog (LOG_DEBUG,
	  "[THR%d] Popping work from get queue to get work", thr_id);
  diff_t = time (NULL);
  while (!work)
    {
      work = hash_pop (true);
  ...
  if (diff_t > 0)
    {
      applog (LOG_DEBUG,
	      "[THR%d] Get work blocked for %d seconds",
	      thr_id, (int) diff_t);
...
static void
submit_work_async (struct work *work)
{
  struct pool *pool = work->pool;
  ...
  if (work->stratum)
    {
      applog (LOG_DEBUG,
	      "Pushing %s work to stratum queue", get_pool_name (pool));
  ...
  else
    {
      ...
      if (unlikely
	  (pthread_create
	   (&submit_thread, NULL, submit_work_thread, (void *) work)))
	quit (1, "Failed to create submit_work_thread");
    }
}

void
inc_hw_errors (struct thr_info *thr)
{
  applog (LOG_INFO,
	  "[THR%d] %s%d: invalid nonce - HW error",
	  thr->id, thr->cgpu->drv->name, thr->cgpu->device_id);
  mutex_lock (&stats_lock);
  hw_errors++;
  thr->cgpu->hw_errors++;
  mutex_unlock (&stats_lock);
...
static void
rebuild_nonce (struct work *work, uint32_t nonce)
{
...
bool
test_nonce (struct work *work, uint32_t nonce)
{
...
static void
update_work_stats (struct thr_info *thr, struct work *work)
{
...
bool
submit_tested_work (struct thr_info *thr, struct work *work)
{
...
bool
submit_nonce (struct thr_info * thr, struct work * work, uint32_t nonce)
{
...
static inline bool
abandon_work (struct work *work, struct timeval *wdiff, uint64_t hashes)
{
...
static void
mt_disable (struct thr_info *mythr, const int thr_id, struct device_drv *drv)
{
...
static void
hash_sole_work (struct thr_info *mythr)
{
  const int thr_id = mythr->id;
  struct cgpu_info *cgpu = mythr->cgpu;
  struct device_drv *drv = cgpu->drv;
  struct timeval getwork_start, tv_start, *tv_end,
    tv_workstart, tv_lastupdate;
  ...
  while (likely (!cgpu->shutdown))
	...
	drv->working_diff = work->work_difficulty;
      if (work->pool->algorithm.type == ALGO_NEOSCRYPT)
	set_target_neoscrypt (work->device_target,
			      work->device_diff, work->thr_id);
      else if (work->pool->algorithm.type == ALGO_ETHASH)
	{
	  double mult = 60e6;
	  work->device_diff = MIN (work->work_difficulty, mult);
	  *(uint64_t *) (work->device_target + 24) =
	    bits64 / work->device_diff;
	  work->device_diff /= mult;
	}
      else
	set_target (work->device_target,
		    work->device_diff,
      ...
      do
	{
	  ...
	  if (unlikely (hashes == -1))
	    {
	      applog (LOG_ERR, "%s %d failure, disabling!",
	  ...
	  if (unlikely ((long) sdiff.tv_sec < cycle))
	    ...
	    max_nonce =
	      max_nonce * 0x400 /
	      (((cycle * 1000000) +
		sdiff.tv_usec) / (cycle * 1000000 / 0x400));
	  timersub (tv_end, &tv_lastupdate, &diff);
	  /* Update the hashmeter at most 5 times per second */
	  if ((hashes_done
	       && (diff.tv_sec > 0 || diff.tv_usec > 200000))
...
static void
convert_to_work (json_t * val, int rolltime,
		 struct pool *pool,
		 struct timeval *tv_lp, struct timeval *tv_lp_reply)
{
  struct work *work;
  bool rc;
  work = make_work ();
...
static struct pool *
select_longpoll_pool (struct pool *cp)
{
  int i;
 ...
 */
static void
wait_lpcurrent (struct pool *pool)
{
  while (!cnx_needed (pool)
	 && (pool->state == POOL_DISABLED || (pool != current_pool ()
					      && pool_strategy !=
					      POOL_LOADBALANCE
					      && pool_strategy !=
					      POOL_BALANCE)))
...
retry_pool:pool = select_longpoll_pool (cp);
  if (!pool)
    {
  ...
  if (pool->has_gbt)
	...
	applog (LOG_WARNING,
		"Long-polling activated for %s via %s", cp->rpc_url, lp_url);
    }
  ...
  while (42)
      ...
      if (pool->has_gbt)
	{
      ...
      val =
	json_rpc_call (curl, curl_err_str, lp_url,
		       pool->rpc_userpass, lpreq, false,
		       true, &rolltime, pool, false);
      cgtime (&reply);
      if (likely (val))
	{
	  soval =
	    json_object_get (json_object_get (val, "result"), "submitold");
	  if (soval)
      ...
      else
	{
	  ...
	  if (failures == 1)
	    applog (LOG_WARNING,
		    "longpoll failed for %s, retrying every 30s", lp_url);
	  cgsleep_ms (30000);
	}
...
#else /* HAVE_LIBCURL */
static void *
longpoll_thread (void __maybe_unused * userdata)
{
  pthread_detach (pthread_self ());
  return NULL;
}
#endif /* HAVE_LIBCURL */
void
reinit_device (struct cgpu_info *cgpu)
{
  mutex_lock (&algo_switch_lock);
...
reap_curl (struct pool *pool)
{
  ...
  mutex_unlock (&pool->pool_lock);
  if (reaped)
    applog (LOG_DEBUG, "Reaped %d curl%s from %s",
	    reaped, reaped > 1 ? "s" : "", get_pool_name (pool));
}

static void *
watchpool_thread (void __maybe_unused * userdata)
{
  int intervals = 0;
  pthread_setcanceltype (PTHREAD_CANCEL_ASYNCHRONOUS, NULL);
	  ...
	  if (!pool->idle && pool->prio < cp_prio ())
	    {
		  ...
		  && (now.tv_sec - pool->tv_idle.tv_sec >
		      opt_fail_switch_delay))
		{
		  applog (LOG_WARNING,
			  "%s stable for %d seconds",
			  get_pool_name (pool), opt_fail_switch_delay);
		  switch_pools (NULL);
		}
	    }
...
static void *
watchdog_thread (void __maybe_unused * userdata)
{
  ...
  while (1)
      ...
      for (i = 0; i < total_devices; ++i)
	{
	  ...
	  if (opt_debug && cgpu->has_adl)
	    {
	      int engineclock = 0, memclock = 0, activity =
		0, fanspeed = 0, fanpercent = 0, powertune = 0;
	      float temp = 0, vddc = 0;
	      if (gpu_stats
		  (gpu, &temp, &engineclock, &memclock, &vddc,
		   &activity, &fanspeed, &fanpercent, &powertune))
		applog (LOG_DEBUG,
			"%.1f C  F: %d%%(%dRPM)  E: %dMHz  M: %dMhz  V: %.3fV  A: %d%%  P: %d%%",
			temp, fanpercent, fanspeed,
	  ...
	  if (cgpu->status != LIFE_WELL
	    ...
	    if (now.tv_sec - thr->sick.tv_sec > 60 &&
		(cgpu->status == LIFE_SICK || cgpu->status == LIFE_DEAD))
	    {
...
static void
log_print_status (struct cgpu_info *cgpu)
{
  char logline[255];
  get_statline (logline, sizeof (logline), cgpu);
...
static void noop_get_statline (char __maybe_unused
			       * buf,
			       size_t
			       __maybe_unused
			       bufsiz,
...
print_summary (void)
{
  ...
  applog (LOG_WARNING,
	  "Runtime: %d hrs : %d mins : %d secs", hours, mins, secs);
  ...
  applog (LOG_WARNING,
	  ...
	  displayed_hashes, mhash_base ? "Mega" : "Kilo");
  ...
  applog (LOG_WARNING,
	  "Accepted difficulty shares: %1.f", total_diff_accepted);
  applog (LOG_WARNING,
	  "Rejected difficulty shares: %1.f", total_diff_rejected);
  if (total_accepted || total_rejected)
    applog (LOG_WARNING, "Reject ratio: %.1f%%", (double)
	    (total_rejected *
	     100) / (double) (total_accepted + total_rejected));
  ...
  applog (LOG_WARNING,
	  "Submitting work remotely delay occasions: %d", total_ro);
  ...
  if (total_pools > 1)
      ...
      for (i = 0; i < total_pools; i++)
	{
	  ...
	  if (pool->solved)
	    applog (LOG_WARNING, "SOLVED %d BLOCK%s!",
	  ...
	  if (pool->accepted || pool->rejected)
	    applog (LOG_WARNING,
		    " Reject ratio: %.1f%%", (double)
		    (pool->rejected *
		     100) / (double) (pool->accepted + pool->rejected));
...
test_pool_thread (void *arg)
{
  ...
  if (pool_active (pool, false))
	...
	applog (LOG_NOTICE,
		"Switching to %s - first alive pool", get_pool_name (pool));
      pool_resus (pool);
      switch_pools (NULL);
    }
  else
...
bool
add_pool_details (struct pool * pool, bool live,
		  char *url, char *user,
...
fork_monitor ()
{
  ...
  if (0 == forkpid)
	...
	{
	  perror
	    ("dup2 - in child, failed to alias read end of pipe to stdin for --monitor");
	  exit (1);
	}
      close (pfd[0]);
      if (r < 0)
 ...
 * variants. */
static void
noop_reinit_device (struct cgpu_info __maybe_unused * cgpu)
{
} void

blank_get_statline_before (char *buf,
			   size_t bufsiz,
			   struct cgpu_info __maybe_unused * cgpu)
{
  tailsprintf (buf, bufsiz, "               | ");
...
noop_get_statline (char __maybe_unused * buf,
		   size_t __maybe_unused bufsiz,
		   struct cgpu_info __maybe_unused * cgpu)
{
} static bool

noop_get_stats (struct cgpu_info __maybe_unused * cgpu)
{
  return true;
}

static bool
noop_thread_prepare (struct thr_info __maybe_unused * thr)
{
  return true;
}

static uint64_t
noop_can_limit_work (struct thr_info __maybe_unused * thr)
{
  return 0xffffffff;
}

static bool
noop_thread_init (struct thr_info __maybe_unused * thr)
{
  return true;
}

static bool
noop_prepare_work (struct thr_info __maybe_unused
		   * thr, struct work __maybe_unused * work)
{
  return true;
}

static void
noop_hw_error (struct thr_info __maybe_unused * thr)
{
} static void
...
void
fill_device_drv (struct device_drv *drv)
{
  if (!drv->drv_detect)
...
bool
add_cgpu (struct cgpu_info *cgpu)
{
  ...
  devices =
    (struct cgpu_info **) realloc (devices,
				   sizeof (struct
					   cgpu_info
					   *) * (total_devices + 2));
  ...
  for (i = 0; i < total_pools; i++)
    {
      struct pool *pool = pools[i];
      pool->testing = true;
      pthread_create (&pool->test_thread, NULL,
		      test_pool_thread, (void *) pool);
}} static void
...
restart_mining_threads (unsigned int new_n_threads)
{
  ...
  for (i = 0; i < mining_threads; i++)
    {
      mining_thr[i] =
	(struct thr_info *) calloc (1, sizeof (struct thr_info));
      if (!mining_thr[i])
  ...
  for (i = 0; i < total_devices; ++i)
    {
      ...
      cgpu->thr = (struct thr_info **)
	malloc (sizeof (struct thr_info *) * (cgpu->threads + 1));
      ...
      for (j = 0; j < cgpu->threads; ++j, ++k)
	{
	  ...
	  applog (LOG_DEBUG, "Thread %d set pool = %d (%s)",
		  k, thr->pool_no,
	  ...
	  if (!cgpu->drv->thread_prepare (thr))
	    {
	      applog (LOG_ERR,
		      "thread_prepare failed for thread %d", thr->id);
	      continue;
	    }
	}
    }
...
static void *
restart_mining_threads_thread (void *userdata)
{
  //get thread id
...
main (int argc, char *argv[])
{
  ...
  if (unlikely (pthread_mutex_init (&lockstat_lock, NULL)))
    quithere (1,
	      "Failed to pthread_mutex_init lockstat_lock errno=%d", errno);
#endif
  ...
  snprintf (packagename, sizeof (packagename),
	    "%s %s", PACKAGE, CGMINER_VERSION);
#ifndef WIN32
  ...
  control_thr =
    (struct thr_info *) calloc (total_control_threads, sizeof (*thr));
  if (!control_thr)
  ...
  if (opt_display_devs)
      ...
      for (i = 0; i < total_devices; ++i)
	{
	  struct cgpu_info *cgpu = devices[i];
	  if (cgpu->name)
	    applog (LOG_ERR, " %2d. %s %d: %s (driver: %s)",
		    i, cgpu->drv->name, cgpu->device_id,
		    cgpu->name, cgpu->drv->dname);
	  else
	    applog (LOG_ERR, " %2d. %s %d (driver: %s)", i,
		    cgpu->drv->name, cgpu->device_id, cgpu->drv->dname);
	}
  ...
  for (i = 0; i < total_pools; i++)
      ...
      if (!pool->rpc_userpass)
	{
	  if (!pool->rpc_user || !pool->rpc_pass)
	    quit (1, "No login credentials supplied for %s",
		  get_pool_name (pool));
	  siz = strlen (pool->rpc_user) + strlen (pool->rpc_pass) + 2;
	  pool->rpc_userpass = (char *) malloc (siz);
	  if (!pool->rpc_userpass)
	    quit (1, "Failed to malloc userpass");
	  snprintf (pool->rpc_userpass, siz, "%s:%s",
