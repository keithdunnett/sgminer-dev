...
static bool on_backup_pool =
    false; // for simple connect strategy... flag if we're on a backup pool
...
bool opt_remoteconf_usecache =
    false; // use last downloaded copy of the config file when download fails
...
static unsigned long compare_pool_settings(struct pool *oldpool,
                                           struct pool *newpool);
...
void get_datestamp(char *f, size_t fsiz, struct timeval *tv) {
  ...
  const time_t tmp_time = tv->tv_sec;
  tm = localtime(&tmp_time);
  snprintf(f, fsiz, "[%d-%02d-%02d %02d:%02d:%02d]", tm->tm_year + 1900,
...
static void get_timestamp(char *f, size_t fsiz, struct timeval *tv) {
  ...
  const time_t tmp_time = tv->tv_sec;
  tm = localtime(&tmp_time);
  snprintf(f, fsiz, "[%02d:%02d:%02d]", tm->tm_hour, tm->tm_min, tm->tm_sec);
}
...
static void sharelog(const char *disposition, const struct work *work) {
  ...
  rv = snprintf(s, sizeof(s), "%lu,%s,%s,%s,%s%u,%u,%s,%s\n", t, disposition,
                target, pool->rpc_url, cgpu->drv->name, cgpu->device_id, thr_id,
                hash, data);
  free(target);
  free(hash);
  free(data);
...
static char *getwork_req =
    "{\"method\": \"getwork\", \"params\": [], \"id\":0}\n";

static char *gbt_req = "{\"id\": 0, \"method\": \"getblocktemplate\", "
                       "\"params\": [{\"capabilities\": [\"coinbasetxn\", "
...
struct pool *add_pool(void) {
  ...
  pools =
      (struct pool **)realloc(pools, sizeof(struct pool *) * (total_pools + 2));
...
static char *set_pool_kernelfile(const char *arg) {
  ...
  applog(LOG_DEBUG, "Setting pool %i algorithm kernel file to %s",
         pool->pool_no, arg);
...
static char *set_poolname_deprecated(char *arg) {
  applog(
      LOG_ERR,
      "Specifying pool name by --poolname is deprecated. Use --name instead.");
  set_pool_name(arg);
...
static char *set_schedtime(const char *arg, struct schedtime *st) {
  ...
  if (st->tm.tm_hour > 23 || st->tm.tm_min > 59 || st->tm.tm_hour < 0 ||
      st->tm.tm_min < 0)
...
static char *set_null(const char __maybe_unused *arg) { return NULL; }

char *set_difficulty_multiplier(char *arg) {
  applog(LOG_WARNING, "Option difficulty-multiplier is deprecated and will be "
                      "removed in v5.1. Pools that need this option should fix "
                      "their setup.");
  if (!(arg && arg[0]))
...
struct opt_table opt_config_table[] = {
    OPT_WITH_ARG(
        "--algorithm|--kernel|-k", set_default_algorithm, NULL, NULL,
        "Set mining algorithm and most common defaults, default: scrypt"),
    OPT_WITH_ARG("--api-allow", set_api_allow, NULL, NULL,
                 "Allow API access only to the given list of [G:]IP[/Prefix] "
                 "addresses[/subnets]"),
    OPT_WITH_ARG("--api-description", set_api_description, NULL, NULL,
                 "Description placed in the API status header, default: "
                 "sgminer version"),
    OPT_WITH_ARG("--api-groups", set_api_groups, NULL, NULL,
                 "API one letter groups G:cmd:cmd[,P:cmd:*...] defining the "
                 "cmds a groups can use"),
    OPT_WITHOUT_ARG("--api-listen", opt_set_bool, &opt_api_listen,
                    "Enable API, default: disabled"),
    OPT_WITHOUT_ARG("--api-mcast", opt_set_bool, &opt_api_mcast,
                    "Enable API Multicast listener, default: disabled"),
    OPT_WITH_ARG("--api-mcast-addr", set_api_mcast_addr, NULL, NULL,
                 "API Multicast listen address"),
    OPT_WITH_ARG("--api-mcast-code", set_api_mcast_code, NULL, NULL,
                 "Code expected in the API Multicast message, don't use '-'"),
    OPT_WITH_ARG(
        "--api-mcast-des", set_api_mcast_des, NULL, NULL,
        "Description appended to the API Multicast reply, default: ''"),
    OPT_WITH_ARG("--api-mcast-port", set_int_1_to_65535, opt_show_intval,
                 &opt_api_mcast_port, "API Multicast listen port"),
    OPT_WITHOUT_ARG("--api-network", opt_set_bool, &opt_api_network,
                    "Allow API (if enabled) to listen on/for any address, "
                    "default: only 127.0.0.1"),
    OPT_WITH_ARG("--api-port", set_int_1_to_65535, opt_show_intval,
                 &opt_api_port, "Port number of miner API"),
#ifdef HAVE_ADL
    OPT_WITHOUT_ARG("--auto-fan", opt_set_bool, &opt_autofan,
                    "Automatically adjust all GPU fan speeds to maintain a "
                    "target temperature"),
    OPT_WITHOUT_ARG("--auto-gpu", opt_set_bool, &opt_autoengine,
                    "Automatically adjust all GPU engine clock speeds to "
                    "maintain a target temperature"),
#endif
    OPT_WITHOUT_ARG(
        "--balance", set_balance, &pool_strategy,
        "Change multipool strategy from failover to even share balance"),
    OPT_WITHOUT_ARG("--blake-compact", opt_set_bool, &opt_blake_compact,
                    "Set SPH_COMPACT_BLAKE64 for Xn derived algorithms (Can "
                    "give better hashrate for some GPUs)"),
#ifdef HAVE_CURSES
    OPT_WITHOUT_ARG("--compact", opt_set_bool, &opt_compact,
                    "Use compact display without per device statistics"),
#endif
    OPT_WITHOUT_ARG("--debug|-D", enable_debug, &opt_debug,
                    "Enable debug output"),
    OPT_WITHOUT_ARG("--debug-log", opt_set_bool, &opt_debug,
                    "Enable debug logging when stderr is redirected to file"),
    OPT_WITH_ARG("--default-profile", set_default_profile, NULL, NULL,
                 "Set Default Profile"),
    OPT_WITH_ARG("--description", set_pool_description, NULL, NULL,
                 "Pool description"),
    OPT_WITH_ARG("--device|-d", set_default_devices, NULL, NULL,
                 "Select device to use, one value, range and/or comma "
                 "separated (e.g. 0-2,4) default: all"),
    OPT_WITHOUT_ARG(
        "--disable-rejecting", opt_set_bool, &opt_disable_pool,
        "Automatically disable pools that continually reject shares"),
    OPT_WITH_ARG("--expiry|-E", set_int_0_to_9999, opt_show_intval, &opt_expiry,
                 "Upper bound on how many seconds after getting work we "
                 "consider a share from it stale"),

    // event options
    OPT_WITH_ARG("--event-on", set_event_type, NULL, NULL,
                 "Select event type to perform task on"),
    OPT_WITH_ARG("--event-runcmd", set_event_runcmd, NULL, NULL,
                 "Command to perform on event"),
    OPT_WITH_ARG("--event-reboot", set_event_reboot, NULL, NULL,
                 "Reboot the system on event"),
    OPT_WITH_ARG("--event-reboot-delay", set_event_reboot_delay, NULL, NULL,
                 "Delay in seconds to wait before rebooting"),
    OPT_WITH_ARG("--event-quit", set_event_quit, NULL, NULL,
                 "Quit sgminer on event"),
    OPT_WITH_ARG("--event-quit-message", set_event_quit_message, NULL, NULL,
                 "Quit message when quitting sgminer on event"),

    OPT_WITHOUT_ARG(
        "--failover-only", opt_set_bool, &opt_fail_only,
        "Don't leak work to backup pools when primary pool is lagging"),
    OPT_WITH_ARG("--failover-switch-delay", set_int_1_to_65535, opt_show_intval,
                 &opt_fail_switch_delay,
                 "Delay in seconds before switching back to a failed pool"),
    OPT_WITHOUT_ARG(
        "--fix-protocol", opt_set_bool, &opt_fix_protocol,
        "Do not redirect to a different getwork protocol (eg. stratum)"),
    OPT_WITH_ARG(
        "--gpu-dyninterval", set_int_1_to_65535, opt_show_intval,
        &opt_dynamic_interval,
        "Set the refresh interval in ms for GPUs using dynamic intensity"),
    OPT_WITH_ARG("--gpu-platform", set_int_0_to_9999, opt_show_intval,
                 &opt_platform_id,
                 "Select OpenCL platform ID to use for GPU mining"),
#ifndef HAVE_ADL
    // gpu-threads can only be set per-card if ADL is available
    OPT_WITH_ARG("--gpu-threads|-g", set_int_1_to_10, opt_show_intval,
                 &opt_g_threads, "Number of threads per GPU (1 - 10)"),
#else
    OPT_WITH_ARG("--gpu-threads|-g", set_default_gpu_threads, NULL, NULL,
                 "Number of threads per GPU - one value or comma separated "
                 "list (e.g. 1,2,1)"),
    OPT_WITH_ARG("--gpu-engine", set_default_gpu_engine, NULL, NULL,
                 "GPU engine (over)clock range in Mhz - one value, range "
                 "and/or comma separated list (e.g. 850-900,900,750-850)"),
    OPT_WITH_ARG("--gpu-fan", set_default_gpu_fan, NULL, NULL,
                 "GPU fan percentage range - one value, range and/or comma "
                 "separated list (e.g. 0-85,85,65)"),
    OPT_WITH_ARG("--gpu-map", set_gpu_map, NULL, NULL,
                 "Map OpenCL to ADL device order manually, paired CSV (e.g. "
                 "1:0,2:1 maps OpenCL 1 to ADL 0, 2 to 1)"),
    OPT_WITH_ARG("--gpu-memclock", set_default_gpu_memclock, NULL, NULL,
                 "Set the GPU memory (over)clock in Mhz - one value for all or "
                 "separate by commas for per card"),
    OPT_WITH_ARG("--gpu-memdiff", set_gpu_memdiff, NULL, NULL,
                 "Set a fixed difference in clock speed between the GPU and "
                 "memory in auto-gpu mode"),
    OPT_WITH_ARG("--gpu-powertune", set_default_gpu_powertune, NULL, NULL,
                 "Set the GPU powertune percentage - one value for all or "
                 "separate by commas for per card"),
    OPT_WITHOUT_ARG("--gpu-reorder", opt_set_bool, &opt_reorder,
                    "Attempt to reorder GPU devices according to PCI Bus ID"),
    OPT_WITH_ARG("--gpu-vddc", set_default_gpu_vddc, NULL, NULL,
                 "Set the GPU voltage in Volts - one value for all or separate "
                 "by commas for per card"),
#endif
    OPT_WITH_ARG("--hamsi-expand-big", set_int_1_to_10, opt_show_intval,
                 &opt_hamsi_expand_big, "Set SPH_HAMSI_EXPAND_BIG for X13 "
                                        "derived algorithms (1 or 4 are "
                                        "common)"),
    OPT_WITHOUT_ARG("--hamsi-short", opt_set_bool, &opt_hamsi_short,
                    "Set SPH_HAMSI_SHORT for X13 derived algorithms (Can give "
                    "better hashrate for some GPUs)"),
    OPT_WITH_ARG(
        "--keccak-unroll", set_int_0_to_9999, opt_show_intval,
        &opt_keccak_unroll,
        "Set SPH_KECCAK_UNROLL for Xn derived algorithms (Default: 0)"),
    OPT_WITH_ARG(
        "--kernelfile", set_default_kernelfile, NULL, NULL,
        "Set the algorithm kernel source file (without file extension)."),
    OPT_WITH_ARG("--lookup-gap", set_default_lookup_gap, NULL, NULL,
                 "Set GPU lookup gap for scrypt mining, comma separated"),
    OPT_WITHOUT_ARG("--luffa-parallel", opt_set_bool, &opt_luffa_parallel,
                    "Set SPH_LUFFA_PARALLEL for Xn derived algorithms (Can "
                    "give better hashrate for some GPUs)"),
#ifdef HAVE_CURSES
    OPT_WITHOUT_ARG("--incognito", opt_set_bool, &opt_incognito,
                    "Do not display user name in status window"),
#endif
    OPT_WITHOUT_ARG(
        "--more-notices", opt_set_bool, &opt_morenotices,
    ...
    OPT_WITH_ARG("--xintensity|-X", set_default_xintensity, NULL, NULL,
                 ...
                 "), overridden --xintensity|-X and --rawintensity."),
    OPT_WITH_ARG("--rawintensity", set_default_rawintensity, NULL, NULL,
                 "Raw intensity of GPU scanning (" MIN_RAWINTENSITY_STR
                 " to " MAX_RAWINTENSITY_STR
                 "), overrides --intensity|-I and --xintensity|-X."),
    OPT_WITH_ARG("--kernel-path|-K", opt_set_charp, opt_show_charp,
                 &opt_kernel_path, "Specify a path to where kernel files are"),
    OPT_WITHOUT_ARG(
        "--load-balance", set_loadbalance, &pool_strategy,
        "Change multipool strategy from failover to quota based balance"),
    OPT_WITH_ARG("--log|-l", set_int_0_to_9999, opt_show_intval,
                 &opt_log_interval, "Interval in seconds between log output"),
    OPT_WITH_ARG("--log-file|-L", set_log_file, NULL, NULL,
                 "Log stderr to file"),
    OPT_WITHOUT_ARG("--log-show-date|-L", opt_set_bool, &opt_log_show_date,
                    "Show date on every log line"),
    OPT_WITHOUT_ARG("--lowmem", opt_set_bool, &opt_lowmem,
                    "Minimise caching of shares for low memory applications"),
#if defined(unix) || defined(__APPLE__)
    OPT_WITH_ARG("--monitor|-m", opt_set_charp, NULL, &opt_stderr_cmd,
                 "Use custom pipe cmd for output messages"),
#endif // defined(unix)
    OPT_WITH_ARG("--name|--pool-name", set_pool_name, NULL, NULL,
                 "Name of pool"),
    OPT_WITHOUT_ARG(
        "--net-delay", opt_set_bool, &opt_delaynet,
        "Impose small delays in networking to not overload slow routers"),
    OPT_WITH_ARG("--nfactor", set_default_nfactor, NULL, NULL,
                 "Override default scrypt N-factor parameter."),
#ifdef HAVE_ADL
    OPT_WITHOUT_ARG("--no-adl", opt_set_bool, &opt_noadl,
                    "Disable the ATI display library used for monitoring and "
                    "setting GPU parameters"),
#else
    OPT_WITHOUT_ARG("--no-adl", opt_set_bool, &opt_noadl, opt_hidden),
#endif
    OPT_WITHOUT_ARG("--no-pool-disable", opt_set_invbool, &opt_disable_pool,
                    opt_hidden),
    OPT_WITHOUT_ARG("--no-client-reconnect", opt_set_invbool,
                    &opt_disable_client_reconnect,
                    "Disable 'client.reconnect' stratum functionality"),
    OPT_WITHOUT_ARG("--no-restart", opt_set_invbool, &opt_restart,
                    "Do not attempt to restart GPUs that hang"),
    OPT_WITHOUT_ARG("--no-submit-stale", opt_set_invbool, &opt_submit_stale,
    ...
    OPT_WITH_ARG("--pass|--pool-pass|-p", set_pass, NULL, NULL,
                 "Password for bitcoin JSON-RPC server"),
    OPT_WITHOUT_ARG("--per-device-stats", opt_set_bool, &want_per_device_stats,
                    "Force verbose mode and output per-device statistics"),

    OPT_WITH_ARG(
        "--poolname", /* TODO: Backward compatibility, to be removed. */
        set_poolname_deprecated, NULL, NULL, opt_hidden),
    OPT_WITH_ARG("--pool-algorithm|--pool-kernel", set_pool_algorithm, NULL,
                 NULL, "Set algorithm for pool"),
    OPT_WITHOUT_ARG("--pool-backup", set_pool_backup, NULL,
                    "Mark this pool as a backup for simple connect strategy"),
    OPT_WITH_ARG("--pool-device", set_pool_devices, NULL, NULL,
                 "Select devices to use with pool, one value, range and/or "
                 "comma separated (e.g. 0-2,4) default: all"),
    OPT_WITH_ARG("--pool-kernelfile", set_pool_kernelfile, NULL, NULL,
                 "Set the pool's algorithm kernel source file (without file "
                 "extension)."),
    OPT_WITH_ARG("--pool-lookup-gap", set_pool_lookup_gap, NULL, NULL,
                 "Set Pool GPU lookup gap for scrypt mining, comma separated"),
#ifdef HAVE_ADL
    OPT_WITH_ARG("--pool-gpu-engine", set_pool_gpu_engine, NULL, NULL,
                 "Pool GPU engine (over)clock range in Mhz - one value, range "
                 "and/or comma separated list (e.g. 850-900,900,750-850)"),
    OPT_WITH_ARG("--pool-gpu-fan", set_pool_gpu_fan, NULL, NULL,
                 "GPU fan for pool"),
    OPT_WITH_ARG("--pool-gpu-memclock", set_pool_gpu_memclock, NULL, NULL,
                 "Set the Pool GPU memory (over)clock in Mhz - one value for "
                 "all or separate by commas for per card"),
    OPT_WITH_ARG("--pool-gpu-powertune", set_pool_gpu_powertune, NULL, NULL,
                 "Set the Pool GPU powertune percentage - one value for all or "
                 "separate by commas for per card"),
    OPT_WITH_ARG("--pool-gpu-threads", set_pool_gpu_threads, NULL, NULL,
                 "Number of threads per GPU for pool"),
    OPT_WITH_ARG("--pool-gpu-vddc", set_pool_gpu_vddc, NULL, NULL,
                 "Set the Pool GPU voltage in Volts - one value for all or "
                 "separate by commas for per card"),
#endif
    OPT_WITH_ARG("--pool-intensity", set_pool_intensity, NULL, NULL,
                 "Intensity of GPU scanning (pool-specific)"),
    OPT_WITH_ARG("--pool-nfactor", set_pool_nfactor, NULL, NULL,
                 "Set N-factor for pool"),
    OPT_WITH_ARG("--pool-profile", set_pool_profile, NULL, NULL,
                 "Profile to use with the pool"),
    OPT_WITH_ARG("--pool-rawintensity", set_pool_rawintensity, NULL, NULL,
                 "Raw intensity of GPU scanning (pool-specific)"),
    OPT_WITH_ARG(
        "--pool-shaders", set_pool_shaders, NULL, NULL,
        "Pool GPU shaders per card for tuning scrypt, comma separated"),
    OPT_WITH_ARG("--pool-thread-concurrency", set_pool_thread_concurrency, NULL,
                 NULL, "Set thread concurrency for pool"),
    OPT_WITH_ARG("--pool-worksize", set_pool_worksize, NULL, NULL,
                 "Override detected optimal worksize for pool - one value or "
                 "comma separated list"),
    OPT_WITH_ARG("--pool-xintensity", set_pool_xintensity, NULL, NULL,
                 "Shader based intensity of GPU scanning (pool-specific)"),

    OPT_WITH_ARG("--priority|--pool-priority", set_pool_priority, NULL, NULL,
                 "Pool priority"),

    OPT_WITH_ARG("--profile-algorithm|--profile-kernel", set_profile_algorithm,
                 NULL, NULL, "Set algorithm for profile"),
    OPT_WITH_ARG("--profile-device", set_profile_devices, NULL, NULL,
                 "Select devices to use with profile, one value, range and/or "
                 "comma separated (e.g. 0-2,4) default: all"),
    OPT_WITH_ARG("--profile-kernelfile", set_profile_kernelfile, NULL, NULL,
                 "Set the profile's algorithm kernel source file (without file "
                 "extension)."),
    OPT_WITH_ARG(
        "--profile-lookup-gap", set_profile_lookup_gap, NULL, NULL,
        "Set Profile GPU lookup gap for scrypt mining, comma separated"),
#ifdef HAVE_ADL
    OPT_WITH_ARG("--profile-gpu-engine", set_profile_gpu_engine, NULL, NULL,
                 "Profile GPU engine (over)clock range in Mhz - one value, "
                 "range and/or comma separated list (e.g. "
                 "850-900,900,750-850)"),
    OPT_WITH_ARG("--profile-gpu-fan", set_profile_gpu_fan, NULL, NULL,
                 "GPU fan for profile"),
    OPT_WITH_ARG("--profile-gpu-memclock", set_profile_gpu_memclock, NULL, NULL,
                 "Set the Profile GPU memory (over)clock in Mhz - one value "
                 "for all or separate by commas for per card"),
    OPT_WITH_ARG("--profile-gpu-powertune", set_profile_gpu_powertune, NULL,
                 NULL, "Set the Profile GPU powertune percentage - one value "
                       "for all or separate by commas for per card"),
    OPT_WITH_ARG("--profile-gpu-threads", set_profile_gpu_threads, NULL, NULL,
                 "Number of threads per GPU for profile"),
    OPT_WITH_ARG("--profile-gpu-vddc", set_profile_gpu_vddc, NULL, NULL,
                 "Set the Profile GPU voltage in Volts - one value for all or "
                 "separate by commas for per card"),
#endif
    OPT_WITH_ARG("--profile-intensity", set_profile_intensity, NULL, NULL,
                 "Intensity of GPU scanning (profile-specific)"),
    OPT_WITH_ARG("--profile-name", set_profile_name, NULL, NULL,
                 "Profile Name"),
    OPT_WITH_ARG("--profile-nfactor", set_profile_nfactor, NULL, NULL,
                 "Set N-factor for profile"),
    OPT_WITH_ARG("--profile-rawintensity", set_profile_rawintensity, NULL, NULL,
                 "Raw intensity of GPU scanning (profile-specific)"),
    OPT_WITH_ARG(
        "--profile-shaders", set_profile_shaders, NULL, NULL,
        "Profile GPU shaders per card for tuning scrypt, comma separated"),
    OPT_WITH_ARG("--profile-thread-concurrency", set_profile_thread_concurrency,
                 NULL, NULL, "Set thread concurrency for profile"),
    OPT_WITH_ARG("--profile-worksize", set_profile_worksize, NULL, NULL,
                 "Override detected optimal worksize for profile - one value "
                 "or comma separated list"),
    OPT_WITH_ARG("--profile-xintensity", set_profile_xintensity, NULL, NULL,
                 "Shader based intensity of GPU scanning (profile-specific)"),

    OPT_WITHOUT_ARG("--protocol-dump|-P", opt_set_bool, &opt_protocol,
                    "Verbose dump of protocol-level activities"),
    OPT_WITH_ARG("--queue|-Q", set_int_0_to_9999, opt_show_intval, &opt_queue,
                 "Minimum number of work items to have queued (0+)"),
    OPT_WITHOUT_ARG("--quiet|-q", opt_set_bool, &opt_quiet,
                    "Disable logging output, display status and errors"),
    OPT_WITH_ARG(
        "--quota|--pool-quota|-U", set_quota, NULL, NULL,
        "quota;URL combination for server with load-balance strategy quotas"),
    OPT_WITHOUT_ARG("--real-quiet", opt_set_bool, &opt_realquiet,
                    "Disable all output"),
    OPT_WITHOUT_ARG(
        "--remove-disabled", opt_set_bool, &opt_removedisabled,
        "Remove disabled devices entirely, as if they didn't exist"),
    OPT_WITH_ARG("--retries", set_null, NULL, NULL, opt_hidden),
    OPT_WITH_ARG("--retry-pause", set_null, NULL, NULL, opt_hidden),
    OPT_WITH_ARG("--rotate", set_rotate, opt_show_intval, &opt_rotate_period,
                 "Change multipool strategy from failover to regularly rotate "
                 "at N minutes"),
    OPT_WITHOUT_ARG(
        "--round-robin", set_rr, &pool_strategy,
        "Change multipool strategy from failover to round robin on failure"),
    OPT_WITH_ARG("--scan-time|-s", set_int_0_to_9999, opt_show_intval,
                 &opt_scantime,
                 "Upper bound on time spent scanning current work, in seconds"),
    OPT_WITH_ARG("--sched-start", set_schedtime, NULL, &schedstart,
                 "Set a time of day in HH:MM to start mining (a once off "
                 "without a stop time)"),
    OPT_WITH_ARG("--sched-stop", set_schedtime, NULL, &schedstop,
                 "Set a time of day in HH:MM to stop mining (will quit without "
                 "a start time)"),
    OPT_WITH_ARG("--shaders", set_default_shaders, NULL, NULL,
                 "GPU shaders per card for tuning scrypt, comma separated"),
    OPT_WITH_ARG("--sharelog", set_sharelog, NULL, NULL,
                 "Append share log to file"),
    OPT_WITH_ARG("--shares", opt_set_intval, NULL, &opt_shares,
                 "Quit after mining N shares (default: unlimited)"),
    OPT_WITH_ARG("--socks-proxy", opt_set_charp, NULL, &opt_socks_proxy,
                 "Set socks4 proxy (host:port)"),
    OPT_WITHOUT_ARG("--show-coindiff", opt_set_bool, &opt_show_coindiff,
                    "Show coin difficulty rather than hash value of a share"),
    OPT_WITH_ARG("--state|--pool-state", set_pool_state, NULL, NULL,
                 "Specify pool state at startup (default: enabled)"),
    OPT_WITH_ARG("--switcher-mode", set_switcher_mode, NULL, NULL,
...
#endif
#if defined(HAVE_LIBCURL) && defined(CURL_HAS_KEEPALIVE)
    OPT_WITH_ARG("--tcp-keepalive", set_int_0_to_9999, opt_show_intval,
                 &opt_tcp_keepalive, "TCP keepalive packet idle time"),
#else
    OPT_WITH_ARG("--tcp-keepalive", set_int_0_to_9999, opt_show_intval,
                 &opt_tcp_keepalive, opt_hidden),
#endif
#ifdef HAVE_ADL
    OPT_WITH_ARG("--temp-cutoff", set_temp_cutoff, opt_show_intval,
                 &opt_cutofftemp, "Temperature which a device will be "
                                  "automatically disabled at, one value or "
                                  "comma separated list"),
    OPT_WITH_ARG("--temp-hysteresis", set_int_1_to_10, opt_show_intval,
                 &opt_hysteresis, "Set how much the temperature can fluctuate "
                                  "outside limits when automanaging speeds"),
    OPT_WITH_ARG("--temp-overheat", set_temp_overheat, opt_show_intval,
                 &opt_overheattemp, "Temperature which a device will be "
                                    "throttled at while automanaging fan "
                                    "and/or GPU, one value or comma separated "
                                    "list"),
    OPT_WITH_ARG("--temp-target", set_temp_target, opt_show_intval,
                 &opt_targettemp, "Temperature which a device should stay at "
                                  "while automanaging fan and/or GPU, one "
                                  "value or comma separated list"),
#endif
#ifdef HAVE_CURSES
    OPT_WITHOUT_ARG("--text-only|-T", opt_set_invbool, &use_curses,
                    "Disable ncurses formatted screen output"),
#else
    OPT_WITHOUT_ARG("--text-only|-T", opt_set_invbool, &use_curses, opt_hidden),
#endif
    OPT_WITH_ARG(
        "--thread-concurrency", set_default_thread_concurrency, NULL, NULL,
        "Set GPU thread concurrency for scrypt mining, comma separated"),
    OPT_WITH_ARG("--url|--pool-url|-o", set_url, NULL, NULL,
                 "URL for bitcoin JSON-RPC server"),
    OPT_WITH_ARG("--user|--pool-user|-u", set_user, NULL, NULL,
                 "Username for bitcoin JSON-RPC server"),
    OPT_WITH_ARG("--vectors", set_vector, NULL, NULL, opt_hidden),
    /* All current kernels only support vectors=1 */
    /* "Override detected optimal vector (1, 2 or 4) - one value or comma
       separated list"), */
    OPT_WITHOUT_ARG("--verbose|-v", opt_set_bool, &opt_verbose,
                    "Log verbose output to stderr as well as status output"),
    OPT_WITH_ARG("--vote", set_int_1_to_65535, opt_show_intval, &opt_vote,
                 "Optional vote value for decred blocks"),
    OPT_WITH_ARG("--watchpool-refresh", set_int_1_to_65535, opt_show_intval,
                 &opt_watchpool_refresh,
                 "Interval in seconds to refresh pool status"),
    OPT_WITH_ARG("--worksize|-w", set_default_worksize, NULL, NULL,
                 "Override detected optimal worksize - one value or comma "
                 "separated list"),
    OPT_WITH_ARG("--userpass|--pool-userpass|-O", set_userpass, NULL, NULL,
                 "Username:Password pair for bitcoin JSON-RPC server"),
    OPT_WITHOUT_ARG("--worktime", opt_set_bool, &opt_worktime,
                    "Display extra work time debug information"),
    OPT_WITH_ARG("--pools", opt_set_bool, NULL, NULL, opt_hidden),
    OPT_WITH_ARG("--profiles", opt_set_bool, NULL, NULL, opt_hidden),
    OPT_WITH_ARG("--includes", opt_set_bool, NULL, NULL, opt_hidden),
    OPT_WITH_ARG("--events", opt_set_bool, NULL, NULL, opt_hidden),
    OPT_WITH_ARG("--difficulty-multiplier", set_difficulty_multiplier, NULL,
                 NULL, "(deprecated) Difficulty multiplier for jobs received "
                       "from stratum pools"),
    OPT_ENDTABLE};
...
static struct opt_table opt_cmdline_table[] = {
    OPT_WITH_ARG("--config|-c", load_config, NULL, NULL,
                 "Load a JSON-format configuration file\n"
                 "See example.conf for an example configuration."),
    OPT_WITH_ARG("--default-config", set_default_config, NULL, NULL,
                 "Specify the filename of the default config file\n"
                 "Loaded at start and used when saving without a name."),
    OPT_WITH_ARG(
        "--remote-config-retry", set_int_0_to_9999, opt_show_intval,
        &opt_remoteconf_retry,
        "Number of times to retry downloading remote config file. Default: 3"),
    OPT_WITH_ARG("--remote-config-wait", set_int_0_to_9999, opt_show_intval,
                 &opt_remoteconf_wait, "Time in seconds to wait between "
                                       "download retries of remote config "
                                       "files. Default: 10secs"),
    OPT_WITHOUT_ARG("--remote-config-usecache", opt_set_bool,
                    &opt_remoteconf_usecache, "Use cached copy of the remote "
                                              "config file when download "
                                              "fails. Default: No"),
    OPT_WITHOUT_ARG("--help|-h", opt_verusage_and_exit, NULL,
                    "Print this message"),
    OPT_WITHOUT_ARG("--ndevs|-n", display_devs, &nDevs,
                    "Display number of detected GPUs, OpenCL platform "
                    "information, and exit"),
    OPT_WITHOUT_ARG("--version|-V", opt_version_and_exit, packagename,
...
static double get_work_blockdiff(const struct work *work) {
  ...
  if (work->pool->algorithm.type == ALGO_NEOSCRYPT) {
    diff64 = bswap_64(
        ((uint64_t)(be32toh(*((uint32_t *)(work->data + 72))) & 0xFFFFFF00))
        << 8);
...
#define json_rpc_call(curl, curl_err_str, url, userpass, rpc_req, probe,       \
                      longpoll, rolltime, pool, share)                         \
  (NULL)
...
static int __total_staged(void) { return HASH_COUNT(staged_work); }
...
extern struct cgpu_info
    gpus[MAX_GPUDEVICES]; /* Maximum number apparently possible */

#ifdef HAVE_CURSES
static inline void unlock_curses(void) { mutex_unlock(&console_lock); }

static inline void lock_curses(void) { mutex_lock(&console_lock); }
...
static void suffix_string(uint64_t val, char *buf, size_t bufsiz,
                          int sigdigits) {
...
static void get_statline(char *buf, size_t bufsiz, struct cgpu_info *cgpu) {
  ...
  tailsprintf(buf, bufsiz,
              ...
              cgpu->diff_accepted, cgpu->diff_rejected, cgpu->hw_errors, wu);
...
static void curses_print_uptime(struct timeval *start_time) {
  ...
  hours = d.quot;
  d = div(d.rem, 60);
  cg_wprintw(statuswin, " - [%u day%c %02d:%02d:%02d]", days,
             (days == 1) ? ' ' : 's', hours, d.quot, d.rem);
}
...
static void curses_print_status(void) {
  ...
  cg_mvwprintw(statuswin, line, 0, PACKAGE " " CGMINER_VERSION " - Started: %s",
               datestamp);
  ...
  cg_mvwprintw(statuswin, ++line, 0,
               "ST: %d  SS: %d  NB: %d  LW: %d  GF: %d  RF: %d", total_staged(),
               total_stale, new_blocks, local_work, total_go, total_ro);
  wclrtoeol(statuswin);
  ...
  if (shared_strategy() && total_pools > 1) {
    ...
    cg_mvwprintw(statuswin, ++line, 0,
                 "Connected to %s (%s) diff %s as user %s", get_pool_name(pool),
                 pool->has_stratum ? "stratum"
                                   : (pool->has_gbt ? "GBT" : "longpoll"),
                 pool->diff, get_pool_user(pool));
  }
...
static void curses_print_devstatus(struct cgpu_info *cgpu, int count) {
  ...
  cg_wprintw(statuswin, "%s %*d: ", cgpu->drv->name, dev_width,
             cgpu->device_id);
  logline[0] = '\0';
  ...
  if ((cgpu->diff_accepted + cgpu->diff_rejected) > 0)
    reject_pct =
        (cgpu->diff_rejected / (cgpu->diff_accepted + cgpu->diff_rejected)) *
        100;
  ...
  cg_wprintw(statuswin, "/%6sh/s | R:%*.1f%% HW:%*d WU:%*.3f/m",
             displayed_hashes, drwidth, reject_pct, hwwidth, cgpu->hw_errors,
             wuwidth + 2, wu);
  logline[0] = '\0';
...
static void switch_logsize(bool __maybe_unused newdevs) {
  if (curses_active_locked()) {
    ...
    if (opt_compact) {
      ...
      logstart =
          devcursor + (opt_removedisabled ? most_devices : total_devices) + 1;
    }
...
void _wlog(const char *str) { wprintw(logwin, "%s", str); }
...
static void share_result(json_t *val, json_t *res, json_t *err,
                         const struct work *work, char *hashshow, bool resubmit,
                         char *worktime) {
  ...
  if (json_is_true(res) || (work->gbt && json_is_null(res)) ||
      (pool->algorithm.type == ALGO_CRYPTONIGHT && json_is_null(err))) {
    ...
    applog(LOG_DEBUG, "[THR%d] PROOF OF WORK RESULT: true (yay!!!)",
           work->thr_id);
    if (!QUIET) {
      if (total_pools > 1) {
        applog(LOG_NOTICE, "Accepted %s %s %d at %s %s%s", hashshow,
               cgpu->drv->name, cgpu->device_id, get_pool_name(pool),
               resubmit ? "(resubmit)" : "", worktime);
      } else {
        applog(LOG_DEBUG, "[THR%d] Accepted %s %s %d %s%s", work->thr_id,
               hashshow, cgpu->drv->name, cgpu->device_id,
               resubmit ? "(resubmit)" : "", worktime);
        applog(LOG_NOTICE, "Accepted %s %s %d %s%s", hashshow, cgpu->drv->name,
               cgpu->device_id, resubmit ? "(resubmit)" : "", worktime);
      }
    }
    ...
    applog(LOG_DEBUG, "[THR%d] PROOF OF WORK RESULT: false (booooo)",
           work->thr_id);
    if (!QUIET) {
      ...
      applog(LOG_NOTICE, "Rejected %s %s %d %s%s %s%s", hashshow,
             cgpu->drv->name, cgpu->device_id,
             (total_pools > 1) ? get_pool_name(pool) : "", reason,
             resubmit ? "(resubmit)" : "", worktime);
     ...
     */
    if (pool->seq_rejects > 10 && !work->stale && opt_disable_pool &&
        enabled_pools > 1) {
...
const char eth_getwork_rpc[] =
    "{\"jsonrpc\":\"2.0\",\"method\":\"eth_getWork\",\"params\":[],\"id\":1}";
const char eth_gethighestblock_rpc[] = "{\"jsonrpc\":\"2.0\",\"method\":\"eth_"
...
static void calc_diff(struct work *work, double known) {
  ...
  suffix_string_double(difficulty, work->pool->diff, sizeof(work->pool->diff),
                       0);
...
static void _copy_work(struct work *work, const struct work *base_work,
                       int noffset) {
  int id = work->id;
...
void __switch_pools(struct pool *selected, bool saveprio) {
  ...
  if (pool != last_pool && pool_strategy != POOL_LOADBALANCE &&
      pool_strategy != POOL_BALANCE) {
    // if the gpus have been initialized or first pool during startup, it's ok
    // to switch...
  ...
  if (startup) {
    ...
    // set initial pool number for restart_mining_threads to prevent mismatched
    // GPU settings
...
static bool test_work_current(struct work *work) {
  ...
  if (!block_exists(hexstr)) {
    ...
    if (opt_morenotices) {
      if (work->longpoll) {
        if (work->stratum) {
          applog(LOG_NOTICE, "Stratum from %s detected new block",
                 get_pool_name(pool));
        } else {
    ...
    if (work->longpoll) {
      ...
      if (shared_strategy() || work->pool == current_pool()) {
        if (opt_morenotices) {
          if (work->stratum)
            applog(LOG_NOTICE, "Stratum from %s requested work restart",
                   get_pool_name(pool));
          else
            applog(LOG_NOTICE, "%sLONGPOLL from %s requested work restart",
                   work->gbt ? "GBT " : "", get_pool_name(pool));
        }
        restart_threads();
      }
...
static void stage_work(struct work *work) {
  applog(LOG_DEBUG, "[THR%d] Pushing work from %s to hash queue", work->thr_id,
         get_pool_name(work->pool));
  work->work_block = work_block;
...
static void display_pool_summary(struct pool *pool) {
  ...
  if (curses_active_locked()) {
    ...
    efficiency = pool->getwork_requested
                     ? pool->accepted * 100.0 / pool->getwork_requested
                     : 0.0;
...
updated:
  for (i = 0; i < total_pools; i++) {
    ...
    wlogprint("%s Quota %d  Prio %d '%s'  User:%s\n",
              pool->idle ? "Dead" : "Alive", pool->quota, pool->prio,
              get_pool_name(pool), get_pool_user(pool));
...
retry:
  ...
  wlogprint("[D]ebug: %s\n[P]er-device: %s\n[Q]uiet: %s\n[V]erbose: %s\n"
            ...
            want_per_device_stats ? "on" : "off", opt_quiet ? "on" : "off",
            opt_verbose ? "on" : "off", opt_protocol ? "on" : "off",
            opt_worktime ? "on" : "off", opt_incognito ? "on" : "off",
...
retry:
  ...
  if (!strncasecmp(&input, "q", 1)) {
    ...
    snprintf(prompt, sizeof(prompt),
             "Config filename to write (Enter for default) [%s]", filename);
...
static void hashmeter(int thr_id, struct timeval *diff, uint64_t hashes_done) {
  ...
  if (thr) {
    ...
    applog(LOG_DEBUG, "[thread %d: %" PRIu64 " hashes, %.1f khash/sec]", thr_id,
           hashes_done, hashes_done / 1000 / secs);
  ...
  local_secs =
      (double)total_diff.tv_sec + ((double)total_diff.tv_usec / 1000000.0);
  ...
  snprintf(statusline, sizeof(statusline),
           ...
           want_per_device_stats ? "ALL " : "", opt_log_interval,
           displayed_rolling, displayed_hashes, total_diff_accepted,
           total_diff_rejected, hw_errors, total_diff1 / total_secs * 60);
...
static bool parse_stratum_response(struct pool *pool, char *s) {
  ...
  if ((json_is_null(id_val) || !id_val) &&
      (pool->algorithm.type != ALGO_ETHASH &&
  ...
  if (!sshare) {
    ...
    // for cryptonight, the result contains the "status" object which should =
    // "OK" on accept
    if (pool->algorithm.type == ALGO_CRYPTONIGHT) {
      json_t *res_id, *res_job;

      // check if the result contains an id... if so then we need to process as
      // first job, not share response
      ...
      if (json_is_null(err_val) &&
          !strcmp(json_string_value(json_object_get(res_val, "status")),
                  "OK")) {
        success = true;
      } else {
        char *ss;
    ...
    if (success) {
      applog(LOG_NOTICE, "Accepted untracked stratum share from %s",
             get_pool_name(pool));
      ...
      applog(LOG_NOTICE, "Rejected untracked stratum share from %s",
             get_pool_name(pool));
...
void clear_stratum_shares(struct pool *pool) {
  ...
  if (cleared) {
    applog(LOG_WARNING, "Lost %d shares due to stratum disconnect on %s",
           cleared, get_pool_name(pool));
...
static void *stratum_rthread(void *userdata) {
  ...
  while (42) {
    ...
    if (!sock_full(pool) && !cnx_needed(pool)) {
      applog(LOG_INFO, "Suspending stratum on %s", get_pool_name(pool));
     ...
     * as dead */
    if (!sock_full(pool) &&
        (sel_ret = select(pool->sock + 1, &rd, NULL, NULL, &timeout)) < 1) {
      applog(LOG_DEBUG, "Stratum select failed on %s with value %d",
             get_pool_name(pool), sel_ret);
      s = NULL;
    } else
      s = recv_line(pool);
    if (!s) {
      applog(LOG_NOTICE, "Stratum connection to %s interrupted",
             get_pool_name(pool));
      pool->getfail_occasions++;
      total_go++;
    ...
    if (!parse_method(pool, s) && !parse_stratum_response(pool, s))
      applog(
          LOG_INFO,
          "Unknown msg (stratum_rthread fails to parse_stratum_response): %s",
          s);
...
static void *stratum_sthread(void *userdata) {
  ...
  while (42) {
    char noncehex[12], nonce2hex[20], s[126 + 16 + 2048];
    struct stratum_share *sshare;
    ...
    if (unlikely(work->nonce2_len > 8)) {
      applog(LOG_ERR, "%s asking for inappropriately long nonce2 length %d",
             get_pool_name(pool), (int)work->nonce2_len);
    ...
    *((uint64_t *)nonce2) = htole64(work->nonce2);
    ...
    sshare->id = swork_id++;
    mutex_unlock(&sshare_lock);
    // Send the share to the pool connection. Vary stratum protocol by
    // algorithm.
    if (pool->algorithm.type == ALGO_ETHASH) {
      uint64_t tmp = bswap_64(work->Nonce);
      char *ASCIIMixHash = bin2hex(work->mixhash, 32);
      char *ASCIIPoWHash = bin2hex(work->data, 32);
      char *ASCIINonce = bin2hex(((const unsigned char *)&tmp), 8);
      snprintf(s, sizeof(s), "{\"id\": %d, \"method\": \"mining.submit\", "
                             "\"params\": [\"%s\", \"%s\", \"0x%s\", \"0x%s\", "
                             "\"0x%s\"]}",
               sshare->id, pool->rpc_user, work->job_id, ASCIINonce,
               ASCIIPoWHash, ASCIIMixHash);
      free(ASCIINonce);
      free(ASCIIMixHash);
      free(ASCIIPoWHash);
      ...
      snprintf(s, sizeof(s), "{\"method\": \"submit\", \"params\": {\"id\": "
                             "\"%s\", \"job_id\": \"%s\", \"nonce\": \"%s\", "
                             "\"result\": \"%s\"}, \"id\":%d}",
               pool->XMRAuthID, work->job_id, ASCIINonce, ASCIIResult,
               sshare->id);
      free(ASCIINonce);
      free(ASCIIResult);
    } else if (pool->algorithm.type == ALGO_DECRED && opt_vote) {
      snprintf(s, sizeof(s), "{\"params\": [\"%s\", \"%s\", \"%s\", \"%s\", "
                             "\"%s\", \"%04x\"], \"id\": %d, \"method\": "
                             "\"mining.submit\"}",
               pool->rpc_user, work->job_id, nonce2hex, work->ntime, noncehex,
               (opt_vote << 1) | 1, sshare->id);
    } else {
      snprintf(s, sizeof(s), "{\"params\": [\"%s\", \"%s\", \"%s\", \"%s\", "
                             "\"%s\"], \"id\": %d, \"method\": "
                             "\"mining.submit\"}",
    ...
    if (pool->algorithm.type == ALGO_CRYPTONIGHT) {
      applog(LOG_INFO, "Submitting share to %s", get_pool_name(pool));
    } else {
      applog(LOG_INFO, "Submitting share %08lx to %s",
             (long unsigned int)htole32(hash32[6]), get_pool_name(pool));
    }
    ...
    while (time(NULL) < sshare->sshare_time + 120) {
      ...
      if (!pool_tset(pool, &pool->submit_fail) && cnx_needed(pool)) {
        applog(LOG_WARNING, "%s stratum share submission failure",
               get_pool_name(pool));
        total_ro++;
        pool->remotefail_occasions++;
      }
      ...
      if (!sessionid_match) {
        applog(LOG_DEBUG,
               "No matching session id for resubmitting stratum share");
        break;
      }
...
static void init_stratum_threads(struct pool *pool) {
  have_longpoll = true;
  if (unlikely(pthread_create(&pool->stratum_sthread, NULL, stratum_sthread,
                              (void *)pool)))
    quit(1, "Failed to create stratum sthread");
  if (unlikely(pthread_create(&pool->stratum_rthread, NULL, stratum_rthread,
                              (void *)pool)))
    quit(1, "Failed to create stratum rthread");
  applog(LOG_DEBUG, "Stratum threads created.");
}
static void *longpoll_thread(void *userdata);

static bool stratum_works(struct pool *pool) {
  applog(LOG_INFO, "Testing %s stratum %s", get_pool_name(pool),
         pool->stratum_url);
  if (!extract_sockaddr(pool->stratum_url, &pool->sockaddr_url,
                        &pool->stratum_port))
    return false;
...
static bool pool_active(struct pool *pool, bool pinging) {
  ...
  if (pool->has_gbt)
    ...
    applog(LOG_INFO, "Testing %s in pool_active", get_pool_name(pool));
...
retry_stratum:
  ...
  if (pool->algorithm.type == ALGO_ETHASH) {
    ...
    if (pool->stratum_url && !opt_fix_protocol && stratum_works(pool)) {
      applog(LOG_NOTICE, "Switching %s to %s", get_pool_name(pool),
             pool->stratum_url);
      if (!pool->rpc_url)
  ...
  if (val) {
    ...
    if (rc) {
      applog(LOG_DEBUG, "Successfully retrieved and deciphered work from %s",
             get_pool_name(pool));
      work->pool = pool;
    ...
    if (pool->hdr_path) {
      ...
      if (strstr(hdr_path, "://")) {
        ...
        snprintf(pool->lp_url, siz, "%s%s%s", pool->rpc_url,
                 need_slash ? "/" : "", copy_start);
      }
    } else
      pool->lp_url = NULL;
    if (!pool->lp_started) {
      pool->lp_started = true;
      if (unlikely(pthread_create(&pool->longpoll_thread, NULL, longpoll_thread,
                                  (void *)pool)))
    ...
    if (!pinging)
      applog(LOG_WARNING, "%s slow/down or URL or credentials invalid",
             get_pool_name(pool));
  }
out:
  curl_easy_cleanup(curl);
...
void set_target(unsigned char *dest_target, double diff,
                double diff_multiplier2, const int thr_id) {
...
void set_target_neoscrypt(unsigned char *target, double diff,
  ...
  if (opt_debug) {
    ...
    applog(LOG_DEBUG, "[THR%d] Generated neoscrypt target 0x%s", thr_id,
           htarget);
    free(htarget);
  }
}
...
static void gen_stratum_work_eth(struct pool *pool, struct work *work) {
  if (pool->algorithm.type != ALGO_ETHASH)
    return;
  applog(LOG_DEBUG, "[THR%d] gen_stratum_work_eth() - algorithm = %s",
         work->thr_id, pool->algorithm.name);
  cg_rlock(&pool->data_lock);
  work->EpochNumber = pool->EpochNumber;
  ...
  work->drv_rolllimit = 0;
  cgtime(&work->tv_staged);
  applog(LOG_DEBUG, "gen_stratum_work_eth () done.");
}
...
static void gen_stratum_work_cn(struct pool *pool, struct work *work) {
  if (pool->algorithm.type != ALGO_CRYPTONIGHT)
    return;
  applog(LOG_DEBUG, "[THR%d] gen_stratum_work_cn() - algorithm = %s",
         work->thr_id, pool->algorithm.name);
  cg_rlock(&pool->data_lock);
  work->job_id = strdup(pool->swork.job_id);
...
static void gen_stratum_work(struct pool *pool, struct work *work) {
  ...
  applog(LOG_DEBUG, "[THR%d] gen_stratum_work() - algorithm = %s", work->thr_id,
         pool->algorithm.name);
  ...
  if (pool->algorithm.type == ALGO_NEOSCRYPT) {
    ...
    memcpy(work->data + 4 + 32, pool->coinbase,
           MIN((int)pool->swork.cb_len, 108));
    memcpy(work->data + 100, &vote, 2);
    for (i = 36; i < 45; i++)
  ...
  if (opt_debug) {
    ...
    if (pool->algorithm.type != ALGO_DECRED) {
      ...
      applog(LOG_DEBUG, "[THR%d] Generated stratum merkle %s", work->thr_id,
             merkle_hash);
      free(merkle_hash);
    }
    applog(LOG_DEBUG, "[THR%d] Generated stratum header %s", work->thr_id,
           header);
    applog(LOG_DEBUG, "[THR%d] Work job_id %s nonce2 %" PRIu64 " ntime %s",
           work->thr_id, work->job_id, work->nonce2, work->ntime);
    free(header);
  }
...
static void apply_initial_gpu_settings(struct pool *pool) {
  ...
  unsigned int needed_threads =
      0; // number of mining threads needed after we change devices
  ...
  /*
    ...
    if(!empty_string((opt = get_pool_setting(pool->devices,
    ((!empty_string(default_profile.devices))?default_profile.devices:"all")))))
    {
      set_devices((char *)opt);
    }

    //lookup gap
    if(!empty_string((opt = get_pool_setting(pool->lookup_gap,
    default_profile.lookup_gap))))
      set_lookup_gap((char *)opt);

    //set intensity
    if(!empty_string((opt = get_pool_setting(pool->rawintensity,
    default_profile.rawintensity)))) {
        set_rawintensity((char *)opt);
    }
    else if(!empty_string((opt = get_pool_setting(pool->xintensity,
    default_profile.xintensity)))) {
      set_xintensity((char *)opt);
    }
    else if(!empty_string((opt = get_pool_setting(pool->intensity,
    ((!empty_string(default_profile.intensity))?default_profile.intensity:"8")))))
    {
      set_intensity((char *)opt);
    }

    //shaders
    if(!empty_string((opt = get_pool_setting(pool->shaders,
    default_profile.shaders))))
      set_shaders((char *)opt);

    //thread-concurrency
    // neoscrypt - if not specified set TC to 0 so that TC will be calculated by
    intensity settings
    if (pool->algorithm.type == ALGO_NEOSCRYPT) {
      opt =
    ((empty_string(pool->thread_concurrency))?"0":get_pool_setting(pool->thread_concurrency,
    default_profile.thread_concurrency));
    }
    // otherwise use pool/profile setting or default to default profile setting
    else {
      opt = get_pool_setting(pool->thread_concurrency,
    default_profile.thread_concurrency);
    }
    ...
    //worksize
    if(!empty_string((opt = get_pool_setting(pool->worksize,
    default_profile.worksize))))
      set_worksize(opt);
  */
  ...
  #endif*/ rd_unlock(&mining_thr_lock);
  // enable/disable devices as needed
  enable_devices();
// recount the number of needed mining threads
#ifdef HAVE_ADL
  if (!empty_string((opt = get_pool_setting(pool->gpu_threads,
                                            default_profile.gpu_threads))))
...
static unsigned long compare_pool_settings(struct pool *oldpool,
  ...
  opt1 = ((oldpool) ? get_pool_setting(oldpool->devices,
                                       ((!empty_string(default_profile.devices))
                                            ? default_profile.devices
                                            : "all"))
                    : "");
  opt2 = get_pool_setting(newpool->devices,
                          ((!empty_string(default_profile.devices))
                               ? default_profile.devices
                               : "all"));
  ...
  // compare gpu threads
  opt1 = ((oldpool) ? get_pool_setting(oldpool->gpu_threads,
                                       default_profile.gpu_threads)
                    : "");
  ...
  // compare algorithm
  if ((oldpool && !cmp_algorithm(&oldpool->algorithm, &newpool->algorithm)) ||
      (!oldpool)) {
    options |= (SWITCHER_APPLY_ALGO | SWITCHER_SOFT_RESET);
  }
  // lookup gap
  opt1 = ((oldpool) ? get_pool_setting(oldpool->lookup_gap,
                                       default_profile.lookup_gap)
                    : "");
  ...
  // Intensity - First determine the intensity type that we are going to use
  // with the new pool
  unsigned int intoptions = 0;
  if (!empty_string(newpool->rawintensity)) {
    intoptions = SWITCHER_APPLY_RAWINT | SWITCHER_SOFT_RESET;
    opt1 = ((oldpool && !empty_string(oldpool->rawintensity))
                ? oldpool->rawintensity
                : "");
    opt2 =
        get_pool_setting(newpool->rawintensity, default_profile.rawintensity);
  } else if (!empty_string(newpool->xintensity)) {
    intoptions = SWITCHER_APPLY_XINT | SWITCHER_SOFT_RESET;
    opt1 =
        ((oldpool && !empty_string(oldpool->xintensity)) ? oldpool->xintensity
                                                         : "");
  ...
  // shaders
  opt1 =
      ((oldpool) ? get_pool_setting(oldpool->shaders, default_profile.shaders)
                 : "");
  ...
  if (newpool->algorithm.type == ALGO_NEOSCRYPT) {
    opt2 = ((empty_string(newpool->thread_concurrency))
                ? "0"
                : get_pool_setting(newpool->thread_concurrency,
                                   default_profile.thread_concurrency));
  }
  // otherwise use pool/profile setting or default to default profile setting
  else {
    opt2 = get_pool_setting(newpool->thread_concurrency,
                            default_profile.thread_concurrency);
  }

  opt1 = ((oldpool) ? get_pool_setting(oldpool->thread_concurrency,
                                       default_profile.thread_concurrency)
                    : "");
  ...
  // worksize
  opt1 =
      ((oldpool) ? get_pool_setting(oldpool->worksize, default_profile.worksize)
                 : "");
...
#ifdef HAVE_ADL
  // gpu-engine
  opt1 = ((oldpool) ? get_pool_setting(oldpool->gpu_engine,
                                       default_profile.gpu_engine)
                    : "");
  ...
  // gpu-memclock
  opt1 = ((oldpool) ? get_pool_setting(oldpool->gpu_memclock,
                                       default_profile.gpu_memclock)
                    : "");
  ...
  // GPU fans
  opt1 =
      ((oldpool) ? get_pool_setting(oldpool->gpu_fan, default_profile.gpu_fan)
                 : "");
  ...
  // GPU powertune
  opt1 = ((oldpool) ? get_pool_setting(oldpool->gpu_powertune,
                                       default_profile.gpu_powertune)
                    : "");
  opt2 =
      get_pool_setting(newpool->gpu_powertune, default_profile.gpu_powertune);
  if (strcasecmp(opt1, opt2) != 0 && !empty_string(opt2)) {
    options |= SWITCHER_APPLY_GPU_POWERTUNE;
  }
  // GPU vddc
  opt1 =
      ((oldpool) ? get_pool_setting(oldpool->gpu_vddc, default_profile.gpu_vddc)
                 : "");
...
static void apply_switcher_options(unsigned long options, struct pool *pool) {
  ...
  if (opt_isset(options, SWITCHER_APPLY_DEVICE)) {
    ...
    // assign pool devices if any
    if (!empty_string(
            (opt = get_pool_setting(pool->devices,
                                    ((!empty_string(default_profile.devices))
                                         ? default_profile.devices
                                         : "all"))))) {
      set_devices((char *)opt);
  ...
  if (opt_isset(options, SWITCHER_APPLY_RAWINT)) {
    applog(LOG_DEBUG, "Switching to rawintensity: pool = %s, default = %s",
  ...
  // xintensity
    ...
    applog(LOG_DEBUG, "Switching to xintensity: pool = %s, default = %s",
  ...
  // intensity
    ...
    applog(LOG_DEBUG, "Switching to intensity: pool = %s, default = %s",
  ...
  if (opt_isset(options, SWITCHER_APPLY_SHADER)) {
    if (!empty_string(
            (opt = get_pool_setting(pool->shaders, default_profile.shaders))))
      set_shaders((char *)opt);
  }
  // thread-concurrency
  if (opt_isset(options, SWITCHER_APPLY_TC)) {
    ...
    // intensity settings
    if (pool->algorithm.type == ALGO_NEOSCRYPT) {
      opt = ((empty_string(pool->thread_concurrency))
                 ? "0"
                 : get_pool_setting(pool->thread_concurrency,
                                    default_profile.thread_concurrency));
    }
    // otherwise use pool/profile setting or default to default profile setting
    else {
  ...
  if (opt_isset(options, SWITCHER_APPLY_WORKSIZE)) {
    if (!empty_string(
            (opt = get_pool_setting(pool->worksize, default_profile.worksize))))
      set_worksize(opt);
  }
#ifdef HAVE_ADL
  // GPU clock
  if (opt_isset(options, SWITCHER_APPLY_GPU_ENGINE)) {
    if (!empty_string((opt = get_pool_setting(pool->gpu_engine,
                                              default_profile.gpu_engine))))
      set_gpu_engine((char *)opt);
  }
  // GPU memory clock
  if (opt_isset(options, SWITCHER_APPLY_GPU_MEMCLOCK)) {
    if (!empty_string((opt = get_pool_setting(pool->gpu_memclock,
                                              default_profile.gpu_memclock))))
      set_gpu_memclock((char *)opt);
  }
  // GPU fans
  if (opt_isset(options, SWITCHER_APPLY_GPU_FAN)) {
    if (!empty_string(
            (opt = get_pool_setting(pool->gpu_fan, default_profile.gpu_fan))))
      set_gpu_fan((char *)opt);
  }
  // GPU powertune
  if (opt_isset(options, SWITCHER_APPLY_GPU_POWERTUNE)) {
    if (!empty_string((opt = get_pool_setting(pool->gpu_powertune,
                                              default_profile.gpu_powertune))))
      set_gpu_powertune((char *)opt);
  }
  // GPU vddc
  if (opt_isset(options, SWITCHER_APPLY_GPU_VDDC)) {
    if (!empty_string(
            (opt = get_pool_setting(pool->gpu_vddc, default_profile.gpu_vddc))))
      set_gpu_vddc((char *)opt);
  }
  // apply gpu settings
  for (i = 0; i < nDevs; ++i) {
...
get_work_prepare_thread(struct thr_info *mythr, struct work *work) {
  int i;
  ...
  if (algo_switch_n == 0) {
    ...
    if (opt_switchmode == SWITCH_ALGO) {
      ...
      if (!cmp_algorithm(&work->pool->algorithm, &mythr->cgpu->algorithm))
        pool_switch_options =
            compare_pool_settings(pools[mythr->pool_no], work->pool);
    }
    // switcher mode - switch on pool change
    else if (opt_switchmode == SWITCH_POOL) {
      if (work->pool->pool_no != mythr->pool_no) {
        if ((pool_switch_options = compare_pool_settings(pools[mythr->pool_no],
                                                         work->pool)) == 0) {
          applog(LOG_DEBUG, "No settings change from pool %s...",
                 isnull(get_pool_name(work->pool), ""));
          rd_lock(&mining_thr_lock);
  ...
  // get the number of active threads to know when to switch... if we only check
  // total threads, we may wait for ever on a disabled GPU
  int active_threads = 0;
  rd_lock(&mining_thr_lock);
  for (i = 0; i < mining_threads; i++) {
    struct cgpu_info *cgpu = mining_thr[i]->cgpu;
    // dont count dead/sick GPU threads or we may wait for ever also...
    if (cgpu->deven != DEV_DISABLED && cgpu->status != LIFE_SICK &&
  ...
  if (algo_switch_n >= active_threads) {
    const char *opt;
    applog(LOG_DEBUG, "Applying pool settings for %s...",
           isnull(get_pool_name(work->pool), ""));
    rd_lock(&mining_thr_lock);
    ...
    /*if(opt_isset(pool_switch_options, SWITCHER_APPLY_DEVICE))
       ...
       //assign pool devices if any
       if(!empty_string((opt = get_pool_setting(work->pool->devices,
       ((!empty_string(default_profile.devices))?default_profile.devices:"all")))))
       {
       set_devices((char *)opt);
       }
       ...
       {
       if(!empty_string((opt = get_pool_setting(work->pool->lookup_gap,
       default_profile.lookup_gap))))
       set_lookup_gap((char *)opt);
       }
       ...
       {
       applog(LOG_DEBUG, "Switching to rawintensity: pool = %s, default = %s",
       work->pool->rawintensity, default_profile.rawintensity);
       opt = get_pool_setting(work->pool->rawintensity,
       default_profile.rawintensity);
       applog(LOG_DEBUG, "rawintensity -> %s", opt);
       set_rawintensity(opt);
       }
       ...
       {
       applog(LOG_DEBUG, "Switching to xintensity: pool = %s, default = %s",
       work->pool->xintensity, default_profile.xintensity);
       opt = get_pool_setting(work->pool->xintensity,
       default_profile.xintensity);
       applog(LOG_DEBUG, "xintensity -> %s", opt);
       set_xintensity(opt);
       }
       ...
       {
       applog(LOG_DEBUG, "Switching to intensity: pool = %s, default = %s",
       work->pool->intensity, default_profile.intensity);
       ...
       {
       if(!empty_string((opt = get_pool_setting(work->pool->shaders,
       default_profile.shaders))))
       set_shaders((char *)opt);
       }
       ...
       by intensity settings
       if (work->pool->algorithm.type == ALGO_NEOSCRYPT) {
       opt =
       ((empty_string(work->pool->thread_concurrency))?"0":get_pool_setting(work->pool->thread_concurrency,
       default_profile.thread_concurrency));
       }
       // otherwise use pool/profile setting or default to default profile
       setting
       else {
       opt = get_pool_setting(work->pool->thread_concurrency,
       default_profile.thread_concurrency);
       }
       ...
       {
       if(!empty_string((opt = get_pool_setting(work->pool->worksize,
       default_profile.worksize))))
       set_worksize(opt);
       }
       ...
       {
       if(!empty_string((opt = get_pool_setting(work->pool->gpu_engine,
       default_profile.gpu_engine))))
       set_gpu_engine((char *)opt);
       }
       ...
       {
       if(!empty_string((opt = get_pool_setting(work->pool->gpu_memclock,
       default_profile.gpu_memclock))))
       set_gpu_memclock((char *)opt);
       }
       ...
       {
       if(!empty_string((opt = get_pool_setting(work->pool->gpu_fan,
       default_profile.gpu_fan))))
       set_gpu_fan((char *)opt);
       }
       ...
       {
       if(!empty_string((opt = get_pool_setting(work->pool->gpu_powertune,
       default_profile.gpu_powertune))))
       set_gpu_powertune((char *)opt);
       }
       ...
       {
       if(!empty_string((opt = get_pool_setting(work->pool->gpu_vddc,
       default_profile.gpu_vddc))))
       set_gpu_vddc((char *)opt);
       }
    ...
    if (opt_isset(pool_switch_options, SWITCHER_HARD_RESET)) {
      ...
      if (opt_isset(pool_switch_options, SWITCHER_APPLY_GT)) {
        if (!empty_string(
                (opt = get_pool_setting(work->pool->gpu_threads,
                                        default_profile.gpu_threads))))
          set_gpu_threads(opt);
      }
...
#endif
      if (unlikely(pthread_create(&restart_thr, NULL,
                                  restart_mining_threads_thread,
                                  (void *)(intptr_t)n_threads)))
  ...
  pthread_cleanup_push(mutex_unlock_cleanup_handler,
                       (void *)&algo_switch_wait_lock);
...
struct work *get_work(struct thr_info *thr, const int thr_id) {
   ...
   * device failures. */
  if (diff_t > 0) {
    applog(LOG_DEBUG, "[THR%d] Get work blocked for %d seconds", thr_id,
           (int)diff_t);
...
static void submit_work_async(struct work *work) {
  ...
  if (stale_work(work, true)) {
    if (opt_submit_stale)
      applog(LOG_NOTICE, "%s stale share detected, submitting (user)",
             get_pool_name(pool));
    else if (pool->submit_old)
      applog(LOG_NOTICE, "%s stale share detected, submitting (pool)",
             get_pool_name(pool));
    else {
      applog(LOG_NOTICE, "%s stale share detected, discarding",
             get_pool_name(pool));
...
void inc_hw_errors(struct thr_info *thr) {
  applog(LOG_INFO, "[THR%d] %s%d: invalid nonce - HW error", thr->id,
...
static void rebuild_nonce(struct work *work, uint32_t nonce) {
  ...
  if (work->pool->algorithm.type == ALGO_ETHASH) {
    uint64_t *work_nonce = (uint64_t *)(work->data + nonce_pos);
    *work_nonce = (uint64_t)htole32(nonce);
    //      redundant ?see below
    //      } else if (work->pool->algorithm.type == ALGO_CRYPTONIGHT)
    //              uint32_t *work_nonce = (uint32_t *) (work->data +
    //              nonce_pos);
...
static inline bool abandon_work(struct work *work, struct timeval *wdiff,
                                uint64_t hashes) {
  if (wdiff->tv_sec > opt_scantime || work->blk.nonce >= MAXTHREADS - hashes ||
      hashes >= 0xfffffffe || stale_work(work, false))
    return true;
  return false;
}
...
static void hash_sole_work(struct thr_info *mythr) {
  ...
  while (likely(!cgpu->shutdown)) {
    ...
    if (wu > 30 && drv->working_diff < drv->max_diff &&
      ...
      drv->working_diff++;
      applog(LOG_DEBUG, "Driver %s working diff changed to %.0f", drv->dname,
             drv->working_diff);
      work->device_diff = MIN(drv->working_diff, work->work_difficulty);
    ...
    if (work->pool->algorithm.type == ALGO_NEOSCRYPT)
      ...
      *(uint64_t *)(work->device_target + 24) = bits64 / work->device_diff;
      ...
      set_target(work->device_target, work->device_diff,
                 work->pool->algorithm.diff_multiplier2, work->thr_id);
    do {
      cgtime(&tv_start);
      ...
      if (unlikely((long)sdiff.tv_sec < cycle)) {
        ...
        max_nonce = max_nonce * 0x400 / (((cycle * 1000000) + sdiff.tv_usec) /
                                         (cycle * 1000000 / 0x400));
...
static void *watchpool_thread(void __maybe_unused *userdata) {
  ...
  while (42) {
    ...
    for (i = 0; i < total_pools; ++i) {
      ...
      // if this pool is alive and the priority is greater (lower) than
      ...
      if (!pool->idle && pool->prio < cp_prio()) {
        ...
        if (pool_strategy == POOL_FAILOVER &&
          ...
          applog(LOG_WARNING, "%s stable for %d seconds", get_pool_name(pool),
                 opt_fail_switch_delay);
          switch_pools(NULL);
        }
      }
    ...
    // if the pool stategy is rotation and we have been over the rotate delay,
    // switch pool
    if (pool_strategy == POOL_ROTATE &&
        now.tv_sec - rotate_tv.tv_sec > 60 * opt_rotate_period) {
      cgtime(&rotate_tv);
      switch_pools(NULL);
    }
    ...
    for (i = 0; i < total_devices; ++i) {
      ...
      if (opt_debug && cgpu->has_adl) {
        ...
        if (gpu_stats(gpu, &temp, &engineclock, &memclock, &vddc, &activity,
          ...
          applog(LOG_DEBUG, "%.1f C  F: %d%%(%dRPM)  E: %dMHz  M: %dMhz  V: "
                            "%.3fV  A: %d%%  P: %d%%",
                 temp, fanpercent, fanspeed, engineclock, memclock, vddc,
                 activity, powertune);
      }
#endif
      ...
      if (cgpu->status != LIFE_WELL &&
          (now.tv_sec - thr->last.tv_sec < WATCHDOG_SICK_TIME)) {
        if (cgpu->status != LIFE_INIT)
          applog(LOG_ERR, "%s: Recovered, declaring WELL!", dev_str);
        cgpu->status = LIFE_WELL;
        cgpu->device_last_well = time(NULL);
      } else if (cgpu->status == LIFE_WELL &&
                 (now.tv_sec - thr->last.tv_sec > WATCHDOG_SICK_TIME)) {
        thr->rolling = cgpu->rolling = 0;
        cgpu->status = LIFE_SICK;
        applog(LOG_ERR, "%s: Idle for more than 2 minutes, declaring SICK!",
               dev_str);
        cgtime(&thr->sick);
        ...
        if (adl_active && cgpu->has_adl && gpu_activity(gpu) > 50) {
          ...
          reinit_device(cgpu);
                 ...
                 (now.tv_sec - thr->last.tv_sec > WATCHDOG_DEAD_TIME)) {
        cgpu->status = LIFE_DEAD;
        applog(LOG_ERR,
...
static void noop_get_statline(char __maybe_unused *buf,
                              size_t __maybe_unused bufsiz,
                              struct cgpu_info __maybe_unused *cgpu);
void blank_get_statline_before(char *buf, size_t bufsiz,
                               struct cgpu_info __maybe_unused *cgpu);
void print_summary(void) {
  ...
  applog(LOG_WARNING, "Average hashrate: %.1f %shash/s", displayed_hashes,
         mhash_base ? "Mega" : "Kilo");
  ...
  applog(LOG_WARNING, "Work Utility (diff1 shares solved / min): %.2f/min\n",
         work_util);
  applog(LOG_WARNING, "Stale submissions discarded due to new blocks: %d",
         total_stale);
  ...
  if (total_pools > 1) {
    for (i = 0; i < total_pools; i++) {
      ...
      if (pool->solved)
        applog(LOG_WARNING, "SOLVED %d BLOCK%s!", pool->solved,
               pool->solved > 1 ? "S" : "");
      applog(LOG_WARNING, " Share submissions: %d",
             pool->accepted + pool->rejected);
      applog(LOG_WARNING, " Accepted shares: %d", pool->accepted);
      applog(LOG_WARNING, " Rejected shares: %d", pool->rejected);
      applog(LOG_WARNING, " Accepted difficulty shares: %1.f",
             pool->diff_accepted);
      applog(LOG_WARNING, " Rejected difficulty shares: %1.f",
             pool->diff_rejected);
      if (pool->accepted || pool->rejected)
        applog(LOG_WARNING, " Reject ratio: %.1f%%",
               (double)(pool->rejected * 100) /
      ...
      applog(LOG_WARNING, " Stale submissions discarded due to new blocks: %d",
             pool->stale_shares);
      applog(LOG_WARNING, " Unable to get work from server occasions: %d",
             pool->getfail_occasions);
  ...
  if (opt_shares) {
    ...
    if (opt_shares > total_diff_accepted)
      applog(LOG_WARNING, "WARNING - Mined only %.0f shares of %d requested.",
             total_diff_accepted, opt_shares);
  }
  applog(LOG_WARNING, " ");
  fflush(stderr);
...
static void *test_pool_thread(void *arg) {
  ...
  if (pool_active(pool, false)) {
    ...
    if (unlikely(first_pool))
      applog(LOG_NOTICE, "Switching to %s - first alive pool",
             get_pool_name(pool));
    pool_resus(pool);
    switch_pools(NULL);
  } else {
...
bool add_pool_details(struct pool *pool, bool live, char *url, char *user,
                      char *pass, char *name, char *desc, char *profile,
                      char *algo) {
  size_t siz;
...
static bool input_pool(bool live) {
  ...
  ret =
      add_pool_details(pool, live, url, user, pass, name, desc, profile, algo);
out:
  immedok(logwin, false);
  if (!ret) {
    if (url)
...
static void fork_monitor() {
  ...
  if (0 == forkpid) {
    ...
    if (r < 0) {
      perror(
          "close - in child, failed to close read end of  pipe for --monitor");
      exit(1);
    }
...
void
 ...
 blank_get_statline_before(char *buf, size_t bufsiz, struct cgpu_info __maybe_unused * cgpu)
{
  tailsprintf(buf, bufsiz, "               | ");
}
static void

noop_get_statline(char __maybe_unused *buf, size_t __maybe_unused bufsiz,
                  struct cgpu_info __maybe_unused *cgpu) {}
...
static bool noop_prepare_work(struct thr_info __maybe_unused *thr,
                              struct work __maybe_unused *work) {
  return true;
}
...
noop_thread_shutdown(struct thr_info __maybe_unused *thr) {}
static void

noop_thread_enable(struct thr_info __maybe_unused *thr) {}
static void
...
bool add_cgpu(struct cgpu_info *cgpu) {
  ...
  devices = (struct cgpu_info **)realloc(devices, sizeof(struct cgpu_info *) *
                                                      (total_devices + 2));
...
restart_mining_threads(unsigned int new_n_threads) {
  struct thr_info *thr;
  unsigned int i, j, k;
  ...
  for (i = 0; i < total_devices; ++i) {
    ...
    cgpu->thr = (struct thr_info **)malloc(sizeof(struct thr_info *) *
                                           (cgpu->threads + 1));
    ...
    for (j = 0; j < cgpu->threads; ++j, ++k) {
      ...
      applog(LOG_DEBUG, "Thread %d set pool = %d (%s)", k, thr->pool_no,
             isnull(get_pool_name(pools[thr->pool_no]), ""));
      thr->cgpu = cgpu;
      thr->device_thread = j;
      cgtime(&thr->last);
      cgpu->thr[j] = thr;
...
int main(int argc, char *argv[]) {
  ...
  if (opt_display_devs) {
    ...
    for (i = 0; i < total_devices; ++i) {
      struct cgpu_info *cgpu = devices[i];
      if (cgpu->name)
        applog(LOG_ERR, " %2d. %s %d: %s (driver: %s)", i, cgpu->drv->name,
               cgpu->device_id, cgpu->name, cgpu->drv->dname);
      else
        applog(LOG_ERR, " %2d. %s %d (driver: %s)", i, cgpu->drv->name,
               cgpu->device_id, cgpu->drv->dname);
    }
  ...
  for (i = 0; i < total_pools; i++) {
    ...
    if (!pool->rpc_userpass) {
      ...
      if (!pool->rpc_userpass)
        quit(1, "Failed to malloc userpass");
      snprintf(pool->rpc_userpass, siz, "%s:%s", pool->rpc_user,
               pool->rpc_pass);
    }
  }
  ...
  do {
    ...
    if (!pools_active) {
      ...
      applog(LOG_ERR, "Please check the details from the list below of the "
                      "servers you have input");
      applog(LOG_ERR, "Most likely you have input the wrong URL, forgotten to "
                      "add a port, or have not set up workers");
      for (i = 0; i < total_pools; i++) {
        struct pool *pool;
        pool = pools[i];
        applog(LOG_WARNING, "Pool: %d  URL: %s  User: %s  Password: %s", i,
               pool->rpc_url, pool->rpc_user, pool->rpc_pass);
      }
#ifdef HAVE_CURSES
      if (use_curses) {
        halfdelay(150);
  ...
  if (total_control_threads != 8)
    quit(1, "incorrect total_control_threads (%d) should be 8",
  ...
  retry:
    ...
    if (!get_upstream_work(work, ce->curl, ce->curl_err_str)) {
      applog(LOG_DEBUG, "%s json_rpc_call failed on get work, retrying in 5s",
             get_pool_name(pool));
