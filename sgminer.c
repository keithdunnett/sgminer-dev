...
struct opt_table opt_config_table[] = {
	...
	OPT_WITH_ARG("--rawintensity",
		     ...
		     " to " MAX_RAWINTENSITY_STR "), overrides --intensity|-I and --xintensity|-X."),
...
static void update_gbt(struct pool *pool)
{
	...
	if (val) {
		...
		if (rc) {
			applog(LOG_DEBUG,
			       "Successfully retrieved and updated GBT from %s", get_pool_name(pool));
			cgtime(&pool->tv_idle);
...
static double get_work_blockdiff(const struct work *work)
{
	...
	if (work->pool->algorithm.type == ALGO_NEOSCRYPT) {
		diff64 = bswap_64(((uint64_t)
				   (be32toh(*((uint32_t *) (work->data + 72))) & 0xFFFFFF00)) << 8);
	...
	if (shared_strategy() && total_pools > 1) {
		...
		cg_mvwprintw(statuswin, ++line, 0,
			     ...
			     pool->has_stratum ? "stratum" : (pool->has_gbt ? "GBT" :
							      "longpoll"), pool->diff, get_pool_user(pool));
	}
...
static void curses_print_devstatus(struct cgpu_info *cgpu, int count)
{
	...
	cg_wprintw(statuswin, "/%6sh/s | R:%*.1f%% HW:%*d WU:%*.3f/m",
		   displayed_hashes, drwidth, reject_pct, hwwidth, cgpu->hw_errors, wuwidth + 2, wu);
	logline[0] = '\0';
...
share_result(json_t * val, json_t * res, json_t * err,
	     const struct work *work, char *hashshow, bool resubmit, char *worktime)
{
	...
	if (json_is_true(res) || (work->gbt && json_is_null(res))
		...
		if (!QUIET) {
			if (total_pools > 1) {
				applog(LOG_NOTICE,
				       ...
				       get_pool_name(pool), resubmit ? "(resubmit)" : "", worktime);
			} else {
				applog(LOG_DEBUG,
		...
		if (opt_shares && total_diff_accepted >= opt_shares) {
			applog(LOG_WARNING,
			       "Successfully mined %d accepted shares as requested and exiting.", opt_shares);
			kill_work();
			return;
		}
		...
		if (unlikely(pool->state == POOL_REJECTING)) {
			applog(LOG_WARNING,
			       "Rejecting %s now accepting shares, re-enabling!", get_pool_name(pool));
			enable_pool(pool);
			switch_pools(NULL);
		}
...
static void show_hash(struct work *work, char *hashshow)
{
	...
	if (opt_show_coindiff) {
		snprintf(hashshow, 64, "Coin %.0f Diff %s/%s%s",
			 get_work_blockdiff(work), diffdisp, wdiffdisp, work->block ? " BLOCK!" : "");
	} else {
	...
	if (work->pool->algorithm.type == ALGO_ETHASH) {
		...
		val =
		    json_rpc_call(curl, curl_err_str, pool->rpc_url,
				  pool->rpc_userpass, s, false, false, &rolltime, pool, true);
		...
		if (unlikely(!val)) {
			...
			if (!pool_tset(pool, &pool->submit_fail)) {
				...
				applog(LOG_WARNING,
				       "%s communication failure, caching submissions", get_pool_name(pool));
			}
			cgsleep_ms(5000);
			goto out;
			...
			if (opt_worktime) {
				...
				if (work->clone) {
					snprintf(workclone, sizeof(workclone),
						 "C:%1.3f", tdiff((struct timeval *)
								  &(work->tv_cloned), (struct timeval *)
								  &(work->tv_getwork_reply)));
				} else
					strcpy(workclone, "O");
...
static bool get_upstream_work(struct work *work, CURL * curl, char *curl_err_str)
{
	...
	if (likely(val)) {
		rc = (pool->algorithm.type ==
		      ALGO_ETHASH) ? work_decode_eth(pool, work, val, ethval2) : work_decode(pool, work, val);
		if (unlikely(!rc))
	...
	pool_stats->getwork_wait_rolling +=
	    ((double)tv_elapsed.tv_sec + ((double)tv_elapsed.tv_usec / 1000000)) * 0.63;
...
static void *submit_work_thread(void *userdata)
{
	...
	while (!submit_upstream_work(work, ce->curl, ce->curl_err_str, resubmit)) {
		if (opt_lowmem) {
			applog(LOG_NOTICE,
			       "%s share being discarded to minimise memory cache", get_pool_name(pool));
			break;
		}
		resubmit = true;
...
static bool stale_work(struct work *work, bool share)
{
	...
	if (opt_fail_only && !share && pool != current_pool()
	    && !work->mandatory && pool_strategy != POOL_LOADBALANCE && pool_strategy != POOL_BALANCE) {
...
void __switch_pools(struct pool *selected, bool saveprio)
{
	...
	if (startup) {
		...
		applog(LOG_NOTICE,
		       "Startup GPU initialization... Using settings from pool %s.", get_pool_name(pool));
	...
	if (!block_exists(hexstr)) {
		...
		if (opt_morenotices) {
			if (work->longpoll) {
				if (work->stratum) {
					applog(LOG_NOTICE,
					       "Stratum from %s detected new block", get_pool_name(pool));
				} else {
					applog(LOG_NOTICE,
 ...
 updated:
	for (i = 0; i < total_pools; i++) {
		...
		wlogprint("%s Quota %d  Prio %d '%s'  User:%s\n",
			  ...
			  pool->quota, pool->prio, get_pool_name(pool), get_pool_user(pool));
...
static void display_options(void)
{
 ...
 retry:
	...
	wlogprint("[D]ebug: %s\n[P]er-device: %s\n[Q]uiet: %s\n[V]erbose: %s\n"
		  ...
		  opt_protocol ? "on" : "off",
		  opt_worktime ? "on" : "off",
		  opt_incognito ? "on" : "off", opt_compact ? "on" : "off", opt_log_interval);
...
static void hashmeter(int thr_id, struct timeval *diff, uint64_t hashes_done)
{
	...
	snprintf(statusline, sizeof(statusline),
		 ...
		 opt_log_interval, displayed_rolling, displayed_hashes,
		 total_diff_accepted, total_diff_rejected, hw_errors, total_diff1 / total_secs * 60);
...
static void
stratum_share_result(json_t * val, json_t * res_val, json_t * err_val, struct stratum_share *sshare)
{
	...
	if (opt_debug || srdiff > 0) {
		applog(LOG_INFO,
		       "Pool %d stratum share result lag time %d seconds", work->pool->pool_no, srdiff);
	}
	show_hash(work, hashshow);
...
static bool parse_stratum_response(struct pool *pool, char *s)
{
	...
	if ((json_is_null(id_val) || !id_val)
	    && (pool->algorithm.type != ALGO_ETHASH && pool->algorithm.type != ALGO_CRYPTONIGHT)) {
		char *ss;
	...
	if (!sshare) {
		...
		if (pool->algorithm.type == ALGO_CRYPTONIGHT) {
			...
			if (json_is_null(err_val)
			    && !strcmp(json_string_value(json_object_get(res_val, "status")), "OK")) {
				success = true;
			} else {
				char *ss;
...
void clear_stratum_shares(struct pool *pool)
{
	...
	if (cleared) {
		applog(LOG_WARNING,
		       "Lost %d shares due to stratum disconnect on %s", cleared, get_pool_name(pool));
...
void clear_pool_work(struct pool *pool)
{
	...
	if (cleared)
		applog(LOG_INFO,
		       "Cleared %d work items due to stratum disconnect on pool %d", cleared, pool->pool_no);
}
...
static void *stratum_rthread(void *userdata)
{
	...
	while (42) {
		...
		if (!sock_full(pool)
			...
			applog(LOG_DEBUG,
			       "Stratum select failed on %s with value %d", get_pool_name(pool), sel_ret);
			s = NULL;
		} else
			s = recv_line(pool);
		if (!s) {
		...
		if (!parse_method(pool, s) && !parse_stratum_response(pool, s))
			applog(LOG_INFO, "Unknown msg (stratum_rthread fails to parse_stratum_response): %s", s);
...
// Send the share to the pool connection. Vary stratum protocol by algorithm.
		if (pool->algorithm.type == ALGO_ETHASH) {
			...
			snprintf(s, sizeof(s),
				 ...
				 pool->XMRAuthID, work->job_id, ASCIINonce, ASCIIResult, sshare->id);
			free(ASCIINonce);
			free(ASCIIResult);
...
// Log the share submission
		...
		while (time(NULL) < sshare->sshare_time + 120) {
			...
			if (!pool_tset(pool, &pool->submit_fail) && cnx_needed(pool)) {
				applog(LOG_WARNING,
				       "%s stratum share submission failure", get_pool_name(pool));
				total_ro++;
				pool->remotefail_occasions++;
			}
...
static bool pool_active(struct pool *pool, bool pinging)
{
 ...
 retry_stratum:if (pool->has_stratum) {
	...
	if (!pool->probed) {
		...
		val =
		    json_rpc_call(curl, curl_err_str, pool->rpc_url,
				  pool->rpc_userpass, gbt_req, true, false, &rolltime, pool, false);
		if (val) {
	...
	if (pool->algorithm.type == ALGO_ETHASH) {
		...
		val =
		    json_rpc_call(curl, curl_err_str, pool->rpc_url,
				  pool->rpc_userpass, pool->rpc_req, true, false, &rolltime, pool, false);
	...
	if (val) {
		...
		if (rc) {
			applog(LOG_DEBUG,
			       "Successfully retrieved and deciphered work from %s", get_pool_name(pool));
			work->pool = pool;
		...
		if (!pool->lp_started) {
			...
			if (unlikely
			    (pthread_create(&pool->longpoll_thread, NULL, longpoll_thread, (void *)pool)))
		...
		if (!pinging)
			applog(LOG_WARNING,
			       "%s slow/down or URL or credentials invalid", get_pool_name(pool));
	}
 out:
	curl_easy_cleanup(curl);
	return ret;
...
static void gen_stratum_work_eth(struct pool *pool, struct work *work)
{
	if (pool->algorithm.type != ALGO_ETHASH)
		return;
	applog(LOG_DEBUG,
	       "[THR%d] gen_stratum_work_eth() - algorithm = %s", work->thr_id, pool->algorithm.name);
	cg_rlock(&pool->data_lock);
	work->EpochNumber = pool->EpochNumber;
...
static void gen_stratum_work_cn(struct pool *pool, struct work *work)
{
	if (pool->algorithm.type != ALGO_CRYPTONIGHT)
		return;
	applog(LOG_DEBUG,
	       "[THR%d] gen_stratum_work_cn() - algorithm = %s", work->thr_id, pool->algorithm.name);
	cg_rlock(&pool->data_lock);
	work->job_id = strdup(pool->swork.job_id);
...
static unsigned long compare_pool_settings(struct pool *oldpool, struct pool *newpool)
{
	...
	opt1 = ((oldpool) ?
		get_pool_setting(oldpool->devices,
				 ((!empty_string
				   (default_profile.devices)) ? default_profile.devices : "all")) : "");
	opt2 =
	...
	if (newpool->algorithm.type == ALGO_NEOSCRYPT) {
		opt2 =
		    ((empty_string(newpool->thread_concurrency)) ?
		     "0" : get_pool_setting(newpool->thread_concurrency, default_profile.thread_concurrency));
	}
	// otherwise use pool/profile setting or default to default profile setting
	else {
	...
	opt1 =
	    ((oldpool) ?
	     get_pool_setting(oldpool->thread_concurrency, default_profile.thread_concurrency) : "");
...
static void apply_switcher_options(unsigned long options, struct pool *pool)
{
	...
	if (opt_isset(options, SWITCHER_APPLY_DEVICE)) {
		...
		//assign pool devices if any
		if (!empty_string
		    ((opt = get_pool_setting(pool->devices, ((!empty_string(default_profile.devices))
		...
		if (pool->algorithm.type == ALGO_NEOSCRYPT) {
			opt =
			    ((empty_string(pool->thread_concurrency)) ? "0" :
			     get_pool_setting(pool->thread_concurrency, default_profile.thread_concurrency));
		}
		// otherwise use pool/profile setting or default to default profile setting
		else {
			opt = get_pool_setting(pool->thread_concurrency, default_profile.thread_concurrency);
	...
	if (opt_isset(options, SWITCHER_APPLY_GPU_POWERTUNE)) {
		if (!empty_string
		    ((opt = get_pool_setting(pool->gpu_powertune, default_profile.gpu_powertune))))
			set_gpu_powertune((char *)opt);
	}
	//GPU vddc
		...
		//switcher mode - switch on pool change
			...
			if (work->pool->pool_no != mythr->pool_no) {
				if ((pool_switch_options =
				     compare_pool_settings(pools[mythr->pool_no], work->pool)) == 0) {
					applog(LOG_DEBUG,
	...
	if (algo_switch_n >= active_threads) {
		...
		if (opt_isset(pool_switch_options, SWITCHER_HARD_RESET)) {
			...
			if (unlikely
			    (pthread_create
			     (&restart_thr, NULL,
			      restart_mining_threads_thread, (void *)(intptr_t) n_threads)))
...
{
	applog(LOG_INFO,
	       "[THR%d] %s%d: invalid nonce - HW error", thr->id, thr->cgpu->drv->name, thr->cgpu->device_id);
	mutex_lock(&stats_lock);
	hw_errors++;
	thr->cgpu->hw_errors++;
	mutex_unlock(&stats_lock);
...
static inline bool abandon_work(struct work *work, struct timeval *wdiff, uint64_t hashes)
{
	if (wdiff->tv_sec > opt_scantime ||
	    work->blk.nonce >= MAXTHREADS - hashes || hashes >= 0xfffffffe || stale_work(work, false))
		return true;
	return false;
}
...
static void hash_sole_work(struct thr_info *mythr)
{
	...
	while (likely(!cgpu->shutdown)) {
		...
		if (wu > 30 && drv->working_diff < drv->max_diff && drv->working_diff < work->work_difficulty) {
			drv->working_diff++;
			applog(LOG_DEBUG,
			       "Driver %s working diff changed to %.0f", drv->dname, drv->working_diff);
			work->device_diff = MIN(drv->working_diff, work->work_difficulty);
		...
		if (work->pool->algorithm.type == ALGO_NEOSCRYPT)
			...
			*(uint64_t *) (work->device_target + 24) = bits64 / work->device_diff;
			...
			set_target(work->device_target,
				   work->device_diff, work->pool->algorithm.diff_multiplier2, work->thr_id);
		do {
			cgtime(&tv_start);
			...
			if (unlikely((long)sdiff.tv_sec < cycle)) {
				...
				max_nonce =
				    max_nonce * 0x400 /
				    (((cycle * 1000000) + sdiff.tv_usec) / (cycle * 1000000 / 0x400));
...
convert_to_work(json_t * val, int rolltime,
		struct pool *pool, struct timeval *tv_lp, struct timeval *tv_lp_reply)
{
	struct work *work;
	bool rc;
	work = make_work();
...
static void wait_lpcurrent(struct pool *pool)
{
	while (!cnx_needed(pool)
	       && (pool->state == POOL_DISABLED || (pool != current_pool()
						    && pool_strategy !=
						    POOL_LOADBALANCE && pool_strategy != POOL_BALANCE))) {
		mutex_lock(&lp_lock);
...
static void *longpoll_thread(void *userdata)
{
 ...
 retry_pool:pool = select_longpoll_pool(cp);
	...
	if (pool->has_stratum) {
		applog(LOG_WARNING,
		       "Block change for %s detection via %s stratum", cp->rpc_url, pool->rpc_url);
		goto out;
	}
	...
	while (42) {
		...
		val =
		    json_rpc_call(curl, curl_err_str, lp_url,
				  pool->rpc_userpass, lpreq, false, true, &rolltime, pool, false);
		cgtime(&reply);
		if (likely(val)) {
		...
		if (curses_active_locked()) {
			...
			if (!opt_compact) {
				...
				for (i = 0; i < total_devices; i++) {
					...
					if (cgpu
					    && (!opt_removedisabled
						|| cgpu->deven != DEV_DISABLED || devices_enabled[i]))
...
static void noop_get_statline(char __maybe_unused
			      * buf, size_t __maybe_unused bufsiz, struct cgpu_info __maybe_unused * cgpu);
...
void print_summary(void)
{
	...
	applog(LOG_WARNING,
	       "Average hashrate: %.1f %shash/s", displayed_hashes, mhash_base ? "Mega" : "Kilo");
	...
	if (total_accepted || total_rejected)
		applog(LOG_WARNING, "Reject ratio: %.1f%%", (double)
		       (total_rejected * 100) / (double)(total_accepted + total_rejected));
	...
	if (total_pools > 1) {
		for (i = 0; i < total_pools; i++) {
			...
			if (pool->accepted || pool->rejected)
				applog(LOG_WARNING, " Reject ratio: %.1f%%", (double)
				       (pool->rejected * 100) / (double)(pool->accepted + pool->rejected));
			applog(LOG_WARNING, " Items worked on: %d", pool->works);
			applog(LOG_WARNING,
			       " Stale submissions discarded due to new blocks: %d", pool->stale_shares);
			applog(LOG_WARNING,
			       " Unable to get work from server occasions: %d", pool->getfail_occasions);
			applog(LOG_WARNING,
...
add_pool_details(struct pool * pool, bool live,
		 char *url, char *user, char *pass, char *name, char *desc, char *profile, char *algo)
{
	size_t siz;
...
static bool input_pool(bool live)
{
	...
	ret = add_pool_details(pool, live, url, user, pass, name, desc, profile, algo);
 out:
	immedok(logwin, false);
	if (!ret) {
		if (url)
...
noop_get_statline(char __maybe_unused * buf,
		  size_t __maybe_unused bufsiz, struct cgpu_info __maybe_unused * cgpu)
{
	...
	for (i = 0; i < total_devices; ++i) {
		...
		for (j = 0; j < cgpu->threads; ++j, ++k) {
			...
			applog(LOG_DEBUG, "Thread %d set pool = %d (%s)",
			       k, thr->pool_no, isnull(get_pool_name(pools[thr->pool_no]), ""));
			thr->cgpu = cgpu;
			thr->device_thread = j;
			cgtime(&thr->last);
			cgpu->thr[j] = thr;
...
int main(int argc, char *argv[])
{
		...
		for (i = 0; i < total_devices; ++i) {
			...
			if (cgpu->name)
				applog(LOG_ERR, " %2d. %s %d: %s (driver: %s)",
				       i, cgpu->drv->name, cgpu->device_id, cgpu->name, cgpu->drv->dname);
			else
 ...
 retry:
		...
		if (!get_upstream_work(work, ce->curl, ce->curl_err_str)) {
			applog(LOG_DEBUG,
			       "%s json_rpc_call failed on get work, retrying in 5s", get_pool_name(pool));
